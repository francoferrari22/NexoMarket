...
namespace NexoMarket.Licensing
{
    ...
    public static class LicenseCore
    {
        ...
        public static bool Verify(LicenseRecord r, string publicKeyXml)
        {
            try
            {
                if (r == null || string.IsNullOrWhiteSpace(r.Signature)) return false;
                string key = !string.IsNullOrWhiteSpace(publicKeyXml) ? publicKeyXml : r.PublicKeyXml;
        ...
        public static string ActivationCode(LicenseRecord r) { return "NLM1-" + Serialize(r); }
        public static string Serialize(LicenseRecord r)
        {
            string[] values = { Prefix, r.StoreId, r.MachineId, r.ClientName,
                r.Days.ToString(System.Globalization.CultureInfo.InvariantCulture), r.IssuedUtc.ToString("o"),
                r.ExpiresUtc.ToString("o"), r.Status, r.Signature, r.PublicKeyXml };
        ...
        public static bool TryParse(string token, out LicenseRecord r)
        {
            ...
            try
            {
                ...
                if (normalized.StartsWith("NLM1-", StringComparison.OrdinalIgnoreCase)) normalized = normalized.Substring(5);
                if (normalized.StartsWith("ACT-", StringComparison.OrdinalIgnoreCase)) normalized = normalized.Substring(4);
                normalized = normalized.Replace("\r", "").Replace("\n", "").Replace(" ", "");
                string[] p = normalized.Split('.');
                if (p.Length != 9 && p.Length != 10) return false;
                if (D(p[0]) != Prefix) return false;
                r = new LicenseRecord();
                ...
                r.Status = D(p[7]); r.Signature = D(p[8]); r.PublicKeyXml = p.Length == 10 ? D(p[9]) : "";
                return true;
            }
            catch { return false; }
        }
        ...
        public static int DaysRemaining(LicenseRecord r, DateTime utcNow)
        {
            ...
            return Math.Max(0, (int)(r.ExpiresUtc.Date - utcNow.Date).TotalDays);
        }
    }
}
