...
namespace NexoMarket.Admin.UI
{
    public sealed class LicenseService
    {
        ...
        public string TokenPath { get { return Path.Combine(_dataRoot,"nexomarket.account.token"); } }
        public string SavedToken()
        {
            try { return File.Exists(TokenPath) ? File.ReadAllText(TokenPath, Encoding.UTF8).Trim() : ""; }
            catch { return ""; }
        }
        ...
        public string AccountId(){
            try
            {
                string stored = GetSetting("seller_account_id");
                if (!string.IsNullOrWhiteSpace(stored)) return stored.Trim().ToUpperInvariant();
                string email=AccountEmail().Trim().ToLowerInvariant();
                if(email.Length==0)return "";
                using(var sha=System.Security.Cryptography.SHA256.Create())
                {
                    byte[] b=sha.ComputeHash(Encoding.UTF8.GetBytes("NEXOMARKET-ACCOUNT|"+email));
                    var sb=new StringBuilder(); foreach(byte x in b)sb.Append(x.ToString("X2"));
                    string id=sb.ToString();
                    try { SetSetting("seller_account_id",id); } catch {}
                    return id;
                }
            }
            catch { return ""; }
        ...
        private string GetSetting(string key){
            try { XDocument d=Data(); foreach(XElement x in d.Root.Element("Settings").Elements("Setting")) if(string.Equals((string)x.Attribute("Key"),key,StringComparison.OrdinalIgnoreCase)) return (string)x.Attribute("Value")??""; } catch {}
            return "";
        }
        private void SetSetting(string key,string value){
            try { XDocument d=Data(); XElement settings=d.Root.Element("Settings"); XElement x=settings.Elements("Setting").FirstOrDefault(e=>string.Equals((string)e.Attribute("Key"),key,StringComparison.OrdinalIgnoreCase)); if(x==null) settings.Add(new XElement("Setting",new XAttribute("Key",key),new XAttribute("Value",value??""))); else x.SetAttributeValue("Value",value??""); d.Save(Path.Combine(_dataRoot,"nexomarket_data.xml")); } catch {}
        }
        ...
        {
            string status; int daysRemaining; DateTime expiresUtc;
            return EnsureAccountTrial(baseUrl, out status, out daysRemaining, out expiresUtc);
        }
