...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        {
            if(method=="GET") {
                string html = "<div class='card'><div class='eyebrow'>VENDEDOR</div><h1>Seller Center</h1><p class='muted'>Ingresá con el correo y la contraseña de tu cuenta. El Store ID se obtiene automáticamente de la cuenta central.</p><a class='btn violet' href='/seller-login'>INGRESAR COMO VENDEDOR</a></div>" +
        ...
        private void CentralSellerRegister(NetworkStream stream,string method,string body)
        {
            ...
            CentralUser u=FindAccount(email); string token=Guid.NewGuid().ToString("N"); lock(_sync)_sessions[token]=u;
            WriteRedirectCookie(stream,"/seller","NexoCentralSession="+token+"; Path=/; HttpOnly; SameSite=Lax");
        }
