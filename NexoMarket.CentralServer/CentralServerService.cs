    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private string AdminResetStoreStatus(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                ...
                e.SetElementValue("Listed","1");
                e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));
                Save();
            }
            Audit("admin_store_status_reset",id,"","","");
            return "OK|status_reset";
        }
...
"@media(max-width:650px){.store-product-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px!important}.store-product-card{padding:7px!important}.store-product-card h3{font-size:14px}.store-product-card .product-description{font-size:10px}.store-product-card .price{font-size:17px}.store-product-card .btn{font-size:10px;padding:8px 5px}}@media(max-width:1000px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.grid{grid-template-columns:1fr;gap:12px}.grid .card{min-height:300px;height:300px}.grid .card .name{font-size:20px}.grid .card .logo-wrap,.grid .card .logo-wrap .logo{width:68px;height:68px}}</style></head><body><div class='wrap'><div class='top'><a class='brand brand-home' href='/' aria-label='Volver al inicio de NexoMarket'><span>NEXO</span>MARKET</a><div class='nav'>"+storeAccountLinks+"</div></div><button id='scrollHome' class='scroll-home' type='button' onclick=\"location.href='/'\">NEXO<span>MARKET</span></button>"+(string.IsNullOrWhiteSpace(S(store,"StorePhoto"))?"":"<div class='store-cover-shell'><img class='store-cover-photo' src='"+E(S(store,"StorePhoto"))+"' alt='Foto del local' loading='eager'/><div class='store-cover-caption'>"+E(S(store,"Name"))+"</div></div>")+"<section class='hero store-hero'>").Append(logo).Append("<div class='store-hero-main'>").Append(publicTier).Append("<div class='live'>● TIENDA CENTRAL EN TIEMPO REAL</div><h1>").Append(E(S(store,"Name"))).Append("</h1><div class='muted'>").Append(E(S(store,"Category"))).Append(" · ").Append(E(S(store,"City"))).Append("</div><p class='muted'>").Append(E(S(store,"Description"))).Append("</p></div><div class='hero-rating'><div class='hero-rating-label'>REPUTACIÓN</div><div class='hero-rating-score'><span id='heroReviewAverage'>0.0</span><span class='hero-rating-max'>/5</span></div><div class='hero-rating-stars' id='heroReviewStars'>☆☆☆☆☆</div><div class='hero-rating-count' id='heroReviewCount'>0 reseñas</div></div></section>"+(S(store,"Active")=="0"?"<div class='closed-store-banner'>🔴 TIENDA CERRADA · En este momento no se aceptan nuevos pedidos.</div>":"")+"<div class='section'><div style='display:flex;justify-content:space-between;align-items:end'><div><h2>Productos</h2><div class='muted'>El catálogo se actualiza automáticamente desde NexoMarket Central.</div></div><span id='liveState' class='live'>● LIVE</span></div><div id='products' class='grid store-product-grid'>");
        ...
        private string Marketplace(string query, string cookie)
        {
            ...
            if (stores.Count > 0) { b.Append("<div class='nexo-directory-v8' data-card-design='NEXO-V8'>"); foreach (CentralStore cs in stores) { string href = "/store/" + Uri.EscapeDataString(cs.StoreId); string d = cs.Distance > 0 ? cs.Distance.ToString("0.0") + " km" : ""; string logo = string.IsNullOrWhiteSpace(cs.Logo) ? "<span class='nexo-logo-letter'>N</span>" : "<img src='" + E(cs.Logo) + "' alt='" + E(cs.Name) + "' loading='lazy'/>"; string promoHtml=StorePromoBadges(cs.StoreId); string cover = string.IsNullOrWhiteSpace(cs.StorePhoto) ? "<div class='nexo-cover-fallback'><span>NEXO</span>MARKET</div>" : "<img class='nexo-cover-image' src='"+E(cs.StorePhoto)+"' alt='"+E(cs.Name)+"' loading='lazy' decoding='async' onerror=\"this.style.display='none';this.nextElementSibling.classList.add('show')\"/>"; string[] ratingParts=(cs.RatingSummary??"0.0|0").Split('|'); string rating=ratingParts.Length>0?ratingParts[0]:"0.0"; string reviewCount=ratingParts.Length>1?ratingParts[1]:"0"; bool renderStorePlus = cs.StorePlus; bool renderCommon = !renderStorePlus && cs.Featured; string tier=renderStorePlus?" nexo-store-plus":(renderCommon?" nexo-featured":""); string tierBadge=renderStorePlus?"<span class='nexo-tier store-plus' data-tier='STORE_PLUS'>✦ TIENDA PLUS</span>":(renderCommon?"<span class='nexo-tier' data-tier='COMMON'>★ TIENDA DESTACADA</span>":""); string starCorner=(cs.StorePlus||cs.Featured)?"<span class='nexo-star'>★</span>":""; string delivery = cs.Delivery ? "<span>🚚 Delivery</span>" : ""; string pickup = cs.Pickup ? "<span>🏪 Retiro</span>" : ""; string desc=string.IsNullOrWhiteSpace(cs.Description)?"Comercio local en NexoMarket.":(cs.Description.Length>105?cs.Description.Substring(0,105)+"…":cs.Description); b.Append("<a class='nexo-store-v8"+tier+"' href='"+E(href)+"' data-store-id='"+E(cs.StoreId)+"' data-store-plus='"+(cs.StorePlus?"1":"0")+"'><div class='nexo-cover-v8'>"+cover+"<div class='nexo-cover-fallback-layer'><span>NEXO</span>MARKET</div><div class='nexo-cover-shade'></div><div class='nexo-cover-glow'></div><div class='nexo-shine'></div>"+tierBadge+"</div><div class='nexo-logo-v8'><div class='nexo-logo-box'>"+logo+"</div>"+starCorner+"</div><div class='nexo-card-body'><div class='nexo-kicker'>"+E(cs.Category.Length==0?"COMERCIO":cs.Category)+" <i>·</i> "+E(cs.City)+"</div><h3>"+E(cs.Name)+"</h3><div class='nexo-status "+(cs.Active?"open":"closed")+"'>● "+(cs.Active?"ABIERTO":"CERRADO")+"</div><p class='nexo-description'>"+E(desc)+"</p><div class='nexo-facts'>"+(string.IsNullOrWhiteSpace(d)?"":"<span>📍 "+E(d)+"</span>")+delivery+pickup+"</div><div class='nexo-bottom'><span class='nexo-rating'>★ "+E(rating)+" <small>("+E(reviewCount)+")</small></span><span class='nexo-arrow'>VER LOCAL <b>→</b></span></div>"+promoHtml+"</div></a>"); } b.Append("</div>"); }
