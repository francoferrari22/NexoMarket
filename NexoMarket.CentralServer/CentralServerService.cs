...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        // Migra registros anteriores al nuevo nivel explícito sin cambiar su estado promocional.
        private void NormalizeFeaturedTiers()
        {
            lock(_sync)
            {
                ...
                {
                    string level=FeaturedLevel(e);
                    if(S(e,"FeaturedTier") != level) { e.SetElementValue("FeaturedTier",level); changed=true; }
                    string f=level=="COMMON"?"1":"0", p=level=="PLUS"?"1":"0";
                    if(S(e,"Featured") != f) { e.SetElementValue("Featured",f); changed=true; }
                    if(S(e,"FeaturedPlus") != p) { e.SetElementValue("FeaturedPlus",p); changed=true; }
                }
                if(changed) Save();
            }
        }
        ...
        // DESTACADA PLUS V2 — fuente única de verdad para el nivel promocional público.
        // PLUS se guarda como nivel explícito y no depende de la bandera visual Featured.
        private static string FeaturedLevel(XElement e)
        {
            if (e == null) return "NONE";
            string level = S(e, "FeaturedTier");
            if (string.Equals(level, "PLUS", StringComparison.OrdinalIgnoreCase)) return "PLUS";
            if (S(e, "FeaturedPlus") == "1") return "PLUS";
            if (string.Equals(level, "COMMON", StringComparison.OrdinalIgnoreCase)) return "COMMON";
            if (S(e, "Featured") == "1") return "COMMON";
            return "NONE";
        }

        private static bool IsFeaturedPlus(XElement e) { return FeaturedLevel(e) == "PLUS"; }
        private static bool IsFeaturedCommon(XElement e) { return FeaturedLevel(e) == "COMMON"; }
        ...
        private string Register(Dictionary<string,string> f)
        {
            ...
            lock (_sync)
            {
                ...
                string adminLevel = old == null ? ((Get(f,"featuredPlus") == "1") ? "PLUS" : (Get(f,"featured") == "1" ? "COMMON" : "NONE")) : FeaturedLevel(old);
                string adminFeatured = adminLevel == "COMMON" ? "1" : "0";
                string adminFeaturedPlus = adminLevel == "PLUS" ? "1" : "0";
                XElement e = new XElement("Store", new XAttribute("UpdatedAt", string.IsNullOrWhiteSpace(Get(f,"updatedAt")) ? DateTime.UtcNow.ToString("o") : Get(f, "updatedAt")),
                    ...
                    new XElement("Slug", Get(f, "slug")), new XElement("PublicUrl", Get(f, "publicUrl")), new XElement("Featured", adminFeatured), new XElement("FeaturedPlus", adminFeaturedPlus), new XElement("FeaturedTier", adminLevel), new XElement("Listed", "1"), new XElement("Active", active),
        ...
        private string AdminSetStoreFeatured(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync){XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));if(e==null)return "ERROR|store_not_found";e.SetElementValue("Featured",featured?"1":"0");e.SetElementValue("FeaturedPlus","0");e.SetElementValue("FeaturedTier",featured?"COMMON":"NONE");e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));Save();}
        ...
        private string AdminSetStoreFeaturedPlus(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                ...
                // DESTACADA PLUS V2 es un nivel independiente y explícito.
                e.SetElementValue("Featured",plus?"0":"0");
                e.SetElementValue("FeaturedTier",plus?"PLUS":"NONE");
        ...
        private string StoreLines(string query)
        {
            ...
            if (hasCoords) list = list.OrderByDescending(x => IsFeaturedPlus(x.Element)).ThenByDescending(x => IsFeaturedCommon(x.Element)).ThenByDescending(x => S(x.Element, "Active") != "0").ThenBy(x => x.DistanceKm).ThenBy(x => S(x.Element, "Name")).ToList();
            else list = list.OrderByDescending(x => IsFeaturedPlus(x.Element)).ThenByDescending(x => IsFeaturedCommon(x.Element)).ThenByDescending(x => S(x.Element, "Active") != "0").ThenBy(x => S(x.Element, "City")).ThenBy(x => S(x.Element, "Name")).ToList();
            StringBuilder b = new StringBuilder();
        ...
        private string Storefront(string slug, string cookie, string query)
        {
            ...
            bool featuredPlus=IsFeaturedPlus(store), featured=IsFeaturedCommon(store);
...
"@media(max-width:650px){.store-product-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px!important}.store-product-card{padding:7px!important}.store-product-card h3{font-size:14px}.store-product-card .product-description{font-size:10px}.store-product-card .price{font-size:17px}.store-product-card .btn{font-size:10px;padding:8px 5px}}@media(max-width:1000px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.grid{grid-template-columns:1fr;gap:12px}.grid .card{min-height:300px;height:300px}.grid .card .name{font-size:20px}.grid .card .logo-wrap,.grid .card .logo-wrap .logo{width:68px;height:68px}}</style></head><body><div class='wrap'><div class='top'><a class='brand brand-home' href='/' aria-label='Volver al inicio de NexoMarket'><span>NEXO</span>MARKET</a><div class='nav'>"+storeAccountLinks+"</div></div><button id='scrollHome' class='scroll-home' type='button' onclick=\"location.href='/'\">NEXO<span>MARKET</span></button>"+(string.IsNullOrWhiteSpace(S(store,"StorePhoto"))?"":"<div class='store-cover-shell'><img class='store-cover-photo' src='"+E(S(store,"StorePhoto"))+"' alt='Foto del local' loading='eager'/><div class='store-cover-caption'>"+E(S(store,"Name"))+"</div></div>")+"<section class='hero store-hero'>").Append(logo).Append("<div class='store-hero-main'>").Append(publicTier).Append("<div class='live'>● TIENDA CENTRAL EN TIEMPO REAL</div><h1>").Append(E(S(store,"Name"))).Append("</h1><div class='muted'>").Append(E(S(store,"Category"))).Append(" · ").Append(E(S(store,"City"))).Append("</div><p class='muted'>").Append(E(S(store,"Description"))).Append("</p></div><div class='hero-rating'><div class='hero-rating-label'>REPUTACIÓN</div><div class='hero-rating-score'><span id='heroReviewAverage'>0.0</span><span class='hero-rating-max'>/5</span></div><div class='hero-rating-stars' id='heroReviewStars'>☆☆☆☆☆</div><div class='hero-rating-count' id='heroReviewCount'>0 reseñas</div></div></section>"+(S(store,"Active")=="0"?"<div class='closed-store-banner'>🔴 TIENDA CERRADA · En este momento no se aceptan nuevos pedidos.</div>":"")+"<div class='section'><div style='display:flex;justify-content:space-between;align-items:end'><div><h2>Productos</h2><div class='muted'>El catálogo se actualiza automáticamente desde NexoMarket Central.</div></div><span id='liveState' class='live'>● LIVE</span></div><div id='products' class='grid store-product-grid'>");
        ...
        private string StoreJson(string query)
        {
            ...
            using (StringReader reader = new StringReader(lines))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    cs.Distance = p.Length > 13 ? ParseDouble(Uri.UnescapeDataString(p[13])) : 0d; cs.Logo = p.Length > 14 ? Uri.UnescapeDataString(p[14]) : ""; string publicTier = p.Length > 22 ? Uri.UnescapeDataString(p[22]) : ""; bool publicPlus = (p.Length > 20 && Uri.UnescapeDataString(p[20]) == "1") || string.Equals(publicTier, "PLUS", StringComparison.OrdinalIgnoreCase); bool publicCommon = !publicPlus && (p.Length > 15 && Uri.UnescapeDataString(p[15]) == "1"); cs.FeaturedPlus = publicPlus; cs.Featured = publicCommon; cs.FeaturedTier = publicPlus ? "PLUS" : (publicCommon ? "COMMON" : "NONE"); cs.StorePhoto = p.Length > 16 ? Uri.UnescapeDataString(p[16]) : ""; cs.Address = p.Length > 17 ? Uri.UnescapeDataString(p[17]) : ""; cs.Description = p.Length > 18 ? Uri.UnescapeDataString(p[18]) : ""; cs.RatingSummary = p.Length > 19 ? Uri.UnescapeDataString(p[19]) : "0.0|0";
                    stores.Add(cs);
                }
            }
        ...
        private string Marketplace(string query, string cookie)
        {
            ...
            List<CentralStore> stores = new List<CentralStore>();
            {
                ...
                using (StringReader reader = new StringReader(lines)) { string line; while ((line = reader.ReadLine()) != null) { string[] p = line.Split('|'); if (p.Length < 12) continue; CentralStore cs = new CentralStore(); cs.StoreId = Uri.UnescapeDataString(p[1]); cs.Name = Uri.UnescapeDataString(p[2]); cs.PublicUrl = Uri.UnescapeDataString(p[3]); cs.City = Uri.UnescapeDataString(p[4]); cs.Province = Uri.UnescapeDataString(p[5]); cs.Category = Uri.UnescapeDataString(p[6]); cs.Latitude = ParseDouble(Uri.UnescapeDataString(p[7])); cs.Longitude = ParseDouble(Uri.UnescapeDataString(p[8])); cs.Active = Uri.UnescapeDataString(p[9]) == "1"; cs.Delivery = Uri.UnescapeDataString(p[10]) == "1"; cs.Pickup = Uri.UnescapeDataString(p[11]) == "1"; cs.Distance = p.Length > 13 ? ParseDouble(Uri.UnescapeDataString(p[13])) : 0d; cs.Logo = p.Length > 14 ? Uri.UnescapeDataString(p[14]) : ""; string publicTier = p.Length > 22 ? Uri.UnescapeDataString(p[22]) : ""; bool publicPlus = (p.Length > 20 && Uri.UnescapeDataString(p[20]) == "1") || string.Equals(publicTier, "PLUS", StringComparison.OrdinalIgnoreCase); bool publicCommon = !publicPlus && (p.Length > 15 && Uri.UnescapeDataString(p[15]) == "1"); cs.FeaturedPlus = publicPlus; cs.Featured = publicCommon; cs.FeaturedTier = publicPlus ? "PLUS" : (publicCommon ? "COMMON" : "NONE"); cs.StorePhoto = p.Length > 16 ? Uri.UnescapeDataString(p[16]) : ""; cs.Address = p.Length > 17 ? Uri.UnescapeDataString(p[17]) : ""; cs.Description = p.Length > 18 ? Uri.UnescapeDataString(p[18]) : ""; cs.RatingSummary = p.Length > 19 ? Uri.UnescapeDataString(p[19]) : "0.0|0"; cs.Phone = p.Length > 21 ? Uri.UnescapeDataString(p[21]) : ""; stores.Add(cs); } }
            }
            StringBuilder b = new StringBuilder();
