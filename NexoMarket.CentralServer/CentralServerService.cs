...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private void CentralSeller(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append("<div class='welcome'><div><span class='eyebrow'>CENTRAL DE VENTAS</span><h1>Hola, "+E(u.Name)+" 👋</h1><p>Tu Seller Center está conectado por Store ID con NexoMarket Windows. Los cambios se sincronizan automáticamente.</p><div class='section-actions'><a class='btn violet' href='/seller/devices'>🔗 VINCULAR WINDOWS</a><a class='btn ghost' href='/seller?view=products'>📦 PRODUCTOS</a><a class='btn ghost' href='/seller?view=orders'>🧾 PEDIDOS</a></div></div><div class='account-mini'><b>STORE ID</b><strong>"+E(u.StoreId)+"</strong><small>"+E(u.Email)+"</small></div></div>");
        ...
        private string SellerProductsView(List<XElement> products)
        {
            ...
            {
                string pid=S(x,"ProductId"), img=S(x,"WebImageUrl"); decimal stock=Money(S(x,"Stock")), min=Money(S(x,"MinimumStock")); string price=Money(S(x,"SalePrice")=="0"?S(x,"Price"):S(x,"SalePrice")).ToString("N2");
        ...
        private void CentralSellerDevices(NetworkStream stream,string cookie,string method,string body)
        {
            ...
            string payload=PairPayload(u.StoreId,pair);
            ...
            b.Append("<main class='sc-main' style='margin-left:0'><div class='welcome'><div><span class='eyebrow'>VINCULACIÓN WINDOWS</span><h1>Conectar NexoMarket Windows</h1><p>Este código es temporal, de un solo uso y está vinculado a tu cuenta y Store ID actuales.</p></div><div class='account-mini'><b>STORE ID</b><strong>"+E(u.StoreId)+"</strong><small>"+E(u.Email)+"</small></div></div>");
            b.Append("<section class='card pairing-card'><div class='eyebrow'>CÓDIGO PARA COPIAR</div><h2>Copiá este código en Windows</h2><div class='pair-code' id='pairCode'>"+E(pair)+"</div><button class='btn violet' type='button' onclick=\"navigator.clipboard&&navigator.clipboard.writeText(document.getElementById('pairCode').innerText).then(function(){this.innerText='✓ COPIADO';}.bind(this))\">COPIAR CÓDIGO</button><p class='muted'>Vence en 10 minutos y solo puede utilizarse una vez.</p><div class='pair-instructions'><b>En Windows:</b><br/>Cuenta central → <b>Vincular Windows</b> → pegá el código → confirmá.</div>"+qr+"<a class='btn ghost' href='/seller/devices'>GENERAR OTRO CÓDIGO</a></section></main>");
        ...
        private string SellerToolsView(CentralUser u,List<XElement> products,List<XElement> orders){return "<div class='section-title'><div><span class='eyebrow'>HERRAMIENTAS</span><h2>Centro de operaciones</h2><p>Accesos rápidos para gestionar tienda, catálogo, pedidos, clientes y conexión.</p></div></div><div class='quick-grid'><a href='/store/"+Uri.EscapeDataString(u.StoreId??"")+"'>🌐 Ver escaparate público</a><a href='/seller?view=products'>📦 Catálogo e inventario ("+products.Count+")</a><a href='/seller?view=orders'>🧾 Pedidos ("+orders.Count+")</a><a href='/seller?view=customers'>👥 Clientes</a><a href='/seller?view=analytics'>📊 Analítica y gráficos</a><a href='/seller?view=finance'>💰 Finanzas</a><a href='/seller?view=settings'>⚙ Configuración</a><a href='/seller/devices'>🔗 Vincular Windows</a><a href='/seller?view=marketing'>🎯 Marketing</a></div><section class='card'><h3>Sincronización</h3><p>La PC con NexoMarket Windows publica productos, promociones, cuentas y recibe pedidos del marketplace. El Store ID identifica al comercio y mantiene una sola identidad aunque cambien las versiones.</p><div class='sync-pill'>● CUENTA: "+E(u.Email)+" · ● STORE ID: "+E(u.StoreId)+"</div></section>";}
