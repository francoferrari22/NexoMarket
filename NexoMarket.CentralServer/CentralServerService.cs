...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private string Storefront(string slug)
        {
            ...
            {
                XDocument cd = LoadFile(_catalogFile, "NexoMarketCatalog", "Products");
                if (cd.Root.Element("Products") != null)
                {
                    productCategories = cd.Root.Element("Products").Elements("Product")
                        .Where(x => S(x, "StoreId") == realId && S(x, "Deleted") != "1" && S(x, "OnlineEnabled") != "0" && S(x, "Active") != "0")
                        .Select(x => S(x, "Category").Trim())
            ...
            lock(_sync)
            {
                ...
                foreach (XElement x in ps)
                {
                    ...
                    b.Append("<div class='card product-card' data-category='").Append(E(S(x, "Category"))).Append("'>").Append(image).Append("<h3>").Append(E(S(x, "Name"))).Append("</h3><div class='muted'>")
            ...
            b.Append("<button class='cart-fab' onclick='toggleCart()'>🛒 Carrito <span id='cartCount'>0</span></button><aside id='cartPanel' class='cart-panel cart'><h2>Tu carrito</h2><div id='cartItems'>Carrito vacío.</div><h3>Total: $ <span id='total'>0</span></h3>");
            b.Append("<form onsubmit='return sendOrder(event)'><input type='hidden' id='storeId' value='").Append(E(realId)).Append("'/><input id='name' placeholder='Nombre completo' required/><input id='email' type='email' placeholder='Correo electrónico'/><input id='phone' placeholder='Teléfono'/><select id='fulfillment'><option>Delivery</option><option>Retiro</option></select><input id='address' placeholder='Dirección / punto de retiro'/><select id='paymentMethod'><option>Transferencia</option><option>Mercado Pago</option><option>Efectivo</option></select><input id='paymentReference' placeholder='Referencia de pago (opcional)'/>");
        ...
        private void CentralBuyer(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append("</table></section><section class='card buyer-enhanced-panel'><div class='section-title'><div><span class='eyebrow'>SEGUIMIENTO INTELIGENTE</span><h2>Seguimiento de mis pedidos</h2><p>Actualización automática y confirmación de recepción.</p></div></div><div id='buyerTrackingCards'>");
