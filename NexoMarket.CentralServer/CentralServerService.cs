...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        // TIENDA PLUS — función promocional independiente.
        // StorePlus es independiente de Featured; los campos antiguos solo se leen durante la migración inicial.
        private void NormalizeStorePlus()
        {
            lock(_sync)
            {
                ...
                {
                    string legacy = (S(e,"FeaturedPlus") == "1" || string.Equals(S(e,"FeaturedTier"),"PLUS",StringComparison.OrdinalIgnoreCase)) ? "1" : "0";
                    string current = S(e,"StorePlus");
                    if(current != "1" && current != "0") current = legacy;
                    if(legacy == "1") current = "1";
                    if(S(e,"StorePlus") != current) { e.SetElementValue("StorePlus",current); changed=true; }
                    if(e.Element("FeaturedPlus") != null) { e.Element("FeaturedPlus").Remove(); changed=true; }
                    if(e.Element("FeaturedTier") != null) { e.Element("FeaturedTier").Remove(); changed=true; }
                }
                if(changed) Save();
            }
        }
        ...
        private string AdminSetStoreFeatured(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync){XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));if(e==null)return "ERROR|store_not_found";e.SetElementValue("Featured",featured?"1":"0");e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));Save();}
        ...
        private string AdminSetStorePlus(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));
                if(e==null)return "ERROR|store_not_found";
                e.SetElementValue("StorePlus",plus?"1":"0");
