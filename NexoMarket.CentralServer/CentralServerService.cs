...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private string Marketplace(string query, string cookie)
        {
            ...
            if (stores.Count > 0) { b.Append("<div class='grid'>"); foreach (CentralStore cs in stores) { string href = "/store/" + Uri.EscapeDataString(cs.StoreId); string d = cs.Distance > 0 ? cs.Distance.ToString("0.0") + " km · " : ""; string logo = string.IsNullOrWhiteSpace(cs.Logo) ? "<span>N</span>" : "<img src='" + E(cs.Logo) + "' alt='" + E(cs.Name) + "' loading='lazy'/>"; string promoHtml=StorePromoBadges(cs.StoreId); string photoHtml=string.IsNullOrWhiteSpace(cs.StorePhoto)?"":"<div class='store-photo-mini'><img src='"+E(cs.StorePhoto)+"' alt='"+E(cs.Name)+"' loading='lazy'/></div>"; string reviewSummary=ReviewSummaryForStore(cs.StoreId); string[] reviewParts=reviewSummary.Split('|'); string reviewAvg=reviewParts.Length>0?reviewParts[0]:"0.0"; string reviewCount=reviewParts.Length>1?reviewParts[1]:"0"; b.Append("<a class='card store-directory-card' href='" + E(href) + "'><div class='logo'>" + logo + "</div>" + (cs.SuperFeatured ? "<div class='super-featured-badge' style='--sf:"+E(cs.FeaturedNeonColor)+"'>★ SÚPER DESTACADA</div>" : (cs.Featured ? "<div class='featured-badge'>★ TIENDA DESTACADA EN LA PLATAFORMA</div>" : "")) + "<div class='name'>" + E(cs.Name) + "</div><div class='meta'>" + E(cs.Category.Length == 0 ? "Comercio" : cs.Category) + " · " + E(cs.City) + "</div><div class='open " + (cs.Active ? "" : "closed") + "'>● " + (cs.Active ? "Abierto" : "Cerrado") + "</div><div class='meta centered-store-info'>"+E(cs.Description.Length>150?cs.Description.Substring(0,150)+"…":cs.Description)+"</div><div class='meta centered-store-info'>"+(cs.Delivery?"🚚 Delivery":"🏪 Retiro")+" · "+E(cs.Address)+"</div><div class='meta centered-store-info'>🕒 Horario: "+E(cs.OpenTime)+" a "+E(cs.CloseTime)+"</div><div class='meta centered-store-info'>💳 Pagos disponibles</div><div class='distance'>📍 " + E(d) + "</div><div class='rating-mini'>⭐ "+E(reviewAvg)+"/5 · "+E(reviewCount)+" opiniones</div>"+promoHtml+photoHtml+"</a>"); } b.Append("</div>"); }
        ...
        private void CentralSeller(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append("<script>(function(){var last='',lastPending=-1,audioCtx=null;function beep(){try{audioCtx=audioCtx||new(window.AudioContext||window.webkitAudioContext)();var o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type='sine';o.frequency.value=880;g.gain.value=.11;o.connect(g);g.connect(audioCtx.destination);var now=audioCtx.currentTime;o.start(now);o.frequency.setValueAtTime(880,now);o.frequency.linearRampToValueAtTime(1175,now+.18);o.frequency.linearRampToValueAtTime(1480,now+.36);setTimeout(function(){try{o.stop();}catch(e){}},680);if(audioCtx.state==='suspended')audioCtx.resume();}catch(e){}}function showNewOrderAlert(){var el=document.getElementById('newOrderAlert');if(!el)return;el.className='new-order-alert show';clearTimeout(window.__nexoOrderAlertTimer);window.__nexoOrderAlertTimer=setTimeout(function(){el.className='new-order-alert';},10000);}function live(){var x=new XMLHttpRequest();x.open('GET','/api/seller/live',true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText),v=d.updatedAt||'',pending=parseInt(d.pendingOrders||0,10);if(lastPending>=0&&pending>lastPending){beep();showNewOrderAlert();}lastPending=pending;var editing=document.querySelector('form.product-form input:focus,form.product-form textarea:focus,form.edit-form input:focus,form.edit-form textarea:focus,form.settings-form input:focus,form.settings-form textarea:focus');var hasDraft=document.querySelector('form.product-form input:not([type=hidden])[value]:not([value=\"\"])')||document.querySelector('form.product-form textarea:not(:placeholder-shown)');if(last&&v!==last&&!editing&&!document.querySelector('details[open]')&&!hasDraft&&!document.querySelector('.order-cards'))location.reload();last=v;}catch(e){}}};x.send();}setTimeout(live,900);setInterval(live,2500);function loadPlatformFee(){var x=new XMLHttpRequest();x.open('GET','/api/seller/platform-fee',true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText),el=document.getElementById('platformFeeMini');if(!el)return;var rate=Number(d.rate||0),amount=Number(d.amount||0);el.textContent=rate>0?('Pago plataforma · '+rate+'% · $ '+amount.toFixed(2)+' · '+d.month):'Pago plataforma · sin porcentaje';}catch(e){}}};x.send();}loadPlatformFee();setInterval(loadPlatformFee,30000);window.shareMyStore=function(){var u=location.href.split('?')[0];if(navigator.share){navigator.share({title:document.title,text:'Mirá mi tienda en NexoMarket',url:u}).catch(function(){});}else if(navigator.clipboard){navigator.clipboard.writeText(u).then(function(){alert('Enlace de la tienda copiado.');});}else{prompt('Copiá este enlace',u);}};document.addEventListener('click',function(){if(audioCtx&&audioCtx.state==='suspended')audioCtx.resume();},{once:true});})();</script>");
