...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private string Register(Dictionary<string,string> f)
        {
            ...
            lock (_sync)
            {
                ...
                XElement e = new XElement("Store", new XAttribute("UpdatedAt", string.IsNullOrWhiteSpace(Get(f,"updatedAt")) ? DateTime.UtcNow.ToString("o") : Get(f, "updatedAt")),
                    ...
                    new XElement("Province", Get(f, "province")), new XElement("Description", Get(f, "description")), new XElement("SystemName", Get(f, "systemName")), new XElement("Logo", Get(f, "logo")),
        ...
        private string StoreConnect(string storeId)
        {
            ...
            lock (_sync)
            {
                ...
                return "OK|" + Escape(storeId) + "|" + Escape(S(store, "Name")) + "|1|" + Escape(syncKey) +
                    ...
                    "|" + Escape(S(store, "Pickup")) + "|" + Escape(S(store, "Latitude")) + "|" + Escape(S(store, "Longitude")) + "|" + Escape(S(store, "SystemName")) + "|" + Escape(S(store, "Featured")) + "|" + Escape(S(store, "Listed"));
            }
        }
        ...
        private void CentralSeller(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append(SellerLink("Resumen","",view)+SellerLink("Pedidos","orders",view)+SellerLink("Deliveries","deliveries",view)+SellerLink("Punto de venta","pos",view)+SellerLink("Productos e inventario","products",view)+SellerLink("Clientes","customers",view)+SellerLink("Analítica","analytics",view)+SellerLink("Finanzas y caja","finance",view)+SellerLink("Marketing","marketing",view)+SellerLink("Reputación","reputation",view)+SellerLink("Herramientas","tools",view)+SellerLink("Configuración","settings",view)+SellerLink("Tienda Online","online",view)+"</aside>");
        ...
        private string SellerProductsView(List<XElement> products)
        {
            ...
            b.Append("<div class='media-pickers'><label class='upload-box'><span>📷</span><b>Imagen principal</b><small>Elegí una foto del teléfono/PC. Se comprime automáticamente y se guarda en almacenamiento central.</small><input id='newImageFile' type='file' accept='image/jpeg,image/png,image/webp,image/gif' capture='environment'/></label><div class='media-preview' id='newMediaPreview'><span>La vista previa aparecerá aquí</span></div></div>");
        ...
        private void CentralSellerStoreSave(NetworkStream stream,string cookie,Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                ...
                string[] fields={"Name","SystemName","LegalName","Category","Address","City","Province","Description","Logo","Slug","Delivery","Pickup","Latitude","Longitude"};
        ...
        private string SellerOnlineSettingsView(CentralUser u)
        {
            ...
            return "<div class='section-title'><div><span class='eyebrow'>TIENDA ONLINE</span><h2>Configurar mi tienda online</h2><p>Nombre, identidad visual, ubicación y datos públicos. Los cambios se conservan en Central y se sincronizan con Windows sin volver al nombre NexoMarket.</p></div></div>"+
            "<section class='card'><form method='post' action='/seller/store/save' class='settings-form'><div class='form-grid'>"+
            ...
            "<label>Nombre legal<input name='legalName' value='"+E(S(store,"LegalName"))+"'/></label>"+
            "<label>Categoría<input name='category' value='"+E(S(store,"Category"))+"'/></label>"+
            ...
            "<label>Dirección<input name='address' value='"+E(S(store,"Address"))+"'/></label>"+
            "<label>Latitud<input name='latitude' value='"+E(S(store,"Latitude"))+"' placeholder='-34.60'/></label><label>Longitud<input name='longitude' value='"+E(S(store,"Longitude"))+"' placeholder='-58.38'/></label>"+
            "<label>Logo URL<input name='logo' value='"+E(S(store,"Logo"))+"' placeholder='https://...'/></label>"+
            "<label>Slug / URL<input name='slug' value='"+E(S(store,"Slug"))+"'/></label></div>"+
            "<label>Descripción pública<textarea name='description'>"+E(S(store,"Description"))+"</textarea></label>"+
            ...
            "<button class='btn violet' type='submit'>GUARDAR TIENDA ONLINE</button></form></section>"+
            "<section class='card'><h3>Identidad y sincronización</h3><div class='identity-grid'><div><small>Cuenta</small><b>"+E(u.Email)+"</b></div><div><small>Store ID</small><b>"+E(u.StoreId)+"</b></div><div><small>Regla</small><b>Una cuenta · una tienda · mismo catálogo</b></div></div></section>";
        }
        ...
        {
            public string StoreId = ""; public string Name = ""; public string Category = ""; public string City = ""; public string Province = ""; public string PublicUrl = ""; public string Logo = ""; public bool Delivery; public bool Pickup; public double Latitude; public double Longitude; public double Distance; public bool Active; public bool Featured; public bool Listed;
            public CentralStore() { }
            public CentralStore(XElement e) { StoreId=S(e,"StoreId"); Name=S(e,"Name"); Category=S(e,"Category"); City=S(e,"City"); Province=S(e,"Province"); PublicUrl=S(e,"PublicUrl"); Logo=S(e,"Logo"); Delivery=S(e,"Delivery")=="1"; Pickup=S(e,"Pickup")=="1"; Active=S(e,"Active")=="1"; Featured=S(e,"Featured")=="1"; Listed=S(e,"Listed")!="0"; double.TryParse(S(e,"Latitude"), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out Latitude); double.TryParse(S(e,"Longitude"), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out Longitude); }
        }
