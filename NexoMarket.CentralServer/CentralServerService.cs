...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private void CentralSeller(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append("<script>(function(){var last='',lastPending=-1,audioCtx=null;function beep(){try{audioCtx=audioCtx||new(window.AudioContext||window.webkitAudioContext)();var o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type='sine';o.frequency.value=880;g.gain.value=.08;o.connect(g);g.connect(audioCtx.destination);o.start();setTimeout(function(){o.frequency.value=1175;},120);setTimeout(function(){o.stop();},240);}catch(e){}}function showNewOrderAlert(){var el=document.getElementById('newOrderAlert');if(!el)return;el.className='new-order-alert show';clearTimeout(window.__nexoOrderAlertTimer);clearInterval(window.__nexoOrderBeepTimer);var n=0;beep();n=1;window.__nexoOrderBeepTimer=setInterval(function(){if(n>=10){clearInterval(window.__nexoOrderBeepTimer);return;}beep();n++;},1000);window.__nexoOrderAlertTimer=setTimeout(function(){el.className='new-order-alert';clearInterval(window.__nexoOrderBeepTimer);},10000);}function live(){var x=new XMLHttpRequest();x.open('GET','/api/seller/live',true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText),v=d.updatedAt||'',pending=parseInt(d.pendingOrders||0,10);if(lastPending>=0&&pending>lastPending){showNewOrderAlert();}lastPending=pending;var editing=document.querySelector('form.product-form input:focus,form.product-form textarea:focus,form.edit-form input:focus,form.edit-form textarea:focus,form.settings-form input:focus,form.settings-form textarea:focus');var hasDraft=document.querySelector('form.product-form input:not([type=hidden])[value]:not([value=\"\"])')||document.querySelector('form.product-form textarea:not(:placeholder-shown)');if(last&&v!==last&&!editing&&!document.querySelector('details[open]')&&!hasDraft&&!document.querySelector('.order-cards'))location.reload();last=v;}catch(e){}}};x.send();}setTimeout(live,900);setInterval(live,2500);document.addEventListener('click',function(){if(audioCtx&&audioCtx.state==='suspended')audioCtx.resume();},{once:true});})();</script>");
