...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
            ...
            using (client)
            {
                try
                {
                    ...
                    using (NetworkStream stream = client.GetStream())
                    {
                        ...
                        if (path == "/assets/nexo-order-alert.wav" && method == "GET") { ServeOrderAlertAudio(stream); return; }
                        ...
                        if (path == "/api/admin/store/store-plus" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", AdminSetStorePlus(HeaderValue(request,"X-Nexo-Admin-Key"), Form(body))); return; }
                        if (path == "/api/admin/store/status" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", AdminSetStoreStatus(HeaderValue(request,"X-Nexo-Admin-Key"), Form(body))); return; }
                        if (path == "/api/admin/store/reset-status" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", AdminResetStoreStatus(HeaderValue(request,"X-Nexo-Admin-Key"), Form(body))); return; }
                        if (path == "/api/admin/recognitions/reset-all" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", AdminResetAllRecognitions(HeaderValue(request,"X-Nexo-Admin-Key"), Form(body))); return; }
                        ...
                        if (path == "/api/admin/store/commission-due-date" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", AdminSetCommissionDueDate(HeaderValue(request,"X-Nexo-Admin-Key"), Form(body))); return; }
                        if (path == "/api/admin/store/details" && method == "GET") { Write(stream, 200, "application/json; charset=utf-8", AdminStoreDetails(HeaderValue(request,"X-Nexo-Admin-Key"), QueryValue(query,"storeId"))); return; } if (path == "/api/admin/store/media" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", AdminStoreMediaUpload(HeaderValue(request,"X-Nexo-Admin-Key"), Form(body))); return; }
                        ...
                        if (path == "/api/orders/create" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", CreateOrder(Form(body), HeaderCookie(request, "NexoCentralSession"))); return; }
                        ...
                        if (path == "/api/orders/confirm" && method == "POST") { Write(stream, 200, "text/plain", ConfirmOrder(Form(body), HeaderCookie(request, "NexoCentralSession"))); return; }
                        ...
                        if (path.StartsWith("/store/", StringComparison.OrdinalIgnoreCase) && method == "GET") { string slug = path.Substring(7).Trim('/'); Write(stream, 200, "text/html; charset=utf-8", Storefront(slug, HeaderCookie(request, "NexoCentralSession"), query)); return; }
        ...
        // RECONOCIMIENTOS 5.23.3
        // Se corta definitivamente la herencia de los reconocimientos anteriores.
        // El primer arranque de esta versión deja TODAS las tiendas sin reconocimiento y
        // guarda una marca en el documento central para que el reseteo no vuelva a ejecutarse.
        private void NormalizeStorePlus()
        {
            lock(_sync)
            {
                if(_doc.Root == null) return;
                const string resetVersion = "5.23.3";
                if(string.Equals((string)_doc.Root.Attribute("RecognitionResetVersion"), resetVersion, StringComparison.Ordinal)) return;
                XElement stores = _doc.Root.Element("Stores");
                if(stores == null) return;
                foreach(XElement e in stores.Elements("Store"))
                {
                    e.SetElementValue("Featured", "0");
                    e.SetElementValue("StorePlus", "0");
                    if(e.Element("FeaturedPlus") != null) e.Element("FeaturedPlus").Remove();
                    if(e.Element("FeaturedTier") != null) e.Element("FeaturedTier").Remove();
                    if(e.Element("Plus") != null) e.Element("Plus").Remove();
                    if(e.Element("Recognition") != null) e.Element("Recognition").Remove();
                }
                _doc.Root.SetAttributeValue("RecognitionResetVersion", resetVersion);
                Save();
            }
        }

        private static bool IsStorePlus(XElement e) { return e != null && S(e,"StorePlus") == "1"; }
        private static bool IsFeaturedCommon(XElement e) { return e != null && S(e,"Featured") == "1"; }
        ...
        private string Register(Dictionary<string,string> f)
        {
            ...
            lock (_sync)
            {
                ...
                // DESTACADA común y TIENDA PLUS son controles exclusivos del Super Admin.
                // La publicación/sincronización del vendedor nunca los sobrescribe.
                string adminFeatured = old == null ? (Get(f,"featured") == "1" ? "1" : "0") : (S(old,"Featured") == "1" ? "1" : "0");
                string adminStorePlus = old == null ? (Get(f,"storePlus") == "1" ? "1" : "0") : (S(old,"StorePlus") == "1" ? "1" : "0");
                XElement e = new XElement("Store", new XAttribute("UpdatedAt", string.IsNullOrWhiteSpace(Get(f,"updatedAt")) ? DateTime.UtcNow.ToString("o") : Get(f, "updatedAt")),
                    ...
                    new XElement("Slug", Get(f, "slug")), new XElement("PublicUrl", Get(f, "publicUrl")), new XElement("Featured", adminFeatured), new XElement("StorePlus", adminStorePlus), new XElement("Listed", "1"), new XElement("Active", active),
        ...
        private string ClaimStore(Dictionary<string,string> f)
        {
            ...
            lock (_sync)
            {
                ...
                XElement store = new XElement("Store", new XAttribute("UpdatedAt", DateTime.UtcNow.ToString("o")),
                    ...
                    new XElement("Featured", "0"), new XElement("StorePlus", "0"), new XElement("Listed", "1"), new XElement("Active", "1"), new XElement("Delivery", Get(f,"delivery") == "0" ? "0" : "1"),
        ...
        private void ServeOrderAlertAudio(NetworkStream stream)
        {
            try
            {
                string path = Path.Combine(AppContext.BaseDirectory, "Assets", "NexoMarket_Alerta_Pedido_6s.wav");
                if (!File.Exists(path)) { Write(stream, 404, "audio/wav", new byte[0]); return; }
                Write(stream, 200, "audio/wav", File.ReadAllBytes(path));
            }
            catch { Write(stream, 500, "audio/wav", new byte[0]); }
        }
        ...
        private string ResetPassword(Dictionary<string,string> f)
        {
            ...
            byte[] hash; hash = Rfc2898DeriveBytes.Pbkdf2(password, salt, 50000, HashAlgorithmName.SHA1, 32);
        ...
        private string CreateOrder(Dictionary<string,string> f, string sessionCookie)
        {
            CentralUser sb=SessionUser(sessionCookie);
            if(sb!=null && sb.Role=="buyer"){f["customerId"]=sb.Id;f["customerEmail"]=sb.Email;if(string.IsNullOrWhiteSpace(Get(f,"customerName")))f["customerName"]=sb.Name;if(string.IsNullOrWhiteSpace(Get(f,"phone")))f["phone"]=sb.Phone;}
        ...
        private string GetOrderStatus(string storeId, string id)
        {
            ...
            lock (_sync)
            {
                ...
                XElement e=d.Root.Element("Orders").Elements("Order").FirstOrDefault(x=>S(x,"StoreId")==storeId&&S(x,"CentralOrderId")==id);
                if(e==null)return "{\"error\":\"notfound\"}";
                StringBuilder r=new StringBuilder("{\"centralOrderId\":").Append(JsonString(S(e,"CentralOrderId"))).Append(",\"status\":").Append(JsonString(S(e,"Status"))).Append(",\"total\":").Append(JsonString(S(e,"Total"))).Append(",\"buyerConfirmed\":").Append(JsonString(S(e,"BuyerConfirmedAt"))).Append(",\"approvedAt\":").Append(JsonString(S(e,"ApprovedAt"))).Append(",\"updatedAt\":").Append(JsonString(S(e,"UpdatedAt"))).Append(",\"items\":[");bool firstItem=true;string rawItems=S(e,"ItemsJson");foreach(Match m in Regex.Matches(rawItems??"", @"""id""\s*:\s*""([^""]+)""[^}]*?""name""\s*:\s*""([^""]*)""(?:[^}]*?""note""\s*:\s*""([^""]*)"")?[^}]*?""price""\s*:\s*([0-9.,-]+)[^}]*?""qty""\s*:\s*(\d+)", RegexOptions.IgnoreCase)){if(!firstItem)r.Append(',');firstItem=false;string pid=m.Groups[1].Value;string image="";XDocument cd=LoadFile(_catalogFile,"NexoMarketCatalog","Products");XElement p=cd.Root.Element("Products")==null?null:cd.Root.Element("Products").Elements("Product").FirstOrDefault(x=>S(x,"StoreId")==storeId&&S(x,"ProductId")==pid);if(p!=null)image=ProductImageUrl(p);r.Append("{\"id\":").Append(JsonString(pid)).Append(",\"name\":").Append(JsonString(m.Groups[2].Value)).Append(",\"note\":").Append(JsonString(m.Groups[3].Value)).Append(",\"price\":").Append(JsonString(m.Groups[4].Value)).Append(",\"qty\":").Append(m.Groups[5].Value).Append(",\"image\":").Append(JsonString(image)).Append('}');}r.Append("]}");return r.ToString();
            }
        }
        ...
        private string ConfirmOrder(Dictionary<string,string> f, string sessionCookie)
        {
            CentralUser sb=SessionUser(sessionCookie);string storeId=Get(f,"storeId"),id=Get(f,"centralOrderId"),email=Get(f,"email");
            if(sb!=null&&sb.Role=="buyer")email=sb.Email;if(string.IsNullOrWhiteSpace(storeId)||string.IsNullOrWhiteSpace(id))return "ERROR|missing";
            lock(_sync){XDocument d=LoadFile(_ordersFile,"NexoMarketOrders","Orders");XElement e=d.Root.Element("Orders").Elements("Order").FirstOrDefault(x=>S(x,"StoreId")==storeId&&S(x,"CentralOrderId")==id);if(e==null)return "ERROR|notfound";string owner=S(e,"CustomerEmail");if(sb!=null&&sb.Role=="buyer"&&!string.Equals(owner,sb.Email,StringComparison.OrdinalIgnoreCase))return "ERROR|owner";if(!string.Equals(owner,email,StringComparison.OrdinalIgnoreCase))return "ERROR|email";if(S(e,"Status")!="Entregado")return "ERROR|not_delivered";e.SetElementValue("BuyerConfirmedAt",DateTime.UtcNow.ToString("o"));e.SetElementValue("UpdatedAt",DateTime.UtcNow.ToString("o"));SaveDoc(_ordersFile,d);XElement changed=new XElement(e);Audit("order_buyer_received",storeId,owner,id,"confirmed");QueueOrderEmails(changed,"pedido recibido por el comprador");}return "OK|confirmed";
        }
        ...
        private string AdminCreateStore(string key,Dictionary<string,string> f)
        {
            ...
            if(email.Length>0)
            {
                ...
                byte[] salt=new byte[16];using(var rng=RandomNumberGenerator.Create())rng.GetBytes(salt);string salt64=Convert.ToBase64String(salt);byte[] hash;hash=Rfc2898DeriveBytes.Pbkdf2(password,salt,50000,HashAlgorithmName.SHA1,32);
        ...
        {
            string denied=AdminDenied(key); if(denied!=null)return denied; string id=NormalizeStoreId(Get(f,"storeId")); bool active=Get(f,"active")!="0";
            CentralUser seller=FindSellerByStore(id);
            lock(_sync){XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));if(e==null)return "ERROR|store_not_found";e.SetElementValue("Active",active?"1":"0");if(active)e.SetElementValue("Listed","1");e.SetElementValue("LastActivityAt",DateTime.UtcNow.ToString("o"));e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));Save();}
            if(seller!=null){string ar=AdminSetAccountActive(key,new Dictionary<string,string>(StringComparer.OrdinalIgnoreCase){{"email",seller.Email},{"active",active?"1":"0"}});if(!ar.StartsWith("OK|",StringComparison.OrdinalIgnoreCase))return "ERROR|store_updated_account_failed|"+Escape(ar);}
            Audit(active?"admin_store_enabled":"admin_store_disabled",id,seller==null?"":seller.Email,"","");
            return "OK|"+(active?"active":"inactive");
        }
        private string AdminSetStoreFeatured(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync){XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));if(e==null)return "ERROR|store_not_found";e.SetElementValue("Featured",featured?"1":"0"); if(featured)e.SetElementValue("StorePlus","0"); e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));Save();}
            return "OK|"+(featured?"featured":"unfeatured");
        }
        // 5.23: estatus promocional centralizado, independiente de la sincronizacion del vendedor.
        private string AdminSetStoreStatus(string key,Dictionary<string,string> f)
        {
            string denied=AdminDenied(key); if(denied!=null)return denied;
            string id=NormalizeStoreId(Get(f,"storeId")); string status=(Get(f,"status")??"").Trim().ToUpperInvariant();
            if(status!="PLUS" && status!="FEATURED" && status!="NONE") return "ERROR|invalid_store_status";
            lock(_sync)
            {
                ...
                e.SetElementValue("StorePlus",status=="PLUS"?"1":"0"); e.SetElementValue("Featured",status=="FEATURED"?"1":"0");
                if(e.Element("FeaturedPlus")!=null)e.Element("FeaturedPlus").Remove(); if(e.Element("FeaturedTier")!=null)e.Element("FeaturedTier").Remove();
                e.SetElementValue("Listed","1"); e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o")); Save();
            }
            return "OK|status="+status;
        }
        private string AdminResetStoreStatus(string key,Dictionary<string,string> f)
        {
            string denied=AdminDenied(key); if(denied!=null)return denied; string id=NormalizeStoreId(Get(f,"storeId"));
            lock(_sync)
            {
                XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));
                if(e==null)return "ERROR|store_not_found"; e.SetElementValue("StorePlus","0"); e.SetElementValue("Featured","0");
                if(e.Element("FeaturedPlus")!=null)e.Element("FeaturedPlus").Remove(); if(e.Element("FeaturedTier")!=null)e.Element("FeaturedTier").Remove();
                e.SetElementValue("Listed","1"); e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o")); Save();
            }
        ...
        private string AdminResetAllRecognitions(string key,Dictionary<string,string> f)
        {
            string denied=AdminDenied(key); if(denied!=null)return denied;
            lock(_sync)
            {
                XElement stores=_doc.Root==null?null:_doc.Root.Element("Stores");
                if(stores==null)return "ERROR|stores_missing";
                int changed=0;
                foreach(XElement e in stores.Elements("Store"))
                {
                    if(S(e,"Featured")!="0" || S(e,"StorePlus")!="0" || e.Element("FeaturedPlus")!=null || e.Element("FeaturedTier")!=null || e.Element("Plus")!=null || e.Element("Recognition")!=null) changed++;
                    e.SetElementValue("Featured","0"); e.SetElementValue("StorePlus","0");
                    if(e.Element("FeaturedPlus")!=null)e.Element("FeaturedPlus").Remove();
                    if(e.Element("FeaturedTier")!=null)e.Element("FeaturedTier").Remove();
                    if(e.Element("Plus")!=null)e.Element("Plus").Remove();
                    if(e.Element("Recognition")!=null)e.Element("Recognition").Remove();
                    e.SetElementValue("Listed","1"); e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));
                }
                _doc.Root.SetAttributeValue("RecognitionResetVersion","5.23.3");
                Save();
                return "OK|recognitions_reset|changed="+changed.ToString(System.Globalization.CultureInfo.InvariantCulture);
            }
        }

        private string AdminSetStorePlus(string key,Dictionary<string,string> f)
        {
            string denied=AdminDenied(key); if(denied!=null)return denied;
            string id=NormalizeStoreId(Get(f,"storeId")); bool plus=Get(f,"storePlus")!="0";
            lock(_sync)
            {
                XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));
                if(e==null)return "ERROR|store_not_found";
                e.SetElementValue("StorePlus",plus?"1":"0"); if(plus)e.SetElementValue("Featured","0");
                e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));
                Save();
            }
            return "OK|"+(plus?"store_plus":"unstore_plus");
        }
        private string AdminSetStoreListed(string key,Dictionary<string,string> f)
        {
            string denied=AdminDenied(key); if(denied!=null)return denied;
            string id=NormalizeStoreId(Get(f,"storeId"));
            lock(_sync){XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));if(e==null)return "ERROR|store_not_found";e.SetElementValue("Listed","1");e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));Save();}
            return "OK|listed_always";
        }
        ...
        private string StoreLines(string query)
        {
            ...
            lock (_sync)
            {
                foreach (XElement e in _doc.Root.Element("Stores").Elements("Store"))
                {
                    ...
                    // Desde 5.22.2 la publicación del directorio es permanente: las tiendas no pueden ocultarse.
            ...
            if (hasCoords) list = list.OrderByDescending(x => IsStorePlus(x.Element)).ThenByDescending(x => IsFeaturedCommon(x.Element)).ThenByDescending(x => S(x.Element, "Active") != "0").ThenBy(x => x.DistanceKm).ThenBy(x => S(x.Element, "Name")).ToList();
            else list = list.OrderByDescending(x => IsStorePlus(x.Element)).ThenByDescending(x => IsFeaturedCommon(x.Element)).ThenByDescending(x => S(x.Element, "Active") != "0").ThenBy(x => S(x.Element, "City")).ThenBy(x => S(x.Element, "Name")).ToList();
            StringBuilder b = new StringBuilder();
        ...
        private string Storefront(string slug, string cookie, string query)
        {
            ...
            b.Append("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta http-equiv='Cache-Control' content='no-store'><title>").Append(E(S(store,"Name"))).Append(" · NexoMarket</title><style>body{font-family:'Segoe UI',Arial;background:#000;color:#fff;margin:0}.wrap{max-width:1240px;margin:auto;padding:20px 20px 100px}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px}.brand{font-weight:900;font-size:24px}.brand-home{color:#fff;text-decoration:none;display:inline-flex;align-items:center;gap:0}.brand-home:hover{filter:brightness(1.12);text-shadow:0 0 14px rgba(57,255,102,.28)}.brand span{color:#39ff66}.nav a{color:#dbe6f0;text-decoration:none;margin-left:14px}.nav a{white-space:nowrap;display:inline-flex;align-items:center;justify-content:center}.nav{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.seller-account-link{white-space:nowrap}.hero{display:flex;gap:18px;align-items:center;background:linear-gradient(135deg,#101923,#15102a);border:1px solid #2f3e55;border-radius:24px;padding:24px;box-shadow:0 16px 40px rgba(84,42,150,.16)}.store-avatar{width:82px;height:82px;border-radius:20px;background:#0b1118;border:1px solid #9b5cff;color:#c59cff;display:flex;align-items:center;justify-content:center;font-size:34px;font-weight:900}.store-avatar-img{width:82px;height:82px;border-radius:20px;object-fit:cover;border:1px solid #9b5cff}.store-cover-photo{width:100%;height:220px;border-radius:18px;object-fit:cover;border:1px solid rgba(57,255,102,.55);display:block;box-shadow:0 0 26px rgba(57,255,102,.10);filter:saturate(.9) brightness(.72)}.store-cover-shell{position:relative;margin-bottom:16px;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,.10);background:#080c12;box-shadow:0 18px 45px rgba(0,0,0,.32)}.store-cover-shell:after{content:'';position:absolute;inset:0;background:linear-gradient(180deg,rgba(3,7,12,.05),rgba(3,7,12,.78)),linear-gradient(90deg,rgba(57,255,102,.04),rgba(167,103,255,.10),transparent)}.store-cover-caption{position:absolute;left:18px;bottom:14px;z-index:2;color:#fff;font-size:11px;letter-spacing:1px;font-weight:900;text-shadow:0 0 10px rgba(255,255,255,.35)}.public-tier{display:inline-flex;padding:7px 12px;border-radius:999px;font-size:10px;font-weight:950;letter-spacing:.8px;margin-bottom:7px;position:relative;isolation:isolate;text-shadow:0 0 9px rgba(255,255,255,.12)}.public-tier.store-plus{color:#f2d8ff;border:1px solid #c98cff;background:linear-gradient(90deg,rgba(118,50,180,.55),rgba(48,28,72,.86));box-shadow:0 0 18px rgba(191,126,255,.62),0 0 28px rgba(255,205,74,.16);text-shadow:0 0 10px rgba(226,190,255,.55);position:relative;isolation:isolate}.public-tier.store-plus:after{content:'';position:absolute;inset:-9px -14px;border-radius:999px;background:radial-gradient(ellipse at center,rgba(201,140,255,.42),transparent 70%);filter:blur(9px);opacity:.55;z-index:-1;pointer-events:none}.public-tier.featured{color:#7cff9a;border:1px solid #39ff66;box-shadow:0 0 14px rgba(57,255,102,.18);background:rgba(7,30,18,.7)}.public-tier.featured:after{content:'';position:absolute;inset:-7px -12px;border-radius:999px;background:radial-gradient(ellipse at center,rgba(57,255,102,.22),transparent 68%);filter:blur(7px);opacity:.72;z-index:-1;pointer-events:none}.hero h1{margin:0 0 6px;font-size:31px}.muted{color:#91a3b5}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;margin-top:18px}.card{background:#0f1721;border:1px solid #28394e;border-radius:18px;padding:16px;min-height:220px;position:relative}.product-img{width:100%;height:170px;object-fit:cover;border-radius:13px;background:#0a1119;margin-bottom:12px}.price{font-size:21px;font-weight:900;margin-top:9px}.sale{color:#39ff66}.btn{background:#39ff66;color:#061009;border:0;border-radius:10px;padding:10px 14px;font-weight:900;cursor:pointer}.btn.violet{background:#fff;color:#000;box-shadow:0 0 18px rgba(255,255,255,.22)}.empty{padding:22px;color:#8b99a9;border:1px dashed #3c4b60;border-radius:16px}.promos{margin-top:25px}.promo-card{border-color:#7b4bd1}.cart-fab{position:fixed;right:22px;bottom:22px;z-index:30;background:#9b5cff;color:#fff;border:0;border-radius:999px;padding:14px 20px;font-weight:900;box-shadow:0 10px 30px rgba(0,0,0,.35);cursor:pointer}.cart-panel{position:fixed;right:20px;bottom:78px;width:min(430px,calc(100vw - 40px));max-height:78vh;overflow:auto;z-index:29;background:#0c141e;border:1px solid #7650bd;border-radius:20px;padding:18px;box-shadow:0 18px 50px rgba(0,0,0,.5);display:none}.cart-panel.open{display:block}.cart input,.cart select{background:#0d141d;color:#fff;border:1px solid #2a3b51;border-radius:9px;padding:10px;margin:4px;width:calc(100% - 18px)}.item{display:flex;justify-content:space-between;gap:10px;border-bottom:1px solid #223143;padding:8px 0}.item>div{min-width:0;flex:1}.item-note{display:block;width:100%;box-sizing:border-box;background:#090d13;color:#e8edf2;border:1px solid rgba(167,103,255,.28);border-radius:8px;padding:7px 8px;margin-top:7px;font-size:11px}.item-order-note{color:#ffd84d!important}.live{color:#8dffac;font-size:11px;font-weight:900}.section{margin-top:24px}@media(max-width:950px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.wrap{padding:10px 8px 100px}.top,.hero{align-items:flex-start;flex-direction:column}.grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.card{padding:5px;min-height:0;border-radius:8px}.product-img{height:118px;border-radius:9px;margin-bottom:6px}.card h3{font-size:12px;line-height:1.2;margin:5px 0}.card .muted{font-size:9px;line-height:1.2;max-height:34px;overflow:hidden}.price{font-size:14px;margin-top:5px}.card .btn{font-size:10px;padding:7px 4px;border-radius:7px;width:100%}.nav a{margin-left:0;margin-right:12px}}body{position:relative;overflow-x:hidden}body:before{content:'';position:fixed;inset:-15%;pointer-events:none;z-index:0;background:radial-gradient(ellipse at 8% 18%,transparent 0 25%,rgba(57,255,102,.045) 25.1%,transparent 25.45%),radial-gradient(ellipse at 92% 72%,transparent 0 22%,rgba(167,103,255,.055) 22.1%,transparent 22.45%),radial-gradient(circle at 50% 5%,rgba(255,255,255,.03),transparent 26%);transform:rotate(-7deg)}body:after{content:'';position:fixed;width:80vw;height:45vh;left:-28vw;bottom:8vh;border:1px solid rgba(255,255,255,.07);border-right-color:rgba(57,255,102,.10);border-radius:50%;transform:rotate(-14deg);pointer-events:none;z-index:0;box-shadow:0 0 70px rgba(57,255,102,.025)}.wrap{position:relative;z-index:1}.scroll-home{position:fixed;left:14px;top:12px;z-index:9999;display:none;border:1px solid rgba(255,255,255,.14);background:rgba(5,7,10,.86);backdrop-filter:blur(10px);color:#fff;border-radius:999px;padding:6px 10px;font-size:10px;font-weight:950;letter-spacing:.6px;box-shadow:0 0 16px rgba(57,255,102,.08);cursor:pointer}.scroll-home span{color:#39ff66}.scroll-home.show{display:block}.scroll-home:hover{border-color:#39ff66;box-shadow:0 0 18px rgba(57,255,102,.18)}.card{background:linear-gradient(145deg,rgba(7,9,12,.96),rgba(12,15,20,.92));border-color:rgba(255,255,255,.09);box-shadow:0 12px 35px rgba(0,0,0,.30),inset 0 1px 0 rgba(255,255,255,.025);transition:transform .2s ease,border-color .2s ease,box-shadow .2s ease}.store-card-cover{position:absolute;inset:0;z-index:0;overflow:hidden;border-radius:18px}.store-card-cover img{width:100%;height:100%;object-fit:cover;display:block;filter:saturate(.82) brightness(.62)}.store-card-cover:after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,rgba(3,7,12,.94) 0%,rgba(5,10,17,.84) 43%,rgba(5,10,17,.62) 100%),linear-gradient(180deg,rgba(3,7,12,.16),rgba(3,7,12,.88))}.store-card-content{position:relative;z-index:1}.notify-setup{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:0 0 14px;padding:10px 13px;border:1px solid rgba(255,255,255,.12);border-radius:12px;background:rgba(8,12,18,.78);color:#bfcbd6;font-size:11px;box-shadow:0 0 18px rgba(167,103,255,.06)}.notify-setup b{color:#fff}.notify-setup .btn{padding:7px 10px;font-size:10px;white-space:nowrap}.notify-setup.ok{border-color:rgba(57,255,102,.28)}.notify-setup.warn{border-color:rgba(255,216,77,.30)}.sound-unlock{position:fixed;right:14px;top:12px;z-index:99998;display:none;border:1px solid rgba(57,255,102,.38);background:rgba(4,10,8,.92);color:#dfffe8;border-radius:999px;padding:7px 11px;font-size:10px;font-weight:900;box-shadow:0 0 18px rgba(57,255,102,.12);cursor:pointer}.sound-unlock.show{display:block}.card:hover{transform:translateY(-4px);border-color:rgba(255,255,255,.35);box-shadow:0 0 22px rgba(255,255,255,.08),0 14px 35px rgba(0,0,0,.4)}.gold-link,.gold-link:hover{color:#d4af37!important;text-decoration:none!important}.btn:hover{background:#fff!important;color:#000!important;box-shadow:0 0 14px rgba(255,255,255,.55),0 0 28px rgba(255,255,255,.12)!important}.price{text-shadow:0 0 10px rgba(255,255,255,.10)}body{position:relative;overflow-x:hidden}body:before{content:'';position:fixed;inset:-18%;pointer-events:none;z-index:0;background:radial-gradient(ellipse at 7% 20%,transparent 0 24%,rgba(57,255,102,.055) 24.1%,transparent 24.5%),radial-gradient(ellipse at 93% 70%,transparent 0 23%,rgba(167,103,255,.06) 23.1%,transparent 23.5%),radial-gradient(circle at 50% 10%,rgba(255,255,255,.035),transparent 26%);transform:rotate(-7deg)}body:after{content:'';position:fixed;width:85vw;height:42vh;right:-30vw;bottom:8vh;border:1px solid rgba(255,255,255,.06);border-left-color:rgba(167,103,255,.10);border-radius:50%;transform:rotate(-16deg);pointer-events:none;z-index:0;box-shadow:0 0 80px rgba(167,103,255,.025)}.wrap{position:relative;z-index:1}.scroll-home{position:fixed;left:14px;top:12px;z-index:9999;display:none;border:1px solid rgba(255,255,255,.14);background:rgba(5,7,10,.86);backdrop-filter:blur(10px);color:#fff;border-radius:999px;padding:6px 10px;font-size:10px;font-weight:950;letter-spacing:.6px;box-shadow:0 0 16px rgba(57,255,102,.08);cursor:pointer}.scroll-home span{color:#39ff66}.scroll-home.show{display:block}.scroll-home:hover{border-color:#39ff66;box-shadow:0 0 18px rgba(57,255,102,.18)}.hero,.panel,.card{background:linear-gradient(145deg,rgba(7,9,12,.95),rgba(12,15,20,.91));border-color:rgba(255,255,255,.09);box-shadow:0 14px 38px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.025)}.card:hover{border-color:rgba(255,255,255,.32);box-shadow:0 0 22px rgba(255,255,255,.07),0 14px 32px rgba(0,0,0,.36);transform:translateY(-4px)}.btn:hover{background:#fff!important;color:#000!important;box-shadow:0 0 14px rgba(255,255,255,.55),0 0 30px rgba(255,255,255,.12)!important}.closed-store-banner{margin:0 0 16px;padding:13px 15px;border:1px solid rgba(255,59,95,.45);border-radius:13px;background:rgba(255,59,95,.06);color:#ff8ca0;font-weight:800;box-shadow:0 0 20px rgba(255,59,95,.05)}.top,.section-head{position:relative}.top:after{content:'';position:absolute;left:0;right:0;bottom:4px;height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.16),rgba(57,255,102,.18),rgba(167,103,255,.18),transparent)}.proof-actions{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}.proof-actions .btn{flex:1;min-width:140px}.proof-preview img{max-width:100%;max-height:220px;border-radius:12px;margin-top:8px;object-fit:contain}.approved-order-alert{position:fixed;left:50%;top:18px;transform:translateX(-50%);z-index:99999;display:none;min-width:min(650px,calc(100vw - 28px));padding:18px 24px;border:2px solid #39ff66;border-radius:16px;background:linear-gradient(135deg,#073a18,#0b7b31 55%,#063215);color:#fff;text-align:center;font-weight:900;box-shadow:0 0 24px rgba(57,255,102,.48),0 16px 45px rgba(0,0,0,.55)}.approved-order-alert.show{display:block;animation:approvedPulse .55s ease-in-out 2}.approved-order-alert .ao-title{font-size:20px;letter-spacing:1px}.approved-order-alert .ao-sub{font-size:12px;margin-top:5px;color:#e8fff0}@keyframes approvedPulse{0%,100%{transform:translateX(-50%) scale(1)}50%{transform:translateX(-50%) scale(1.025)}}.review-card{margin-top:18px;background:linear-gradient(145deg,#17120a,#0d0f12);border:1px solid rgba(212,175,55,.55);border-radius:20px;padding:18px;box-shadow:0 0 24px rgba(212,175,55,.08),inset 0 1px 0 rgba(255,255,255,.04)}.review-summary{display:flex;justify-content:space-between;gap:15px;align-items:center}.review-label{font-size:11px;letter-spacing:1.6px;color:#d7bd62;font-weight:900}.review-score{font-size:26px;font-weight:900;margin-top:5px}.gold-stars{color:#ffd84d;letter-spacing:2px;text-shadow:0 0 10px rgba(255,216,77,.22)}.review-count{color:#9e9580;font-size:12px;margin-top:4px}.review-form{margin-top:16px;padding-top:16px;border-top:1px solid rgba(212,175,55,.18)}.star-picker{display:flex;gap:4px;margin:7px 0 12px}.star-picker button{background:none;border:0;color:#6b6250;font-size:32px;cursor:pointer;padding:0 3px}.star-picker button.selected{color:#ffd84d;text-shadow:0 0 12px rgba(255,216,77,.35)}.emoji-row{display:flex;gap:6px;margin-bottom:7px}.emoji-row button{border:1px solid #3b3428;background:#15120e;border-radius:9px;padding:7px 9px;font-size:18px;cursor:pointer}.review-form input,.review-form textarea{width:100%;box-sizing:border-box;background:#08090b;color:#fff;border:1px solid #3b3428;border-radius:10px;padding:10px;margin:5px 0}.review-form textarea{min-height:90px;resize:vertical}.review-message{font-size:12px;margin-top:8px}.review-list{display:grid;gap:8px;margin-top:15px}.review-item{background:rgba(255,255,255,.025);border:1px solid rgba(212,175,55,.16);border-radius:12px;padding:11px}.review-item .stars{color:#ffd84d;letter-spacing:1px}.review-item small{color:#827c6e}.review-open{white-space:nowrap}@media(max-width:650px){.review-summary{align-items:flex-start;flex-direction:column}.review-open{width:100%}.review-score{font-size:23px}}h1,h2,h3{color:#dce2ea;text-shadow:0 0 8px rgba(255,255,255,.12),0 0 16px rgba(167,103,255,.08)}.nav a{color:#bfc8d2;text-shadow:0 0 7px rgba(255,255,255,.10)}.muted{color:#aab5c0}.hero{box-shadow:0 18px 55px rgba(0,0,0,.36),0 0 28px rgba(167,103,255,.06),inset 0 1px 0 rgba(255,255,255,.06)}@media(max-width:650px){.store-cover-photo{height:170px}.store-cover-shell{border-radius:16px}.hero h1{font-size:25px}}.grid .card{grid-template-columns:58% 42%;padding:0;min-height:300px}.grid .card .store-card-content{grid-column:1;grid-row:1;padding:22px;display:flex;flex-direction:column;justify-content:center;z-index:2;background:linear-gradient(135deg,rgba(7,10,15,.70),rgba(12,9,20,.56));backdrop-filter:blur(5px)}.grid .card .store-card-cover{grid-column:2;grid-row:1;position:relative;inset:auto;min-height:300px;height:100%;border-radius:0;z-index:1;overflow:hidden}.grid .card .store-card-cover img{height:100%;animation:storeCoverFade .9s ease-out both;filter:saturate(.88) brightness(.74)}.grid .card .store-card-cover:before{content:'';position:absolute;inset:0;background:radial-gradient(circle at 70% 50%,rgba(255,255,255,.18),transparent 48%);box-shadow:inset 0 0 55px rgba(255,255,255,.10);z-index:2}.grid .card .logo-wrap{width:118px;height:118px;margin:0 0 13px}.grid .card .logo-wrap .logo,.grid .card .logo-wrap .logo img{width:118px;height:118px;border-radius:26px}.grid .card .name{font-size:25px}.grid .card .rating-public{font-size:14px;color:#ffe39a;text-shadow:0 0 12px rgba(255,216,77,.25)}@keyframes storeCoverFade{from{opacity:0;transform:scale(1.025)}to{opacity:1;transform:scale(1)}}@media(max-width:650px){.grid .card{grid-template-columns:58% 42%;min-height:245px}.grid .card .store-card-cover{min-height:245px}.grid .card .store-card-content{padding:13px}.grid .card .logo-wrap,.grid .card .logo-wrap .logo,.grid .card .logo-wrap .logo img{width:86px;height:86px;border-radius:18px}.grid .card .name{font-size:18px}}.store-hero{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:20px;overflow:hidden;background:linear-gradient(135deg,rgba(15,20,27,.86),rgba(28,16,40,.74));backdrop-filter:blur(16px);box-shadow:0 22px 60px rgba(0,0,0,.36),0 0 34px rgba(212,175,55,.06),inset 0 1px 0 rgba(255,255,255,.06)}.hero-rating{min-width:190px;padding:16px 18px;border-radius:18px;border:1px solid rgba(212,175,55,.58);background:linear-gradient(145deg,rgba(70,49,9,.34),rgba(19,15,8,.34));box-shadow:0 0 25px rgba(212,175,55,.10);text-align:center}.hero-rating-label{font-size:10px;letter-spacing:2px;color:#d7bd62;font-weight:900}.hero-rating-score{font-size:40px;line-height:1;font-weight:950;color:#fff;text-shadow:0 0 16px rgba(255,216,77,.22)}.hero-rating-max{font-size:15px;color:#a79c86}.hero-rating-stars{font-size:20px;color:#ffd84d;letter-spacing:2px;margin-top:8px}.hero-rating-count{font-size:11px;color:#c7bda6;margin-top:5px}@media(max-width:700px){.store-hero{grid-template-columns:auto 1fr}.hero-rating{grid-column:1 / -1;text-align:left}.hero-rating-score{display:inline-block;font-size:31px}.hero-rating-stars{display:inline-block;margin-left:8px}.hero-rating-count{display:inline-block;margin-left:8px}}.grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}.grid .card{display:block;position:relative;min-height:320px;height:320px;padding:0;overflow:hidden;border-radius:22px;background:rgba(5,8,13,.78);border:1px solid rgba(255,255,255,.12);box-shadow:0 18px 45px rgba(0,0,0,.34),inset 0 1px 0 rgba(255,255,255,.05);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px)}.grid .card .store-card-cover{position:absolute;inset:0;width:100%;height:100%;min-height:0;margin:0;border-radius:0;z-index:0;overflow:hidden}.grid .card .store-card-cover img{width:100%;height:100%;object-fit:cover;display:block;filter:saturate(.88) brightness(.58);transform:scale(1.01);transition:transform .45s ease,filter .45s ease}.grid .card .store-card-cover:after{content:'';position:absolute;inset:0;background:linear-gradient(180deg,rgba(3,7,12,.10) 0%,rgba(3,7,12,.28) 35%,rgba(3,7,12,.86) 78%,rgba(3,7,12,.97) 100%),linear-gradient(90deg,rgba(57,255,102,.035),rgba(167,103,255,.05),transparent 72%)}.grid .card .store-card-cover.cover-fallback{display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at 30% 25%,rgba(57,255,102,.20),transparent 30%),radial-gradient(circle at 75% 65%,rgba(167,103,255,.24),transparent 34%),linear-gradient(145deg,#060a10,#17102b)}.grid .card .store-card-cover.cover-fallback:after{background:linear-gradient(180deg,rgba(3,7,12,.05),rgba(3,7,12,.75))}.grid .card .store-card-content{position:relative;z-index:2;display:flex;flex-direction:column;justify-content:flex-end;min-width:0;height:100%;padding:16px 16px 15px;background:linear-gradient(180deg,transparent 35%,rgba(4,7,12,.10) 44%,rgba(4,7,12,.62) 70%,rgba(4,7,12,.94) 100%)}.grid .card .logo-wrap{position:absolute;left:15px;top:15px;width:72px;height:72px;margin:0;z-index:4}.grid .card .logo-wrap .logo{width:72px;height:72px;margin:0;border-radius:18px;border:1px solid rgba(255,255,255,.28);background:rgba(7,11,17,.80);box-shadow:0 8px 25px rgba(0,0,0,.35),0 0 22px rgba(167,103,255,.16);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px)}.grid .card .logo-wrap .logo img{width:100%;height:100%;object-fit:cover;display:block;border-radius:17px}.grid .card .logo-star{right:-7px;top:-7px;width:30px;height:30px;font-size:15px}.grid .card .tier-badge{align-self:flex-start;margin:0 0 7px 0;padding:6px 9px;font-size:9px;z-index:3}.grid .card .name{font-size:21px;line-height:1.08;margin:0;max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 2px 12px rgba(0,0,0,.8),0 0 12px rgba(255,255,255,.10)}.grid .card .meta{font-size:11px;line-height:1.35;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 2px 9px rgba(0,0,0,.85)}.grid .card .open{font-size:11px;margin-top:7px;text-shadow:0 2px 8px rgba(0,0,0,.85)}.grid .card .distance{font-size:10px;margin-top:4px;text-shadow:0 2px 8px rgba(0,0,0,.85)}.grid .card .rating-public{font-size:11px;margin:5px 0 0;color:#ffe39a;text-shadow:0 0 10px rgba(255,216,77,.20)}.grid .card .promo-badges{margin-top:7px;gap:4px;max-height:24px;overflow:hidden}.grid .card .promo-badges span{font-size:8px;padding:3px 6px;background:rgba(167,103,255,.22);border-color:rgba(255,255,255,.16);backdrop-filter:blur(8px)}.grid .card:hover{transform:translateY(-5px);border-color:rgba(57,255,102,.42);box-shadow:0 0 28px rgba(57,255,102,.09),0 22px 50px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.08)}.grid .card:hover .store-card-cover img{transform:scale(1.06);filter:saturate(.98) brightness(.68)}.grid .card.featured{border-color:rgba(57,255,102,.62);box-shadow:0 0 24px rgba(57,255,102,.10),0 18px 45px rgba(0,0,0,.34)}.grid .card.plus{border-color:#39ff66;box-shadow:0 0 28px rgba(167,103,255,.24),0 0 40px rgba(57,255,102,.10),0 18px 48px rgba(0,0,0,.38)}.grid .card.plus .store-card-cover:after{background:linear-gradient(180deg,rgba(32,7,51,.08),rgba(8,5,18,.90) 78%,rgba(4,7,12,.98)),linear-gradient(90deg,rgba(57,255,102,.05),rgba(167,103,255,.12),transparent 72%)}.grid .card.plus:before{height:2px;background:linear-gradient(90deg,transparent,#39ff66,#b86cff,#39ff66,transparent);box-shadow:0 0 14px rgba(57,255,102,.55)}" +
".store-product-grid{grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:18px!important}" +
".store-product-card{display:flex!important;flex-direction:column!important;min-height:0!important;height:auto!important;padding:12px!important;overflow:hidden!important;text-align:center!important}" +
".store-product-image-wrap{position:relative;width:100%;aspect-ratio:9/16;overflow:hidden;border-radius:15px;background:#080c12;margin-bottom:11px;flex:none}" +
".store-product-image-wrap:after{content:'';position:absolute;left:0;right:0;bottom:0;height:34%;background:linear-gradient(180deg,transparent,rgba(0,0,0,.82));pointer-events:none}" +
".store-product-image-wrap .product-img{width:100%!important;height:100%!important;aspect-ratio:9/16;object-fit:cover!important;object-position:center;border-radius:15px;margin:0!important;display:block}" +
".store-product-card h3{font-size:17px;line-height:1.22;margin:3px 0 7px;white-space:normal;overflow:visible;text-overflow:clip;word-break:break-word}" +
".store-product-card .product-meta{font-size:10px;line-height:1.35;color:#9aabbc;margin-bottom:7px;min-height:0}" +
".store-product-card .product-description{font-size:11px;line-height:1.45;color:#d7e0e8;white-space:normal;overflow:visible;display:block;word-break:break-word;margin-bottom:9px}" +
".store-product-card .price{font-size:22px;line-height:1.15;margin:4px 0 5px;font-weight:950;white-space:normal}" +
".store-product-card .stock{font-size:11px;color:#9fb0bf;margin-bottom:10px}" +
".store-product-card .btn{width:100%;margin-top:auto}" +
".coupons-section{margin-top:28px}" +
".coupons-title{display:flex;align-items:center;gap:10px;margin-bottom:4px}" +
".coupons-title:before{content:'◆';color:#d4af37;font-size:13px}" +
".coupon-promo-card,.coupon-card{background:rgba(12,11,7,.62)!important;border:1px solid rgba(212,175,55,.58)!important;box-shadow:0 14px 35px rgba(0,0,0,.30),inset 0 1px 0 rgba(255,255,255,.04),0 0 24px rgba(212,175,55,.06)!important;backdrop-filter:blur(14px);text-align:center}" +
".coupon-promo-card .sale,.coupon-card .sale{color:#e3c45c!important}" +
".coupon-card{min-height:0!important;height:100%;padding:12px!important;display:flex;flex-direction:column;justify-content:space-between;aspect-ratio:1/1.08;overflow:hidden}.coupon-card h3{font-size:17px!important;letter-spacing:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.coupon-card .muted{font-size:10px;line-height:1.25;min-height:25px;max-height:38px;overflow:hidden}.coupon-card .btn{font-size:10px;padding:8px 6px;width:100%;margin-top:8px}.coupon-card .price{font-size:18px!important;margin:4px 0!important}.coupon-promo-card{min-height:0!important}" +
"#coupons.grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;align-items:stretch}#coupons.grid .coupon-card{width:100%;min-width:0;min-height:0;height:auto;aspect-ratio:1/1.08;display:flex;flex-direction:column;justify-content:space-between;padding:11px!important;margin:0!important}@media(max-width:650px){#coupons.grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}#coupons.grid .coupon-card{aspect-ratio:1/1.12;padding:7px!important}#coupons.grid .coupon-card h3{font-size:12px!important}#coupons.grid .coupon-card .price{font-size:13px!important}#coupons.grid .coupon-card .muted{font-size:7px;line-height:1.2;min-height:22px;max-height:30px}#coupons.grid .coupon-card .btn{font-size:7px;padding:6px 3px}}" +".coupon-promo-card .price,.coupon-card .price{color:#ffd84d!important;text-shadow:0 0 12px rgba(255,216,77,.18)}" +
".coupon-promo-card .muted,.coupon-card .muted{color:#d1c7a9!important}" +
".review-bottom{margin-top:34px}.delivery-address-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.cart-summary{display:flex;justify-content:space-between;gap:10px;font-size:12px;color:#9da9b6}.coupon-applied{color:#f1c75b;font-weight:900}.qty-control{display:flex;align-items:center;gap:5px;margin:7px 0}.qty-control button{border:1px solid #72551b;background:#15110a;color:#f4d26a;border-radius:8px;padding:4px 9px;font-weight:900}.qty-control input{width:54px!important;text-align:center;margin:0!important;padding:6px!important}.qty-control .remove-item{font-size:9px;padding:5px 7px}.buyer-account-link{color:#f4d26a!important}" +
"@media(min-width:651px) and (max-width:1100px){.store-product-grid{grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:10px!important}.store-product-card{padding:8px!important}.store-product-card h3{font-size:14px}.store-product-card .product-description{font-size:10px}.store-product-card .price{font-size:18px}}" +
"@media(max-width:650px){.store-product-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px!important}.store-product-card{padding:7px!important}.store-product-card h3{font-size:14px}.store-product-card .product-description{font-size:10px}.store-product-card .price{font-size:17px}.store-product-card .btn{font-size:10px;padding:8px 5px}}@media(max-width:1000px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.grid{grid-template-columns:1fr;gap:12px}.grid .card{min-height:300px;height:300px}.grid .card .name{font-size:20px}.grid .card .logo-wrap,.grid .card .logo-wrap .logo{width:68px;height:68px}}</style></head><body><div class='wrap'><div class='top'><a class='brand brand-home' href='/' aria-label='Volver al inicio de NexoMarket'><span>NEXO</span>MARKET</a><div class='nav'>"+storeAccountLinks+"</div></div><button id='scrollHome' class='scroll-home' type='button' onclick=\"location.href='/'\">NEXO<span>MARKET</span></button>"+(string.IsNullOrWhiteSpace(S(store,"StorePhoto"))?"":"<div class='store-cover-shell'><img class='store-cover-photo' src='"+E(S(store,"StorePhoto"))+"' alt='Foto del local' loading='eager'/><div class='store-cover-caption'>"+E(S(store,"Name"))+"</div></div>")+"<section class='hero store-hero'>").Append(logo).Append("<div class='store-hero-main'>").Append(publicTier).Append("<div class='live'>● TIENDA CENTRAL EN TIEMPO REAL</div><h1>").Append(E(S(store,"Name"))).Append("</h1><div class='muted'>").Append(E(S(store,"Category"))).Append(" · ").Append(E(S(store,"City"))).Append("</div><p class='muted'>").Append(E(S(store,"Description"))).Append("</p></div><div class='hero-rating'><div class='hero-rating-label'>REPUTACIÓN</div><div class='hero-rating-score'><span id='heroReviewAverage'>0.0</span><span class='hero-rating-max'>/5</span></div><div class='hero-rating-stars' id='heroReviewStars'>☆☆☆☆☆</div><div class='hero-rating-count' id='heroReviewCount'>0 reseñas</div></div></section>"+(S(store,"Active")=="0"?"<div class='closed-store-banner'>🔴 TIENDA CERRADA · En este momento no se aceptan nuevos pedidos.</div>":"")+"<div class='section'><div style='display:flex;justify-content:space-between;align-items:end'><div><h2>Productos</h2><div class='muted'>El catálogo se actualiza automáticamente desde NexoMarket Central.</div></div><span id='liveState' class='live'>● LIVE</span></div><div id='products' class='grid store-product-grid'>");
            lock(_sync)
            {
                XDocument d=LoadFile(_catalogFile,"NexoMarketCatalog","Products");
                ...
                foreach (XElement x in ps)
                {
                    ...
                    string priceDisplay = Money(shown).ToString("N2", CultureInfo.GetCultureInfo("es-AR"));
                    string stock = S(x, "Stock");
                    b.Append("<div class='card store-product-card'><div class='store-product-image-wrap'>").Append(image).Append("</div><h3>").Append(E(S(x, "Name"))).Append("</h3><div class='product-meta'>")
                        .Append(E(S(x, "Category"))).Append(" · ").Append(E(S(x, "Brand"))).Append("</div><div class='product-description'>")
                        .Append(E(S(x, "PublicDescription"))).Append("</div><div class='price ")
                        .Append(string.IsNullOrWhiteSpace(sale) || sale == "0" ? "" : "sale").Append("'>$ ").Append(E(priceDisplay))
                        .Append("</div><div class='stock'>Stock: ").Append(E(stock)).Append("</div><button class='btn' onclick='")
            ...
            lock (_sync)
            {
                ...
                foreach (XElement p in promotions)
                {
                    ...
                    b.Append("<div class='card coupon-promo-card'><div class='sale'>OFERTA</div><h3>").Append(E(pname))
            ...
            b.Append("</div><div class='coupon-subtitle'>CUPONES DISPONIBLES</div><div id='coupons' class='grid'>");
            lock(_sync)
            {
                XDocument cd=LoadFile(_catalogFile,"NexoMarketCatalog","Products");
                ...
                foreach(XElement c in coupons)
                {
                    ...
                    b.Append("<div class='card coupon-card'><div class='sale'>CUPÓN</div><h3>"+E(S(c,"Code"))+"</h3><div class='price sale'>"+E(discount)+" OFF</div><div class='muted'>"+E(S(c,"Description"))+"</div><button class='btn violet' onclick='useCoupon("+JsonString(S(c,"Code"))+","+JsonNumber(S(c,"DiscountPercent"))+","+JsonNumber(S(c,"DiscountAmount"))+")'>USAR CUPÓN</button></div>");
                }
            }
            b.Append("</div></section><div id='approvedOrderAlert' class='approved-order-alert' role='status'><div class='ao-title'>🟢 PEDIDO APROBADO</div><div class='ao-sub'>La tienda confirmó tu pedido. Ya está siendo preparado.</div></div>");
            b.Append("<button class='cart-fab' onclick='toggleCart()'>🛒 Carrito <span id='cartCount'>0</span></button><aside id='cartPanel' class='cart-panel cart'><h2>Tu carrito</h2><div id='cartItems'>Carrito vacío.</div><div class='cart-summary'><span>Subtotal: $ <b id='subtotalPreview'>0.00</b></span><span id='couponDiscountPreview'></span></div><h3>Total: $ <span id='total'>0</span></h3>");
            b.Append("<form onsubmit='return sendOrder(event)'><input type='hidden' id='storeId' value='").Append(E(realId)).Append("'/><input id='name' placeholder='Nombre completo' required/><input id='email' type='email' placeholder='Correo electrónico'/><input id='phone' placeholder='Teléfono'/><select id='fulfillment'><option>Delivery</option><option>Retiro</option></select><div class='delivery-address-grid'><input id='street' placeholder='Calle *'/><input id='houseNumber' placeholder='Número de casa *'/><input id='apartment' placeholder='Departamento (opcional)'/><input id='city' placeholder='Ciudad *'/><input id='province' placeholder='Provincia (opcional)'/></div><input id='address' placeholder='Dirección completa' readonly/><select id='paymentMethod'><option>Transferencia</option><option>Mercado Pago</option><option>Efectivo</option></select><input id='paymentReference' placeholder='Referencia de pago (opcional)'/>");
            b.Append("<label class='proof-upload'><b>📎 COMPROBANTE DE PAGO</b><small>Elegí una imagen del dispositivo.</small><input id='paymentProofFile' type='file' accept='image/jpeg,image/png,image/webp,image/gif' style='display:none'/><div class='proof-actions'><button type='button' class='btn' onclick='openProofFile()'>📁 ELEGIR IMAGEN</button></div><div id='proofPreview' class='proof-preview'></div><input type='hidden' id='paymentProofPath'/><span id='proofState' class='muted'>Todavía no cargaste el comprobante.</span></label>");
            b.Append("<input id='notes' placeholder='Nota general para el vendedor (opcional)'/><input id='couponCode' placeholder='Código de cupón (opcional)'/><button class='btn' type='submit'>CONFIRMAR PEDIDO</button></form><hr style='border-color:#253447;margin:18px 0'><h3>Seguimiento</h3><div id='orderStatus' class='muted'>Después de confirmar un pedido aparecerá aquí.</div><button class='btn' id='confirmReceived' style='display:none' onclick='confirmReceived()'>CONFIRMAR RECEPCIÓN</button><button class='btn violet' style='margin-left:8px' onclick='loadHistory()'>VER HISTORIAL</button><div id='history' class='muted' style='margin-top:12px'></div></aside>");
            b.Append("<div class='review-bottom'>").Append(reviewCardHtml).Append("</div>");
            b.Append("<script>");
            b.Append("var buyerLoggedEmail="+JsonString(viewer!=null&&viewer.Role=="buyer"?viewer.Email:"")+",buyerLoggedName="+JsonString(viewer!=null&&viewer.Role=="buyer"?viewer.Name:"")+",couponPercent=0,couponAmount=0,cart=[],lastOrderId='',proofInput=document.getElementById('paymentProofFile');function toggleCart(){document.getElementById('cartPanel').classList.toggle('open')}function addPromotion(id,name,price,productIds){var key='promo:'+id,x=cart.filter(function(i){return i.id===key})[0];if(x)x.qty++;else cart.push({id:key,name:name,price:price,qty:1,note:'',promotionId:id,productIds:productIds});render();toggleOpen()}function add(id,name,price){var x=cart.filter(function(i){return i.id===id})[0];if(x)x.qty++;else cart.push({id:id,name:name,price:price,qty:1,note:''});render();toggleOpen()}function changeQty(n,d){if(!cart[n])return;cart[n].qty=Math.max(1,(Number(cart[n].qty)||1)+d);render()}function setQty(n,v){if(!cart[n])return;var q=parseInt(v,10);cart[n].qty=Math.max(1,isNaN(q)?1:q);render()}function removeItem(n){cart.splice(n,1);render()}function toggleOpen(){document.getElementById('cartPanel').classList.add('open')}function updateItemNote(n,v){if(cart[n])cart[n].note=v||'';}function escReview(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(new RegExp(String.fromCharCode(34),'g'),'&quot;').replace(new RegExp(String.fromCharCode(39),'g'),'&#39;')}function render(){var h='',subtotal=0,c=0;cart.forEach(function(i,n){var line=(Number(i.price)||0)*(Number(i.qty)||0);h+=`<div class='item cart-item'><div><b>${escReview(i.name)}</b><div class='qty-control'><button type='button' onclick='changeQty(${n},-1)'>−</button><input type='number' min='1' value='${i.qty}' onchange='setQty(${n},this.value)'><button type='button' onclick='changeQty(${n},1)'>+</button><button type='button' class='remove-item' onclick='removeItem(${n})'>ELIMINAR</button></div><input class='item-note' placeholder='Nota para este producto (opcional)' value='${escReview(i.note||'')}' oninput='updateItemNote(${n},this.value)'></div><b>$ ${line.toFixed(2)}</b></div>`;subtotal+=line;c+=Number(i.qty)||0});var discount=couponPercent>0?Math.min(subtotal,subtotal*couponPercent/100):Math.min(subtotal,couponAmount),total=Math.max(0,subtotal-discount);document.getElementById('cartItems').innerHTML=h||'Carrito vacío.';document.getElementById('total').innerHTML=total.toFixed(2);document.getElementById('cartCount').innerHTML=c;var d=document.getElementById('couponDiscountPreview');if(d)d.innerHTML=discount>0?`<div class='coupon-applied'>✓ CUPÓN APLICADO · −$ ${discount.toFixed(2)}</div>`:'';var sub=document.getElementById('subtotalPreview');if(sub)sub.textContent=subtotal.toFixed(2)}function syncAddress(){var a=['street','houseNumber','apartment','city','province'].map(function(id){var e=document.getElementById(id);return e?(e.value||'').trim():''}).filter(Boolean);document.getElementById('address').value=a.join(', ')}function refreshCatalog(){var x=new XMLHttpRequest();x.open('GET','/api/catalog/live?storeId='+encodeURIComponent(document.getElementById('storeId').value),true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText),stamp=d.updatedAt||'';if(window.lastCatalogStamp===undefined)window.lastCatalogStamp=stamp;else if(stamp!==window.lastCatalogStamp){window.lastCatalogStamp=stamp;location.reload()}var live=document.getElementById('liveState');if(live)live.innerHTML='● LIVE '+(stamp?new Date(stamp).toLocaleTimeString():'')}catch(e){}}};x.send()}function pollStatus(){if(!lastOrderId)return;var u='/api/orders/status?storeId='+encodeURIComponent(document.getElementById('storeId').value)+'&centralOrderId='+encodeURIComponent(lastOrderId),x=new XMLHttpRequest();x.open('GET',u,true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText);if(d.error)return;var prev=window.__nexoBuyerStatus||'';document.getElementById('orderStatus').innerHTML='Pedido <b>'+escReview(d.centralOrderId)+'</b> · Estado: <b>'+escReview(d.status)+'</b><br>Total: $ '+escReview(d.total)+(d.updatedAt?' · Actualizado: '+new Date(d.updatedAt).toLocaleString():'');document.getElementById('confirmReceived').style.display=(d.status==='Entregado'&&!d.buyerConfirmed)?'inline-block':'none';if((prev==='Pendiente'||prev==='')&&d.status==='Preparando')showApprovedSignal();window.__nexoBuyerStatus=d.status}catch(e){}}};x.send()}function approvedBeep(){try{var C=window.AudioContext||window.webkitAudioContext;if(!C)return;window.__nexoApprovedAudio=window.__nexoApprovedAudio||new C();var a=window.__nexoApprovedAudio,o=a.createOscillator(),g=a.createGain();o.type='sine';o.frequency.value=880;g.gain.value=.07;o.connect(g);g.connect(a.destination);o.start();setTimeout(function(){o.frequency.value=1175},120);setTimeout(function(){o.stop()},240)}catch(e){}}function showApprovedSignal(){var el=document.getElementById('approvedOrderAlert');if(!el)return;el.className='approved-order-alert show';clearTimeout(window.__nexoApprovedTimer);clearInterval(window.__nexoApprovedBeepTimer);var n=0;approvedBeep();n=1;window.__nexoApprovedBeepTimer=setInterval(function(){if(n>=10){clearInterval(window.__nexoApprovedBeepTimer);return}approvedBeep();n++},1000);window.__nexoApprovedTimer=setTimeout(function(){el.className='approved-order-alert';clearInterval(window.__nexoApprovedBeepTimer)},10000)}function loadHistory(){location.href='/buyer';return;}function confirmReceived(){var data='storeId='+encodeURIComponent(document.getElementById('storeId').value)+'&centralOrderId='+encodeURIComponent(lastOrderId)+'&email='+encodeURIComponent(document.getElementById('email').value),x=new XMLHttpRequest();x.open('POST','/api/orders/confirm',true);x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){alert(x.responseText.indexOf('OK|')===0?'Recepción confirmada.':'No se pudo confirmar.');pollStatus()}};x.send(data)}function useCoupon(code,pct,amt){document.getElementById('couponCode').value=code;couponPercent=Number(pct)||0;couponAmount=Number(amt)||0;render();toggleOpen()}function toggleReviewForm(){var f=document.getElementById('reviewForm');if(f)f.style.display=f.style.display==='none'?'block':'none'}function pickRating(n){document.getElementById('reviewRating').value=n;var bs=document.querySelectorAll('#starPicker button');for(var i=0;i<bs.length;i++)bs[i].className=(i<n?'selected':'')}function addReviewEmoji(e){var x=document.getElementById('reviewEmoji');x.value=(x.value+' '+(e||'')).trim()}function starsText(n){n=Math.max(0,Math.min(5,Number(n)||0));var full=Math.round(n),s='';for(var i=0;i<full;i++)s+='★';while(s.length<5)s+='☆';return s}function loadReviews(){var x=new XMLHttpRequest();x.open('GET','/api/reviews?storeId='+encodeURIComponent(document.getElementById('storeId').value),true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText);var av=Number(d.average||0).toFixed(1),cnt=(d.count||0)+' reseña'+((d.count||0)===1?'':'s');document.getElementById('reviewAverage').textContent=av;document.getElementById('reviewStars').textContent=starsText(d.average||0);document.getElementById('reviewCount').textContent=cnt;document.getElementById('heroReviewAverage').textContent=av;document.getElementById('heroReviewStars').textContent=starsText(d.average||0);document.getElementById('heroReviewCount').textContent=cnt;var h='';(d.reviews||[]).slice(0,20).forEach(function(r){h+=`<div class='review-item'><div class='stars'>${starsText(r.rating)}</div><b>${escReview(r.author)}</b>${r.emoji?' <span>'+escReview(r.emoji)+'</span>':''}<small> · ${new Date(r.createdAt).toLocaleDateString()}</small>${r.comment?`<div style='margin-top:5px'>${escReview(r.comment)}</div>`:''}</div>`});document.getElementById('reviewList').innerHTML=h}catch(e){}}};x.send()}function saveReview(){var rating=parseInt(document.getElementById('reviewRating').value||'0',10),msg=document.getElementById('reviewMessage');if(rating<1||rating>5){msg.textContent='Elegí de 1 a 5 estrellas.';return}var data='storeId='+encodeURIComponent(document.getElementById('storeId').value)+'&centralOrderId='+encodeURIComponent(document.getElementById('reviewOrderId').value)+'&rating='+encodeURIComponent(rating)+'&comment='+encodeURIComponent(document.getElementById('reviewComment').value)+'&emoji='+encodeURIComponent(document.getElementById('reviewEmoji').value),x=new XMLHttpRequest();x.open('POST','/api/reviews/save',true);x.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=UTF-8');x.onreadystatechange=function(){if(x.readyState===4){try{var d=JSON.parse(x.responseText);msg.textContent=d.message||'Listo.';if(d.ok){loadReviews();document.getElementById('reviewComment').value='';document.getElementById('reviewEmoji').value='';pickRating(0)}}catch(e){msg.textContent='No se pudo guardar la reseña.'}}};x.send(data)}function proofToBase64(file){return new Promise(function(resolve,reject){var r=new FileReader();r.onload=function(){resolve(r.result.split(',')[1]||'')};r.onerror=reject;r.readAsDataURL(file)})}function uploadProof(file){return new Promise(async function(resolve,reject){try{if(!file){reject(new Error('El comprobante es obligatorio.'));return}if(file.type.indexOf('image/')!==0){reject(new Error('El comprobante debe ser una imagen.'));return}if(file.size>5*1024*1024){reject(new Error('El comprobante supera 5 MB.'));return}var base=await proofToBase64(file),body='storeId='+encodeURIComponent(document.getElementById('storeId').value)+'&fileName='+encodeURIComponent('comprobante-'+Date.now()+'.jpg')+'&contentType='+encodeURIComponent(file.type)+'&base64='+encodeURIComponent(base),x=new XMLHttpRequest();x.open('POST','/api/order-proof/upload',true);x.timeout=90000;x.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=UTF-8');x.onreadystatechange=function(){if(x.readyState===4){if(x.status===200&&x.responseText.indexOf('OK|')===0)resolve(x.responseText.split('|')[2]);else reject(new Error(x.responseText||'No se pudo subir el comprobante.'))}};x.onerror=function(){reject(new Error('No se pudo conectar con el almacenamiento.'))};x.ontimeout=function(){reject(new Error('La carga tardó demasiado.'))};x.send(body)}catch(e){reject(e)}})}function handleProofInput(input){if(!input)return;input.addEventListener('change',async function(){var f=this.files&&this.files[0],pr=document.getElementById('proofPreview'),st=document.getElementById('proofState');if(!f)return;if(f.type.indexOf('image/')!==0){st.textContent='Error: el comprobante debe ser una imagen.';this.value='';return}pr.innerHTML=`<img src='${URL.createObjectURL(f)}' alt='Comprobante'>`;st.textContent='Subiendo comprobante...';try{var url=await uploadProof(f);document.getElementById('paymentProofPath').value=url;st.textContent='✓ Comprobante cargado correctamente.'}catch(err){document.getElementById('paymentProofPath').value='';st.textContent='Error: '+err.message;this.value=''}})}function openProofFile(){if(proofInput)proofInput.click()}handleProofInput(proofInput);function sendOrder(e){e.preventDefault();if(!cart.length){alert('Agregá al menos un producto.');return false}var proof=document.getElementById('paymentProofPath').value;if(!proof){alert('Tenés que adjuntar el comprobante de pago antes de confirmar el pedido.');return false}var name=(document.getElementById('name').value||'').trim();if(!name){alert('Ingresá tu nombre completo.');document.getElementById('name').focus();return false}var fulfillment=document.getElementById('fulfillment').value;syncAddress();var street=(document.getElementById('street').value||'').trim(),house=(document.getElementById('houseNumber').value||'').trim(),city=(document.getElementById('city').value||'').trim(),address=(document.getElementById('address').value||'').trim();if(fulfillment==='Delivery'&&(!street||!house||!city)){alert('Para Delivery son obligatorios calle, número de casa y ciudad.');return false}var total=cart.reduce(function(a,i){return a+(Number(i.price)||0)*(Number(i.qty)||0)},0),data={storeId:document.getElementById('storeId').value,customerName:name,customerEmail:document.getElementById('email').value,phone:document.getElementById('phone').value,fulfillment:fulfillment,address:address,street:street,houseNumber:house,apartment:document.getElementById('apartment').value,city:city,province:document.getElementById('province').value,paymentMethod:document.getElementById('paymentMethod').value,paymentReference:document.getElementById('paymentReference').value,paymentProofPath:proof,notes:document.getElementById('notes').value,couponCode:document.getElementById('couponCode').value,total:total.toFixed(2),itemsJson:JSON.stringify(cart)},body=[];Object.keys(data).forEach(function(k){body.push(encodeURIComponent(k)+'='+encodeURIComponent(data[k]))});var btn=e.target.querySelector('button[type=submit]');if(btn){btn.disabled=true;btn.textContent='ENVIANDO PEDIDO...'}var x=new XMLHttpRequest();x.open('POST','/api/orders/create',true);x.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=UTF-8');x.timeout=90000;x.onreadystatechange=function(){if(x.readyState===4){if(btn){btn.disabled=false;btn.textContent='CONFIRMAR PEDIDO'}if(x.status===200&&x.responseText.indexOf('OK|')===0){lastOrderId=x.responseText.split('|')[1];try{localStorage.setItem('nexoLastOrder:'+document.getElementById('storeId').value,lastOrderId)}catch(e){};alert('Pedido enviado. Número central: '+lastOrderId);cart=[];render();pollStatus()}else alert('No se pudo enviar el pedido: '+(x.responseText||'sin respuesta'))}};x.onerror=function(){if(btn){btn.disabled=false;btn.textContent='CONFIRMAR PEDIDO'}alert('No se pudo conectar con NexoMarket.')};x.ontimeout=function(){if(btn){btn.disabled=false;btn.textContent='CONFIRMAR PEDIDO'}alert('El envío tardó demasiado. Intentá nuevamente.')};x.send(body.join('&'));return false}document.querySelectorAll(\"#street,#houseNumber,#apartment,#city,#province\").forEach(function(el){el.addEventListener(\"input\",syncAddress)});var nav=document.querySelector('.nav');if(nav&&buyerLoggedEmail){var a=document.createElement('a');a.href='/buyer';a.className='buyer-account-link';a.textContent='👤 MI CUENTA';nav.appendChild(a);var l=document.createElement('a');l.href='/logout';l.textContent='SALIR';nav.appendChild(l);}var reviewTarget=(new URLSearchParams(location.search).get('review')||'');var reviewButton=document.querySelector('.review-open');if(reviewButton&&!reviewTarget)reviewButton.style.display='none';if(buyerLoggedEmail){document.getElementById('email').value=buyerLoggedEmail;document.getElementById('email').readOnly=true;document.getElementById('name').value=buyerLoggedName;document.getElementById('name').readOnly=true;try{lastOrderId=localStorage.getItem('nexoLastOrder:'+document.getElementById('storeId').value)||'';if(lastOrderId)pollStatus()}catch(e){}}if(reviewTarget){document.getElementById(\"reviewOrderId\").value=reviewTarget;toggleReviewForm();setTimeout(function(){document.getElementById(\"storeReviews\").scrollIntoView({behavior:\"smooth\",block:\"center\"})},200)}render();loadReviews();setInterval(refreshCatalog,1800);setInterval(pollStatus,4000);(function(){var lastScroll=window.pageYOffset||0,home=document.getElementById('scrollHome');window.addEventListener('scroll',function(){var y=window.pageYOffset||0;if(y>55&&y<lastScroll)home.className='scroll-home show';else if(y<=55||y>lastScroll)home.className='scroll-home';lastScroll=y},{passive:true})})();</script></body></html>");
            return b.ToString();
        }
        ...
        private string StoreJson(string query)
        {
            ...
            using (StringReader reader = new StringReader(lines))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    cs.Distance = p.Length > 13 ? ParseDouble(Uri.UnescapeDataString(p[13])) : 0d; cs.Logo = p.Length > 14 ? Uri.UnescapeDataString(p[14]) : ""; bool publicStorePlus = p.Length > 20 && Uri.UnescapeDataString(p[20]) == "1"; bool publicCommon = p.Length > 15 && Uri.UnescapeDataString(p[15]) == "1"; cs.StorePlus = publicStorePlus; cs.Featured = publicCommon; cs.StorePhoto = p.Length > 16 ? Uri.UnescapeDataString(p[16]) : ""; cs.Address = p.Length > 17 ? Uri.UnescapeDataString(p[17]) : ""; cs.Description = p.Length > 18 ? Uri.UnescapeDataString(p[18]) : ""; cs.RatingSummary = p.Length > 19 ? Uri.UnescapeDataString(p[19]) : "0.0|0";
                    stores.Add(cs);
                }
            }
            ...
            for (int i = 0; i < stores.Count; i++)
            {
                ...
                b.Append("{\"storeId\":").Append(JsonString(x.StoreId)).Append(",\"name\":").Append(JsonString(x.Name))
                 ...
                 .Append(",\"pickup\":").Append(x.Pickup ? "true" : "false").Append(",\"distanceKm\":").Append(x.Distance.ToString("0.0", System.Globalization.CultureInfo.InvariantCulture)).Append(",\"featured\":").Append(x.Featured ? "true" : "false").Append(",\"storePlus\":").Append(x.StorePlus ? "true" : "false").Append(",\"ratingSummary\":").Append(JsonString(x.RatingSummary)).Append('}');
            }
            b.Append(']'); return b.ToString();
        }
        ...
        private string Marketplace(string query, string cookie)
        {
            ...
            List<CentralStore> stores = new List<CentralStore>();
            {
                ...
                using (StringReader reader = new StringReader(lines)) { string line; while ((line = reader.ReadLine()) != null) { string[] p = line.Split('|'); if (p.Length < 12) continue; CentralStore cs = new CentralStore(); cs.StoreId = Uri.UnescapeDataString(p[1]); cs.Name = Uri.UnescapeDataString(p[2]); cs.PublicUrl = Uri.UnescapeDataString(p[3]); cs.City = Uri.UnescapeDataString(p[4]); cs.Province = Uri.UnescapeDataString(p[5]); cs.Category = Uri.UnescapeDataString(p[6]); cs.Latitude = ParseDouble(Uri.UnescapeDataString(p[7])); cs.Longitude = ParseDouble(Uri.UnescapeDataString(p[8])); cs.Active = Uri.UnescapeDataString(p[9]) == "1"; cs.Delivery = Uri.UnescapeDataString(p[10]) == "1"; cs.Pickup = Uri.UnescapeDataString(p[11]) == "1"; cs.Distance = p.Length > 13 ? ParseDouble(Uri.UnescapeDataString(p[13])) : 0d; cs.Logo = p.Length > 14 ? Uri.UnescapeDataString(p[14]) : ""; bool publicStorePlus = p.Length > 20 && Uri.UnescapeDataString(p[20]) == "1"; bool publicCommon = p.Length > 15 && Uri.UnescapeDataString(p[15]) == "1"; cs.StorePlus = publicStorePlus; cs.Featured = publicCommon; cs.StorePhoto = p.Length > 16 ? Uri.UnescapeDataString(p[16]) : ""; cs.Address = p.Length > 17 ? Uri.UnescapeDataString(p[17]) : ""; cs.Description = p.Length > 18 ? Uri.UnescapeDataString(p[18]) : ""; cs.RatingSummary = p.Length > 19 ? Uri.UnescapeDataString(p[19]) : "0.0|0"; cs.Phone = p.Length > 21 ? Uri.UnescapeDataString(p[21]) : ""; stores.Add(cs); } }
            }
            StringBuilder b = new StringBuilder();
            ...
            b.Append("<style>\n*{box-sizing:border-box}html{background:#05070a}body{font-family:'Segoe UI',Arial,sans-serif;background:#05070a;color:#fff;margin:0;position:relative;overflow-x:hidden;min-height:100vh}body:before{content:'';position:fixed;inset:-18%;pointer-events:none;z-index:0;background:radial-gradient(ellipse at 5% 12%,transparent 0 24%,rgba(57,255,102,.10) 24.1%,transparent 24.65%),radial-gradient(ellipse at 94% 24%,transparent 0 21%,rgba(54,164,255,.12) 21.1%,transparent 21.65%),radial-gradient(ellipse at 74% 88%,transparent 0 23%,rgba(167,103,255,.12) 23.1%,transparent 23.65%),radial-gradient(circle at 18% 78%,rgba(57,255,102,.055),transparent 25%),radial-gradient(circle at 82% 8%,rgba(167,103,255,.065),transparent 24%);transform:rotate(-7deg);animation:marketGlow 18s ease-in-out infinite alternate}body:after{content:'';position:fixed;width:92vw;height:48vh;left:-30vw;bottom:4vh;border:1px solid rgba(255,255,255,.09);border-right-color:rgba(57,255,102,.24);border-radius:50%;transform:rotate(-15deg);pointer-events:none;z-index:0;box-shadow:0 0 90px rgba(57,255,102,.07),0 0 140px rgba(54,164,255,.04)}@keyframes marketGlow{from{transform:translate3d(-1%,0,0) rotate(-7deg)}to{transform:translate3d(1%,1%,0) rotate(-4deg)}}.wrap{max-width:1240px;margin:auto;padding:22px 22px 70px;position:relative;z-index:1}.wrap:before{content:'';position:fixed;inset:-10%;pointer-events:none;z-index:-1;background:linear-gradient(116deg,transparent 0 34%,rgba(255,255,255,.028) 34.1%,transparent 34.3%,transparent 63%,rgba(57,255,102,.025) 63.1%,transparent 63.35%),radial-gradient(ellipse at 15% 48%,transparent 0 28%,rgba(57,255,102,.05) 28.1%,transparent 28.55%),radial-gradient(ellipse at 88% 62%,transparent 0 24%,rgba(167,103,255,.055) 24.1%,transparent 24.55%);animation:marketLines 20s ease-in-out infinite alternate}@keyframes marketLines{from{transform:translateX(-1%) rotate(-3deg)}to{transform:translateX(1%) rotate(2deg)}}.top{display:flex;justify-content:space-between;align-items:center;padding:6px 4px 18px;position:relative}.top:after{content:'';position:absolute;left:0;right:0;bottom:4px;height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.25),rgba(57,255,102,.35),rgba(167,103,255,.30),transparent)}.brand{font-weight:900;font-size:23px;letter-spacing:-.5px}.brand .n{color:#39ff66;text-shadow:0 0 12px rgba(57,255,102,.45)}.top a{color:#dce7f1;text-decoration:none;margin-left:18px;font-weight:800}.top a:hover{color:#fff;text-shadow:0 0 12px rgba(255,255,255,.35)}.hero{padding:30px;border:1px solid rgba(255,255,255,.12);background:linear-gradient(135deg,rgba(12,22,31,.78),rgba(8,14,23,.68));border-radius:26px;box-shadow:0 24px 70px rgba(0,0,0,.34),inset 0 1px 0 rgba(255,255,255,.05);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);position:relative;overflow:hidden}.hero:before,.hero:after{content:'';position:absolute;pointer-events:none;border-radius:50%;border:1px solid rgba(255,255,255,.10);box-shadow:0 0 60px rgba(57,255,102,.08)}.hero:before{width:760px;height:340px;right:-390px;top:-190px;border-left-color:rgba(54,164,255,.35);transform:rotate(-15deg)}.hero:after{width:700px;height:300px;left:-430px;bottom:-190px;border-right-color:rgba(167,103,255,.28);transform:rotate(-8deg)}.hero>*{position:relative;z-index:1}.eyebrow{font-size:11px;letter-spacing:2px;color:#39ff66;font-weight:900;text-shadow:0 0 14px rgba(57,255,102,.35)}.nexo{color:#39ff66;font-size:49px;font-weight:950;text-shadow:0 0 18px rgba(57,255,102,.24)}.market{font-size:43px;font-weight:850;text-shadow:0 0 14px rgba(255,255,255,.10)}.hero-sub{color:#b9cce0;margin-top:10px;font-size:15px;line-height:1.55}.location-box{margin-top:20px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:10px;border:1px solid rgba(255,255,255,.09);border-radius:17px;background:rgba(2,7,12,.42);box-shadow:inset 0 1px 0 rgba(255,255,255,.03)}.location-box input{background:rgba(4,10,16,.62);color:#fff;border:1px solid rgba(82,170,255,.28);border-radius:12px;padding:12px 14px;width:330px;outline:none}.location-box input:focus{border-color:rgba(255,255,255,.55);box-shadow:0 0 18px rgba(54,164,255,.10)}.btn{background:linear-gradient(135deg,#39ff66,#72ff91);color:#041008;border:0;border-radius:12px;padding:11px 17px;font-weight:950;cursor:pointer;box-shadow:0 0 20px rgba(57,255,102,.13);transition:all .2s ease}.btn:hover{background:#fff;color:#000;transform:translateY(-2px);box-shadow:0 0 18px rgba(255,255,255,.5),0 0 38px rgba(57,255,102,.12)}.btn.alt{background:rgba(255,255,255,.045);color:#fff;border:1px solid rgba(255,255,255,.16);box-shadow:inset 0 1px 0 rgba(255,255,255,.05)}.hint{color:#7f96aa;font-size:12px;margin-top:12px}.section-head{display:flex;justify-content:space-between;align-items:end;margin:28px 2px 14px}.section-head h2{margin:0;font-size:25px}.section-head p{margin:6px 0 0;color:#8fa4b7;font-size:13px}.mini{color:#7dff9e;font-size:11px;font-weight:900;letter-spacing:.7px;text-shadow:0 0 12px rgba(57,255,102,.22)}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}.grid .card{display:grid;grid-template-columns:210px 1fr;column-gap:18px;align-items:start;min-height:260px}.grid .card .logo{float:none;width:210px;height:220px;grid-row:1 / span 9;margin:0}.grid .card .store-photo-mini{grid-column:2}.grid .card .promo-badges{grid-column:2}.grid .card .name{font-size:23px}.grid .card .meta{font-size:13px}.grid .card .open{font-size:13px}.grid .card .distance{font-size:12px}.card{display:block;color:#fff;text-decoration:none;background:linear-gradient(145deg,rgba(14,25,36,.76),rgba(8,13,21,.70));border:1px solid rgba(126,184,255,.20);border-radius:21px;padding:18px;min-height:160px;transition:transform .22s ease,border-color .22s ease,box-shadow .22s ease;position:relative;overflow:hidden;backdrop-filter:blur(15px);-webkit-backdrop-filter:blur(15px);box-shadow:0 16px 40px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.035)}.card:before{content:'';position:absolute;left:8%;right:8%;top:0;height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.36),rgba(54,164,255,.28),rgba(167,103,255,.30),transparent)}.card:after{content:'';position:absolute;width:190px;height:190px;right:-120px;bottom:-125px;border-radius:50%;border:1px solid rgba(54,164,255,.12);box-shadow:0 0 38px rgba(54,164,255,.06);pointer-events:none}.card:hover{transform:translateY(-5px);border-color:rgba(255,255,255,.38);box-shadow:0 0 28px rgba(54,164,255,.09),0 22px 48px rgba(0,0,0,.36),inset 0 1px 0 rgba(255,255,255,.07)}.logo-wrap{position:relative;width:210px;height:220px;grid-row:1 / span 10}.logo-wrap .logo{width:100%;height:100%}.logo-star{position:absolute;right:8px;top:8px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#05070a;color:#ffd84d;border:1px solid #ffd84d;box-shadow:0 0 16px rgba(255,216,77,.55);font-size:18px}.tier-badge{display:inline-flex;align-items:center;gap:6px;width:max-content;padding:7px 12px;border-radius:999px;font-size:10px;font-weight:950;letter-spacing:.7px;margin-bottom:8px}.tier-featured{color:#7cff9a;border:1px solid #39ff66;background:rgba(7,30,18,.7);box-shadow:0 0 14px rgba(57,255,102,.18),inset 0 0 12px rgba(57,255,102,.04);position:relative;isolation:isolate;text-shadow:0 0 9px rgba(57,255,102,.38)}.tier-featured:after{content:'';position:absolute;inset:-7px -12px;border-radius:999px;background:radial-gradient(ellipse at center,rgba(57,255,102,.22),transparent 68%);filter:blur(7px);opacity:.72;z-index:-1;pointer-events:none}.card.featured{border:1px solid rgba(57,255,102,.72);box-shadow:0 0 18px rgba(57,255,102,.12),inset 0 0 24px rgba(57,255,102,.025)}.tier-store-plus{color:#e1c7ff;border:1px solid #39ff66;background:linear-gradient(90deg,rgba(113,45,180,.42),rgba(26,17,45,.82));box-shadow:0 0 18px rgba(167,103,255,.55),0 0 30px rgba(57,255,102,.20),inset 0 0 22px rgba(167,103,255,.18)}.card.plus{border:1px solid #39ff66;background:linear-gradient(145deg,rgba(48,19,75,.78),rgba(7,18,24,.76));box-shadow:0 0 26px rgba(167,103,255,.30),0 0 38px rgba(57,255,102,.14),inset 0 0 30px rgba(167,103,255,.10);isolation:isolate}.card.plus:after{content:'';position:absolute;inset:-20px;z-index:-1;border-radius:28px;background:radial-gradient(ellipse at 50% 50%,rgba(167,103,255,.34),rgba(167,103,255,.13) 42%,transparent 72%);filter:blur(18px);opacity:.35;animation:plusNeonPulse 5s ease-in-out infinite;pointer-events:none}.card.plus .tier-store-plus{position:relative;z-index:2}.tier-store-plus{text-shadow:0 0 10px rgba(210,169,255,.45)}@keyframes plusNeonPulse{0%,100%{opacity:.24;transform:scale(.985)}50%{opacity:.68;transform:scale(1.015)}}@media(prefers-reduced-motion:reduce){.card.plus:after{animation:none;opacity:.38}}.card.plus:before{height:2px;background:linear-gradient(90deg,transparent,#39ff66,#b86cff,#39ff66,transparent);box-shadow:0 0 12px rgba(57,255,102,.5)}.rating-public{margin:7px 0 4px;color:#ffd84d;font-size:13px;font-weight:900;text-shadow:0 0 8px rgba(255,216,77,.18)}.rating-public b{color:#fff}.card .logo{width:78px;height:78px;border-radius:18px;background:linear-gradient(145deg,rgba(5,14,21,.75),rgba(11,12,25,.72));border:1px solid rgba(167,103,255,.45);display:flex;align-items:center;justify-content:center;color:#d1b4ff;font-size:30px;font-weight:900;float:left;margin-right:15px;overflow:hidden;box-shadow:0 0 22px rgba(167,103,255,.08)}.logo img{width:100%;height:100%;object-fit:cover;display:block}.name{font-size:20px;font-weight:950;padding-top:3px}.meta{color:#92a9bc;font-size:13px;margin-top:7px}.open{color:#55ff7e;font-size:12px;margin-top:12px;font-weight:800}.open.closed{color:#ff7187}.distance{color:#d9bb5c;font-size:12px;margin-top:6px}.store-photo-mini{margin-top:10px;height:58px;border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,.08)}.store-photo-mini img{width:100%;height:100%;object-fit:cover}.promo-badges{display:flex;gap:4px;flex-wrap:wrap;margin-top:9px}.promo-badges span{font-size:9px;color:#fff;background:rgba(167,103,255,.18);border:1px solid rgba(167,103,255,.38);border-radius:999px;padding:4px 7px;white-space:nowrap}.empty{margin-top:18px;border:1px dashed rgba(126,184,255,.28);border-radius:18px;padding:28px;color:#a2b4c2;background:rgba(8,16,25,.56);backdrop-filter:blur(12px)}.empty b{font-size:18px}.auth-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(310px,1fr));gap:15px;margin-top:22px}.panel{background:rgba(7,13,20,.72);border:1px solid rgba(255,255,255,.10);border-radius:19px;padding:20px;backdrop-filter:blur(14px);box-shadow:0 14px 35px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.035)}.panel h2{margin-top:0}.panel p{color:#92a6b7;font-size:13px;line-height:1.5}.footer{margin-top:38px;border-top:1px solid rgba(255,255,255,.08);padding-top:15px;color:#60768a;font-size:11px}@media(max-width:950px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.grid{grid-template-columns:1fr}.grid .card{display:block;min-height:0}.grid .card .logo{width:100%;height:150px;margin-bottom:10px}.grid .card .store-photo-mini,.grid .card .promo-badges{grid-column:auto}}@media(max-width:650px){.wrap{padding:14px}.top{align-items:flex-start;gap:10px;flex-direction:column}.top a{margin-left:0;margin-right:14px}.grid{grid-template-columns:1fr}.nexo{font-size:37px}.market{font-size:32px}.location-box{align-items:stretch}.location-box input{width:100%}.hero{padding:22px}.section-head{align-items:flex-start;gap:10px;flex-direction:column}}.store-card-cover{position:absolute;inset:0;z-index:0;overflow:hidden;border-radius:21px}.store-card-cover img{width:100%;height:100%;object-fit:cover;display:block;filter:saturate(.78) brightness(.52)}.store-card-cover:after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,rgba(3,7,12,.95) 0%,rgba(4,9,16,.86) 46%,rgba(5,9,16,.60) 100%),linear-gradient(180deg,rgba(3,7,12,.08),rgba(3,7,12,.90))}.grid .card .store-card-content{position:relative;z-index:1;display:block}.grid .card .logo-wrap{width:78px;height:78px;display:block;margin:0 0 10px}.grid .card .logo-wrap .logo{width:78px;height:78px;float:none;margin:0}.grid .card .name{color:#dfe5ed;text-shadow:0 0 8px rgba(255,255,255,.18),0 0 18px rgba(167,103,255,.10)}.grid .card .meta{color:#aeb9c5;text-shadow:0 0 7px rgba(255,255,255,.07)}.grid .card .open,.grid .card .distance,.grid .card .rating-public{position:relative;z-index:2}.card.featured .store-card-cover img{filter:saturate(.86) brightness(.50)}.card.plus .store-card-cover img{filter:saturate(.90) brightness(.46)}@media(max-width:650px){.grid .card .logo-wrap,.grid .card .logo-wrap .logo{width:72px;height:72px}.grid .card .name{font-size:19px}}.nexo-cta{margin-top:26px;padding:28px;border-radius:24px;border:1px solid rgba(57,255,102,.30);background:linear-gradient(135deg,rgba(15,22,31,.72),rgba(47,19,77,.55),rgba(7,34,20,.52));box-shadow:0 18px 55px rgba(0,0,0,.30),0 0 35px rgba(167,103,255,.10),inset 0 1px 0 rgba(255,255,255,.08);backdrop-filter:blur(18px);display:flex;justify-content:space-between;gap:24px;align-items:center;overflow:hidden;position:relative}.nexo-cta:before{content:'';position:absolute;inset:-50%;background:radial-gradient(circle at 25% 40%,rgba(57,255,102,.10),transparent 28%),radial-gradient(circle at 80% 60%,rgba(167,103,255,.14),transparent 30%);animation:ctaGlow 8s ease-in-out infinite alternate}.nexo-cta>*{position:relative;z-index:1}.nexo-cta h2{font-size:26px;margin:8px 0;color:#dfe6ed;text-shadow:0 0 10px rgba(255,255,255,.15),0 0 20px rgba(167,103,255,.12)}.nexo-cta p{max-width:850px;color:#b7c3ce;line-height:1.6}.cta-glow{font-size:38px;line-height:1.15;text-align:center;filter:drop-shadow(0 0 14px rgba(57,255,102,.35));min-width:70px}.buyer-panel,.seller-panel{background:linear-gradient(145deg,rgba(12,20,29,.70),rgba(20,12,34,.60));border-color:rgba(255,255,255,.12);backdrop-filter:blur(16px);box-shadow:0 16px 45px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.06)}.panel-icon{font-size:28px;margin-bottom:4px;filter:drop-shadow(0 0 10px rgba(255,255,255,.18))}.auth-grid .panel{transition:transform .22s ease,border-color .22s ease,box-shadow .22s ease}.auth-grid .panel:hover{transform:translateY(-5px);border-color:rgba(57,255,102,.32);box-shadow:0 0 28px rgba(57,255,102,.08),0 18px 45px rgba(0,0,0,.35)}@keyframes ctaGlow{from{transform:translate3d(-1%,0,0) scale(1)}to{transform:translate3d(1%,1%,0) scale(1.04)}}h1,h2,h3,.name{color:#d7dde4;text-shadow:0 0 7px rgba(255,255,255,.12),0 0 15px rgba(167,103,255,.09)}.meta,.muted,.hint,.hero-sub,.footer{color:#aeb9c4}.grid .card{background:linear-gradient(145deg,rgba(7,11,17,.72),rgba(14,18,28,.62));backdrop-filter:blur(13px);border-color:rgba(255,255,255,.12)}.section-head{margin-top:28px}.btn{box-shadow:0 0 16px rgba(57,255,102,.12)}@media(max-width:650px){.nexo-cta{padding:20px;flex-direction:column;align-items:flex-start}.nexo-cta h2{font-size:21px}.cta-glow{display:none}.nexo-cta .btn{margin-top:7px;width:100%}}.grid .card{display:grid;grid-template-columns:45% 55%;padding:0;min-height:280px;overflow:hidden;background:linear-gradient(135deg,rgba(7,10,16,.54),rgba(14,10,24,.48));backdrop-filter:blur(10px);border-color:rgba(255,255,255,.14)}.grid .card .store-card-cover{position:relative;inset:auto;grid-column:1;height:100%;min-height:280px;border-radius:0;overflow:hidden}.grid .card .store-card-cover img{filter:saturate(.86) brightness(.70);transition:transform .35s ease,filter .35s ease}.grid .card .store-card-cover:after{background:linear-gradient(90deg,rgba(3,7,12,.06),rgba(3,7,12,.18)),linear-gradient(180deg,rgba(3,7,12,.02),rgba(3,7,12,.35))}.grid .card:hover .store-card-cover img{transform:scale(1.035);filter:saturate(.92) brightness(.78)}.grid .card .store-card-content{grid-column:2;padding:18px 18px 18px 20px;display:flex;flex-direction:column;justify-content:center;min-width:0;background:linear-gradient(145deg,rgba(8,12,19,.56),rgba(18,12,30,.42));backdrop-filter:blur(8px)}.grid .card .logo-wrap{width:98px;height:98px;margin:0 0 12px}.grid .card .logo-wrap .logo,.grid .card .logo-wrap .logo img{width:98px;height:98px;border-radius:22px}.grid .card .logo-wrap .logo{overflow:hidden;box-shadow:0 0 22px rgba(57,255,102,.10),0 0 28px rgba(167,103,255,.10)}.grid .card .name{font-size:22px;line-height:1.05}.grid .card .meta{font-size:12px;line-height:1.45}.grid .card .rating-public{color:#ffe39a;text-shadow:0 0 10px rgba(255,216,77,.18)}.cover-fallback{display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at 50% 45%,rgba(57,255,102,.14),transparent 36%),linear-gradient(145deg,#070b10,#1b1030)}.cover-fallback div{font-size:28px;font-weight:950;color:#eef4f7;text-shadow:0 0 14px rgba(255,255,255,.20)}.cover-fallback span{color:#39ff66}.scroll-home{background:linear-gradient(135deg,rgba(3,6,10,.96),rgba(20,10,31,.92))!important;color:#eef3f7!important;border:1px solid rgba(255,255,255,.25)!important;box-shadow:0 0 12px rgba(255,255,255,.10),0 0 22px rgba(57,255,102,.08)!important}.scroll-home span{color:#39ff66!important}.scroll-home.show{display:block}@media(max-width:650px){.grid{grid-template-columns:1fr}.grid .card{display:grid;grid-template-columns:42% 58%;min-height:245px}.grid .card .store-card-cover{min-height:245px}.grid .card .store-card-content{padding:13px}.grid .card .logo-wrap,.grid .card .logo-wrap .logo,.grid .card .logo-wrap .logo img{width:76px;height:76px;border-radius:17px}.grid .card .name{font-size:18px}.grid .card .meta{font-size:10px}.grid .card .open,.grid .card .distance,.grid .card .rating-public{font-size:10px}}\n/* NEXOMARKET CARD SYSTEM V8 — HARD OVERRIDE: this block intentionally comes LAST. */\n.nexo-directory-v8{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:20px!important;width:100%!important;margin:20px 0 0!important;align-items:stretch!important}\n.nexo-store-v8{display:block!important;position:relative!important;height:326px!important;min-height:326px!important;width:100%!important;overflow:hidden!important;padding:0!important;margin:0!important;border-radius:24px!important;text-decoration:none!important;color:#fff!important;background:#080c12!important;border:1px solid rgba(255,255,255,.14)!important;box-shadow:0 18px 55px rgba(0,0,0,.42),0 0 0 1px rgba(255,255,255,.02) inset!important;isolation:isolate!important;transform:none!important;transition:transform .3s ease,box-shadow .3s ease,border-color .3s ease!important}\n.nexo-store-v8:hover{transform:translateY(-8px)!important;border-color:rgba(57,255,102,.58)!important;box-shadow:0 28px 70px rgba(0,0,0,.52),0 0 34px rgba(57,255,102,.13),0 0 70px rgba(167,103,255,.07)!important}\n.nexo-cover-v8{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;z-index:0!important;overflow:hidden!important;background:radial-gradient(circle at 20% 20%,rgba(57,255,102,.18),transparent 38%),radial-gradient(circle at 85% 20%,rgba(167,103,255,.22),transparent 42%),#070b11!important}\n.nexo-cover-image{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:cover!important;display:block!important;filter:saturate(.88) contrast(1.04) brightness(.58)!important;transform:scale(1.03)!important;transition:transform .55s ease,filter .55s ease!important}\n.nexo-store-v8:hover .nexo-cover-image{transform:scale(1.09)!important;filter:saturate(1.02) contrast(1.05) brightness(.68)!important}\n.nexo-cover-shade{position:absolute!important;inset:0!important;background:linear-gradient(180deg,rgba(3,6,10,.05) 0%,rgba(3,6,10,.12) 28%,rgba(3,6,10,.62) 56%,rgba(3,6,10,.97) 100%),linear-gradient(110deg,rgba(57,255,102,.07),transparent 48%,rgba(167,103,255,.14))!important;z-index:2!important}\n.nexo-cover-glow{position:absolute!important;left:-15%;right:-15%;bottom:-35%;height:65%!important;background:radial-gradient(ellipse,rgba(57,255,102,.18),transparent 62%)!important;filter:blur(22px)!important;z-index:3!important;pointer-events:none!important}\n.nexo-shine{position:absolute!important;inset:-80% -35%!important;background:linear-gradient(115deg,transparent 43%,rgba(255,255,255,.16) 49%,transparent 54%)!important;z-index:4!important;transform:translateX(-35%) rotate(5deg)!important;transition:transform .75s ease!important;pointer-events:none!important}\n.nexo-store-v8:hover .nexo-shine{transform:translateX(35%) rotate(5deg)!important}\n.nexo-cover-fallback{position:absolute!important;inset:0!important;display:flex!important;align-items:center!important;justify-content:center!important;font-size:34px!important;font-weight:950!important;letter-spacing:-1px!important;color:#e9eff4!important;background:radial-gradient(circle at 25% 25%,rgba(57,255,102,.22),transparent 35%),radial-gradient(circle at 80% 65%,rgba(167,103,255,.26),transparent 36%),linear-gradient(145deg,#060a0f,#18102b)!important}\n.nexo-cover-fallback span,.nexo-cover-fallback-layer span{color:#39ff66!important}\n.nexo-cover-fallback-layer{position:absolute!important;inset:0!important;display:none!important;align-items:center!important;justify-content:center!important;font-size:32px!important;font-weight:950!important;color:#edf3f7!important;z-index:1!important}.nexo-cover-fallback-layer.show{display:flex!important}\n.nexo-logo-v8{position:absolute!important;left:16px!important;top:16px!important;z-index:10!important;width:68px!important;height:68px!important}\n.nexo-logo-box{width:68px!important;height:68px!important;border-radius:19px!important;overflow:hidden!important;background:rgba(5,9,14,.78)!important;border:1px solid rgba(255,255,255,.34)!important;box-shadow:0 10px 30px rgba(0,0,0,.45),0 0 24px rgba(57,255,102,.10),inset 0 1px 0 rgba(255,255,255,.12)!important;backdrop-filter:blur(12px)!important;display:flex!important;align-items:center!important;justify-content:center!important}\n.nexo-logo-box img{width:100%!important;height:100%!important;object-fit:cover!important;display:block!important}.nexo-logo-letter{font-size:28px!important;font-weight:950!important;color:#39ff66!important;text-shadow:0 0 18px rgba(57,255,102,.55)!important}\n.nexo-star{position:absolute!important;right:-8px!important;top:-8px!important;width:28px!important;height:28px!important;border-radius:50%!important;display:flex!important;align-items:center!important;justify-content:center!important;background:#111722!important;border:1px solid #ffd84d!important;color:#ffd84d!important;font-size:14px!important;box-shadow:0 0 16px rgba(255,216,77,.45)!important}\n.nexo-tier{position:absolute!important;right:15px!important;top:16px!important;z-index:60!important;padding:7px 10px!important;border-radius:999px!important;font-size:9px!important;font-weight:950!important;letter-spacing:.7px!important;color:#7cff9a!important;background:rgba(4,19,10,.78)!important;border:1px solid rgba(57,255,102,.78)!important;box-shadow:0 0 18px rgba(57,255,102,.18)!important;backdrop-filter:blur(10px)!important}.nexo-tier.store-plus{color:#f0dcff!important;border-color:#d39bff!important;background:linear-gradient(90deg,rgba(94,39,146,.75),rgba(19,10,31,.82))!important;box-shadow:0 0 20px rgba(167,103,255,.48),0 0 30px rgba(57,255,102,.10)!important}.nexo-cover-v8>.nexo-tier.store-plus{display:block!important;visibility:visible!important;opacity:1!important;z-index:60!important;pointer-events:none!important;white-space:nowrap!important}\n.nexo-card-body{position:absolute!important;left:0!important;right:0!important;bottom:0!important;z-index:8!important;padding:0 17px 16px!important;min-width:0!important;background:linear-gradient(180deg,transparent 0%,rgba(4,7,12,.20) 10%,rgba(4,7,12,.84) 38%,rgba(4,7,12,.97) 100%)!important}\n.nexo-kicker{font-size:9px!important;line-height:1.2!important;text-transform:uppercase!important;letter-spacing:1.15px!important;color:#b4c0cb!important;font-weight:850!important;margin-bottom:5px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}.nexo-kicker i{font-style:normal;color:#39ff66!important;margin:0 2px!important}\n.nexo-card-body h3{font-size:22px!important;line-height:1.05!important;margin:0!important;color:#fff!important;font-weight:950!important;letter-spacing:-.45px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;text-shadow:0 2px 16px rgba(0,0,0,.85)!important}\n.nexo-status{margin-top:6px!important;font-size:10px!important;font-weight:950!important;letter-spacing:.5px!important}.nexo-status.open{color:#39ff66!important;text-shadow:0 0 12px rgba(57,255,102,.35)!important}.nexo-status.closed{color:#ff7180!important}\n.nexo-description{font-size:10px!important;line-height:1.35!important;color:#c0cad3!important;margin:6px 0 8px!important;display:-webkit-box!important;-webkit-line-clamp:2!important;-webkit-box-orient:vertical!important;overflow:hidden!important;min-height:27px!important;text-shadow:0 2px 8px rgba(0,0,0,.8)!important}\n.nexo-facts{display:flex!important;gap:5px!important;flex-wrap:wrap!important;max-height:27px!important;overflow:hidden!important}.nexo-facts span{display:inline-flex!important;align-items:center!important;padding:5px 7px!important;border-radius:8px!important;background:rgba(255,255,255,.07)!important;border:1px solid rgba(255,255,255,.09)!important;color:#d9e1e8!important;font-size:8.5px!important;font-weight:800!important;backdrop-filter:blur(8px)!important}\n.nexo-bottom{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:10px!important;margin-top:9px!important;padding-top:9px!important;border-top:1px solid rgba(255,255,255,.10)!important}.nexo-rating{font-size:10px!important;font-weight:900!important;color:#ffd86a!important}.nexo-rating small{color:#aeb8c2!important;font-weight:700!important}.nexo-arrow{font-size:8.5px!important;font-weight:950!important;letter-spacing:.7px!important;color:#39ff66!important}.nexo-arrow b{font-size:15px!important;margin-left:3px!important;display:inline-block!important;transition:transform .25s ease!important}.nexo-store-v8:hover .nexo-arrow b{transform:translateX(4px)!important}\n.nexo-store-v8 .promo-badges{position:absolute!important;left:17px!important;top:90px!important;z-index:9!important;display:flex!important;gap:4px!important;max-width:calc(100% - 34px)!important;overflow:hidden!important}.nexo-store-v8 .promo-badges span{font-size:8px!important;padding:4px 6px!important;border-radius:999px!important;background:rgba(167,103,255,.28)!important;border:1px solid rgba(255,255,255,.18)!important;backdrop-filter:blur(8px)!important;white-space:nowrap!important}\n.nexo-store-v8.nexo-featured{border-color:rgba(57,255,102,.58)!important}.nexo-store-v8.nexo-featured:before{content:'';position:absolute!important;left:8%!important;right:8%!important;top:0!important;height:2px!important;background:linear-gradient(90deg,transparent,#39ff66,transparent)!important;box-shadow:0 0 18px rgba(57,255,102,.65)!important;z-index:20!important}.nexo-store-v8.nexo-featured .nexo-tier:not(.plus){text-shadow:0 0 9px rgba(57,255,102,.50)!important}.nexo-store-v8.nexo-featured .nexo-tier:not(.plus):after{content:'';position:absolute!important;inset:-8px -13px!important;border-radius:999px!important;background:radial-gradient(ellipse at center,rgba(57,255,102,.25),transparent 70%)!important;filter:blur(8px)!important;opacity:.75!important;z-index:-1!important;pointer-events:none!important}.nexo-store-v8.nexo-store-plus{border-color:rgba(196,126,255,.82)!important;isolation:isolate!important}.nexo-store-v8.nexo-store-plus:before{content:'';position:absolute!important;left:6%!important;right:6%!important;top:0!important;height:2px!important;background:linear-gradient(90deg,transparent,#39ff66,#c47eff,#39ff66,transparent)!important;box-shadow:0 0 20px rgba(196,126,255,.70)!important;z-index:20!important}.nexo-store-v8.nexo-store-plus:after{content:'';position:absolute!important;inset:-24px!important;border-radius:28px!important;background:radial-gradient(ellipse at 50% 48%,rgba(167,103,255,.38),rgba(167,103,255,.13) 43%,transparent 74%)!important;filter:blur(19px)!important;opacity:.28!important;animation:nexoPlusPulse5 5s ease-in-out infinite!important;pointer-events:none!important;z-index:-1!important}.nexo-store-v8.nexo-store-plus .nexo-tier.store-plus{animation:nexoPlusBadge5 5s ease-in-out infinite!important}.nexo-store-v8.nexo-store-plus .nexo-tier.store-plus:after{content:'';position:absolute!important;inset:-9px -14px!important;border-radius:999px!important;background:radial-gradient(ellipse at center,rgba(167,103,255,.38),transparent 70%)!important;filter:blur(9px)!important;opacity:.30!important;z-index:-1!important;pointer-events:none!important}@keyframes nexoPlusPulse5{0%,100%{opacity:.20;transform:scale(.985)}50%{opacity:.66;transform:scale(1.015)}}@keyframes nexoPlusBadge5{0%,100%{filter:brightness(1);transform:translateZ(0)}50%{filter:brightness(1.10);transform:translateZ(0)}}@media(prefers-reduced-motion:reduce){.nexo-store-v8.nexo-store-plus:after,.nexo-store-v8.nexo-store-plus .nexo-tier.store-plus{animation:none!important}.nexo-store-v8.nexo-store-plus:after{opacity:.32!important}}\n.nexo-front-tier{position:absolute!important;left:16px!important;top:16px!important;z-index:100!important;display:inline-flex!important;align-items:center!important;gap:6px!important;padding:8px 12px!important;border-radius:999px!important;font-size:10px!important;font-weight:950!important;letter-spacing:.8px!important;line-height:1!important;white-space:nowrap!important;pointer-events:none!important;backdrop-filter:blur(12px)!important;-webkit-backdrop-filter:blur(12px)!important}.nexo-front-tier.plus{color:#f5dcff!important;background:linear-gradient(100deg,rgba(92,39,142,.97),rgba(35,17,55,.97))!important;border:1px solid #d39bff!important;box-shadow:0 0 14px rgba(183,117,255,.75),0 0 26px rgba(255,211,74,.18)!important;text-shadow:0 0 10px rgba(231,203,255,.65)!important}.nexo-front-tier.plus:before{content:'★'!important;color:#ffd84d!important;text-shadow:0 0 10px rgba(255,216,77,.9)!important}.nexo-front-tier.common{color:#7cff9a!important;background:rgba(5,29,16,.97)!important;border:1px solid #39ff66!important;box-shadow:0 0 14px rgba(57,255,102,.42)!important}.nexo-store-v8.nexo-store-plus>.nexo-front-tier.plus{display:inline-flex!important;visibility:visible!important;opacity:1!important}
@media(max-width:1000px){.nexo-directory-v8{grid-template-columns:repeat(2,minmax(0,1fr))!important}}\n@media(max-width:640px){.nexo-directory-v8{grid-template-columns:1fr!important;gap:14px!important}.nexo-store-v8{height:310px!important;min-height:310px!important}.nexo-card-body h3{font-size:21px!important}}\n\n/* NexoMarket 5.19.5 - responsive CTA/panel layout fix */\n.nexo-cta .cta-copy{min-width:0;flex:1;display:flex;flex-direction:column;align-items:flex-start}\n.nexo-cta .cta-copy .eyebrow{display:inline-block;margin-bottom:4px;color:#39ff66;font-weight:950;letter-spacing:1.1px;text-shadow:0 0 14px rgba(57,255,102,.25)}\n.nexo-cta .cta-copy h2{line-height:1.2;max-width:900px;background:linear-gradient(90deg,#ffffff 0%,#8fd7ff 48%,#c38cff 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;color:#eaf2ff}\n.nexo-cta .cta-copy p{margin:8px 0 16px;max-width:850px}\n.nexo-cta .cta-copy .btn{display:flex;align-items:center;justify-content:center;gap:7px;width:auto;min-height:46px;margin:5px 0;box-sizing:border-box;text-align:center;text-decoration:none;line-height:1.2;white-space:normal}\n.nexo-cta .cta-copy .btn.alt{background:linear-gradient(135deg,#39ff66,#72ff91);color:#041008;border-color:transparent;box-shadow:0 8px 24px rgba(57,255,102,.18),0 0 24px rgba(57,255,102,.10)}\n.nexo-cta .cta-copy .btn.ghost{background:rgba(255,255,255,.055);color:#eef5ff;border:1px solid rgba(142,202,255,.28)}\n.auth-grid .panel{display:flex;flex-direction:column;align-items:flex-start;min-width:0;box-sizing:border-box}\n.auth-grid .panel h2{font-size:27px;line-height:1.15;margin:3px 0 10px;background:linear-gradient(90deg,#f3f7ff,#9fd8ff,#c99bff);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;color:#eaf2ff}\n.auth-grid .panel p{font-size:15px;line-height:1.65;margin:0 0 15px;color:#aabed0}\n.auth-grid .panel .gold-link{display:flex;align-items:center;justify-content:center;width:auto;min-height:46px;box-sizing:border-box;margin-top:auto;text-decoration:none;line-height:1.15;white-space:normal;text-align:center}\n.auth-grid .buyer-panel{border-color:rgba(82,186,255,.22);background:linear-gradient(145deg,rgba(10,22,34,.82),rgba(24,15,43,.68))}\n.auth-grid .seller-panel{border-color:rgba(57,255,102,.22);background:linear-gradient(145deg,rgba(10,25,22,.80),rgba(21,13,39,.68))}\n.auth-grid .buyer-panel .gold-link{background:linear-gradient(135deg,#55c8ff,#8f7cff);color:#06101b;border:0;box-shadow:0 8px 24px rgba(82,186,255,.18)}\n.auth-grid .seller-panel .gold-link{background:linear-gradient(135deg,#39ff66,#8dffad);color:#041008;border:0;box-shadow:0 8px 24px rgba(57,255,102,.18)}\n.auth-grid .panel .mini{margin-top:12px;line-height:1.35;position:static;display:block}\n@media(max-width:650px){\n  .nexo-cta{display:block;padding:22px 18px;margin-top:22px;border-radius:22px}\n  .nexo-cta .cta-copy{width:100%;display:flex;flex-direction:column;align-items:stretch}\n  .nexo-cta .cta-copy .eyebrow{font-size:11px;line-height:1.4}\n  .nexo-cta .cta-copy h2{font-size:25px;line-height:1.22;margin:9px 0 10px;max-width:none}\n  .nexo-cta .cta-copy p{font-size:14px;line-height:1.55;margin:0 0 12px;max-width:none}\n  .nexo-cta .cta-copy .btn{width:100%;min-height:50px;margin:5px 0;padding:13px 12px;font-size:14px}\n  .nexo-cta .cta-glow{display:none}\n  .auth-grid{grid-template-columns:1fr;gap:14px;margin-top:18px}\n  .auth-grid .panel{width:100%;padding:20px 18px;border-radius:20px}\n  .auth-grid .panel-icon{font-size:30px;margin-bottom:5px}\n  .auth-grid .panel h2{font-size:26px;margin:3px 0 10px}\n  .auth-grid .panel p{font-size:14px;line-height:1.6;margin-bottom:15px}\n  .auth-grid .panel .gold-link{width:100%;min-height:50px;margin-top:0;padding:13px 12px;font-size:14px}\n  .auth-grid .panel .mini{width:100%;margin-top:11px;font-size:10px;line-height:1.4;white-space:normal}\n}\n@media(min-width:651px){\n  .auth-grid .panel{min-height:235px}\n}</style></head><body><div class='wrap'>");
            ...
            if (stores.Count > 0)
            {
                b.Append("<div class='nexo-directory-v8' data-card-design='NEXO-V8'>");
                foreach (CentralStore cs in stores)
                {
                    string href = "/store/" + Uri.EscapeDataString(cs.StoreId);
                    string d = cs.Distance > 0 ? cs.Distance.ToString("0.0") + " km" : "";
                    string logo = string.IsNullOrWhiteSpace(cs.Logo) ? "<span class='nexo-logo-letter'>N</span>" : "<img src='" + E(cs.Logo) + "' alt='" + E(cs.Name) + "' loading='lazy'/>";
                    string promoHtml = StorePromoBadges(cs.StoreId);
                    string cover = string.IsNullOrWhiteSpace(cs.StorePhoto) ? "<div class='nexo-cover-fallback'><span>NEXO</span>MARKET</div>" : "<img class='nexo-cover-image' src='" + E(cs.StorePhoto) + "' alt='" + E(cs.Name) + "' loading='lazy' decoding='async' onerror=\"this.style.display='none';this.nextElementSibling.classList.add('show')\"/>";
                    string[] ratingParts = (cs.RatingSummary ?? "0.0|0").Split('|');
                    string rating = ratingParts.Length > 0 ? ratingParts[0] : "0.0";
                    string reviewCount = ratingParts.Length > 1 ? ratingParts[1] : "0";
                    bool renderStorePlus = cs.StorePlus;
                    bool renderCommon = !renderStorePlus && cs.Featured;
                    string tier = renderStorePlus ? " nexo-store-plus" : (renderCommon ? " nexo-featured" : "");
                    string tierBadge = renderCommon ? "<span class='nexo-tier' data-tier='COMMON'>★ TIENDA DESTACADA</span>" : "";
                    string frontTier = renderStorePlus ? "<div class='nexo-front-tier plus'>✦ TIENDA PLUS</div>" : (renderCommon ? "<div class='nexo-front-tier common'>★ TIENDA DESTACADA</div>" : "");
                    string starCorner = (cs.StorePlus || cs.Featured) ? "<span class='nexo-star'>★</span>" : "";
                    string delivery = cs.Delivery ? "<span>🚚 Delivery</span>" : "";
                    string pickup = cs.Pickup ? "<span>🏪 Retiro</span>" : "";
                    string desc = string.IsNullOrWhiteSpace(cs.Description) ? "Comercio local en NexoMarket." : (cs.Description.Length > 105 ? cs.Description.Substring(0,105) + "…" : cs.Description);
                    b.Append("<a class='nexo-store-v8" + tier + "' href='" + E(href) + "' data-store-id='" + E(cs.StoreId) + "' data-store-plus='" + (cs.StorePlus ? "1" : "0") + "' data-store-tier='" + (cs.StorePlus ? "PLUS" : (cs.Featured ? "FEATURED" : "NONE")) + "'>" + frontTier + "<div class='nexo-cover-v8'>" + cover + "<div class='nexo-cover-fallback-layer'><span>NEXO</span>MARKET</div><div class='nexo-cover-shade'></div><div class='nexo-cover-glow'></div><div class='nexo-shine'></div>" + tierBadge + "</div><div class='nexo-logo-v8'><div class='nexo-logo-box'>" + logo + "</div>" + starCorner + "</div><div class='nexo-card-body'><div class='nexo-kicker'>" + E(cs.Category.Length == 0 ? "COMERCIO" : cs.Category) + " <i>·</i> " + E(cs.City) + "</div><h3>" + E(cs.Name) + "</h3><div class='nexo-status " + (cs.Active ? "open" : "closed") + "'>● " + (cs.Active ? "ABIERTO" : "CERRADO") + "</div><p class='nexo-description'>" + E(desc) + "</p><div class='nexo-facts'>" + (string.IsNullOrWhiteSpace(d) ? "" : "<span>📍 " + E(d) + "</span>") + delivery + pickup + "</div><div class='nexo-bottom'><span class='nexo-rating'>★ " + E(rating) + " <small>(" + E(reviewCount) + ")</small></span><span class='nexo-arrow'>VER LOCAL <b>→</b></span></div>" + promoHtml + "</div></a>");
                }
                b.Append("</div>");
            }
        ...
        private bool VerifyAccount(string email,string password,out CentralUser user)
        {
            ...
            try
            {
                ...
                byte[] actual=Rfc2898DeriveBytes.Pbkdf2(password??"",salt,50000,HashAlgorithmName.SHA1,32); if(actual.Length!=expected.Length)return false; int diff=0; for(int i=0;i<actual.Length;i++)diff|=actual[i]^expected[i]; return diff==0;
            } catch { return false; }
        }
        ...
        private void CentralSellerRegister(NetworkStream stream,string method,string body)
        {
            ...
            hash=Rfc2898DeriveBytes.Pbkdf2(password,salt,50000,HashAlgorithmName.SHA1,32);
        ...
        private CentralUser SessionUser(string cookie)
        {
            ...
            try
            {
                ...
                CentralUser u=FindAccount(email); if(u==null || (u.Role!="seller" && u.Role!="buyer"))return null;
        ...
        private void CentralSeller(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append("<script>(function(){var last='',lastPending=-1,lastOrderAt='',audioCtx=null,audioReady=false,orderAudio=document.getElementById('newOrderAudio'),orderSoundCount=0,orderSoundPlaying=false;function ensureAudio(){try{var C=window.AudioContext||window.webkitAudioContext;if(C){audioCtx=audioCtx||new C();if(audioCtx.state==='suspended')audioCtx.resume();audioReady=true;}if(orderAudio){orderAudio.load();}var u=document.getElementById('soundUnlock');if(u)u.className='sound-unlock';return true;}catch(e){return false;}}function stopOrderSound(){orderSoundPlaying=false;orderSoundCount=0;if(orderAudio){try{orderAudio.pause();orderAudio.currentTime=0;}catch(e){}}}function playOrderSound(){if(!orderAudio)return;try{orderAudio.currentTime=0;var p=orderAudio.play();if(p&&p.catch)p.catch(function(){});}catch(e){}}function startOrderSound(){ensureAudio();stopOrderSound();if(!orderAudio)return;orderSoundPlaying=true;orderSoundCount=0;playOrderSound();orderAudio.onended=function(){if(!orderSoundPlaying)return;orderSoundCount++;if(orderSoundCount>=10){stopOrderSound();return;}setTimeout(function(){if(orderSoundPlaying)playOrderSound();},120);};}function beep(){try{if(!audioReady&&!ensureAudio())return;var o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type='sine';o.frequency.setValueAtTime(880,audioCtx.currentTime);o.frequency.linearRampToValueAtTime(1175,audioCtx.currentTime+.12);g.gain.setValueAtTime(.07,audioCtx.currentTime);g.gain.exponentialRampToValueAtTime(.001,audioCtx.currentTime+.30);o.connect(g);g.connect(audioCtx.destination);o.start();o.stop(audioCtx.currentTime+.31);}catch(e){}}function notifySetup(){var el=document.getElementById('notifySetup'),btn=document.getElementById('enableNotifications');if(!el||!btn)return;var state=('Notification' in window)?Notification.permission:'unsupported';if(state==='granted'){el.className='notify-setup ok';btn.textContent='✓ NOTIFICACIONES ACTIVAS';btn.disabled=true;}else if(state==='denied'){el.className='notify-setup warn';btn.textContent='PERMISOS BLOQUEADOS';btn.disabled=false;}else{btn.textContent='ACTIVAR NOTIFICACIONES';btn.disabled=false;}}async function enableNotifications(){ensureAudio();if('Notification' in window){try{await Notification.requestPermission();}catch(e){}}notifySetup();}function osNotify(){if(!('Notification' in window)||Notification.permission!=='granted')return;try{var n=new Notification('NexoMarket · Nuevo pedido',{body:'Tenés un pedido nuevo pendiente. Abrí el Seller Center para revisarlo.',tag:'nexo-new-order',requireInteraction:true});n.onclick=function(){try{window.focus();}catch(e){}try{n.close();}catch(e){}};}catch(e){}}function showNewOrderAlert(){var el=document.getElementById('newOrderAlert');if(!el)return;el.className='new-order-alert show';clearTimeout(window.__nexoOrderAlertTimer);startOrderSound();try{if(navigator.vibrate)navigator.vibrate([180,90,180]);}catch(e){}osNotify();window.__nexoOrderAlertTimer=setTimeout(function(){el.className='new-order-alert';},10000);}function live(){var x=new XMLHttpRequest();x.open('GET','/api/seller/live',true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText),v=d.updatedAt||'',pending=parseInt(d.pendingOrders||0,10),orderAt=d.latestOrderAt||'';if(lastOrderAt&&orderAt&&orderAt!==lastOrderAt)showNewOrderAlert();else if(lastPending>=0&&pending>lastPending)showNewOrderAlert();lastOrderAt=orderAt;lastPending=pending;var editing=document.querySelector('form.product-form input:focus,form.product-form textarea:focus,form.edit-form input:focus,form.edit-form textarea:focus,form.settings-form input:focus,form.settings-form textarea:focus');var hasDraft=document.querySelector('form.product-form input:not([type=hidden])[value]:not([value=\"\"])')||document.querySelector('form.product-form textarea:not(:placeholder-shown)');if(last&&v!==last&&!editing&&!document.querySelector('details[open]')&&!hasDraft&&!document.querySelector('.order-cards'))location.reload();last=v;}catch(e){}}};x.send();}var nbtn=document.getElementById('enableNotifications');if(nbtn)nbtn.addEventListener('click',enableNotifications);var sun=document.getElementById('soundUnlock');if(sun)sun.addEventListener('click',function(){ensureAudio();beep();});document.addEventListener('pointerdown',function(){ensureAudio();},{once:true,passive:true});setTimeout(function(){notifySetup();if('Notification' in window&&Notification.permission==='default'){try{Notification.requestPermission().then(notifySetup).catch(function(){});}catch(e){}}},650);setTimeout(live,900);setInterval(live,2500);})();</script>");
        ...
        private string SellerSummaryView(List<XElement> orders,List<XElement> products)
        {
            ...
            b.Append("</table></section>" ); b.Append(SellerCommissionCard(CurrentSellerStoreId())); b.Append("<section class='card'><h3>Productos destacados</h3><div class='mini-list'>"); foreach(XElement p in products.Take(8)) b.Append("<div class='mini-row'><div><b>"+E(S(p,"Name"))+"</b><small>SKU "+E(S(p,"SKU"))+" · Stock "+E(S(p,"Stock"))+"</small></div><strong>$ "+Money(S(p,"SalePrice")=="0"?S(p,"Price"):S(p,"SalePrice")).ToString("N0")+"</strong></div>"); if(products.Count==0)b.Append("<p class='muted'>Sin productos sincronizados.</p>"); b.Append("</div></section></div><section class='card'><h3>Acciones rápidas</h3><div class='quick-grid'><a href='/seller?view=products'>"+SellerIcon("Productos")+"Gestionar catálogo e inventario</a><a href='/seller?view=orders'>"+SellerIcon("Pedidos")+"Atender pedidos</a><a href='/seller?view=customers'>"+SellerIcon("Clientes")+"Ver clientes</a><a href='/seller?view=analytics'>"+SellerIcon("Analítica")+"Analizar ventas</a><a href='/seller?view=finance'>"+SellerIcon("Finanzas")+"Revisar finanzas</a><a href='/seller?view=marketing'>"+SellerIcon("Marketing")+"Marketing y promociones</a></div></section>"); return b.ToString();
        }
        ...
        private string SellerProductsView(List<XElement> products)
        {
            ...
            b.Append("<div class='media-pickers'><div class='upload-box'><span>🖼️</span><b>Imagen principal</b><small>Elegí desde Galería, Fotos o Archivos. No se abre la cámara automáticamente.</small><input id='newImageFile' type='file' accept='image/jpeg,image/png,image/webp,image/gif' style='display:none'/><button type='button' class='btn' onclick='chooseSellerImage(document.getElementById(&quot;newImageFile&quot;))'>📁 ELEGIR IMAGEN</button></div><div class='media-preview' id='newMediaPreview'><span>La vista previa aparecerá aquí</span></div></div>");
        ...
        private string AdminStoreMediaUpload(string key,Dictionary<string,string> f)
        {
            string denied=AdminDenied(key); if(denied!=null)return denied;
            string storeId=NormalizeStoreId(Get(f,"storeId")); string target=(Get(f,"target")??"").Trim().ToLowerInvariant(); string productId=(Get(f,"productId")??"").Trim();
            if(storeId.Length==0)return "ERROR|store_required";
            if(target!="logo"&&target!="cover"&&target!="product")return "ERROR|invalid_target";
            XElement store;
            lock(_sync){store=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),storeId,StringComparison.OrdinalIgnoreCase));}
            if(store==null)return "ERROR|store_not_found";
            if(target=="product"&&productId.Length==0)return "ERROR|product_required";
            if(target=="product")
            {
                lock(_sync){XDocument d=LoadFile(_catalogFile,"NexoMarketCatalog","Products");XElement p=d.Root.Element("Products")==null?null:d.Root.Element("Products").Elements("Product").FirstOrDefault(x=>S(x,"StoreId")==storeId&&S(x,"ProductId")==productId);if(p==null)return "ERROR|product_not_found";}
            }
            string upload=UploadMedia(f);
            if(!upload.StartsWith("OK|",StringComparison.OrdinalIgnoreCase))return upload;
            string[] parts=upload.Split(new[]{'|'},3); string url=parts.Length>=3?parts[2]:""; if(url.Length==0)return "ERROR|media_url_missing";
            lock(_sync)
            {
                if(target=="logo") store.SetElementValue("Logo",url);
                else if(target=="cover") store.SetElementValue("StorePhoto",url);
                else { XDocument d=LoadFile(_catalogFile,"NexoMarketCatalog","Products"); XElement p=d.Root.Element("Products").Elements("Product").FirstOrDefault(x=>S(x,"StoreId")==storeId&&S(x,"ProductId")==productId); if(p==null)return "ERROR|product_not_found"; p.SetElementValue("WebImageUrl",url); p.SetElementValue("ImagePath",url); p.SetElementValue("UpdatedAt",DateTime.UtcNow.ToString("o")); SaveDoc(_catalogFile,d); }
                store.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o")); Save();
            }
            Audit("admin_media_updated",storeId,"",productId,target);
            return "OK|media|"+Escape(target)+"|"+Escape(url);
        }
        private string AdminStoreDetails(string key,string storeId)
        {
            ...
            StringBuilder b=new StringBuilder("{");
            string[] keys={"StoreId","Name","SystemName","Category","Description","City","Province","Address","Latitude","Longitude","Logo","StorePhoto","PublicUrl","Slug","Active","Listed","Featured","StorePlus","Delivery","Pickup","OpenTime","CloseTime","AutoSchedule","CreatedAt","UpdatedAt","LastActivityAt","CommissionPercent","CommissionDueDate","CommissionPaidMonth"};
            ...
            b.Append(",\"sellerEmail\":").Append(JsonString(seller==null?"":seller.Email)).Append(",\"sellerName\":").Append(JsonString(seller==null?"":seller.Name)).Append(",\"commission\":").Append(info);
            b.Append(",\"products\":["); bool pf=true;
            lock(_sync){XDocument cd=LoadFile(_catalogFile,"NexoMarketCatalog","Products"); XElement ps=cd.Root.Element("Products"); if(ps!=null) foreach(XElement p in ps.Elements("Product").Where(x=>S(x,"StoreId")==storeId&&S(x,"Deleted")!="1")){ if(!pf)b.Append(','); pf=false; b.Append("{\"productId\":").Append(JsonString(S(p,"ProductId"))).Append(",\"name\":").Append(JsonString(S(p,"Name"))).Append(",\"image\":").Append(JsonString(ProductImageUrl(p))).Append('}'); }}
            b.Append("]}");return b.ToString();
        }
        ...
        private string AdminResetAccountPassword(string key,Dictionary<string,string> f)
        {
            ...
            byte[] salt=new byte[16];using(var rng=RandomNumberGenerator.Create())rng.GetBytes(salt);string salt64=Convert.ToBase64String(salt);byte[] hash;hash=Rfc2898DeriveBytes.Pbkdf2(password,salt,50000,HashAlgorithmName.SHA1,32);string hash64=Convert.ToBase64String(hash);bool ok=false;
        ...
        private string AdminSetCommissionDueDate(string key,Dictionary<string,string> f)
        {
            string denied=AdminDenied(key); if(denied!=null)return denied;
            string id=NormalizeStoreId(Get(f,"storeId")); string raw=(Get(f,"dueDate")??"").Trim();
            if(id.Length==0)return "ERROR|store_required";
            DateTime due;
            if(!DateTime.TryParseExact(raw,"yyyy-MM-dd",CultureInfo.InvariantCulture,DateTimeStyles.None,out due))return "ERROR|invalid_due_date";
            lock(_sync)
            {
                ...
                st.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o")); Save();
            }
        ...
        private string SellerMarketingView(List<XElement> promotions)
        {
            ...
            b.Append("<section class='card coupon-section'><div class='coupon-section-head'><div><span class='eyebrow'>PROMOCIONES Y CUPONES</span><h3>Cupones activos</h3><p class='muted'>Diseño compacto tipo tarjeta: 3 por fila en celular y escritorio.</p></div><span class='coupon-count'>"+coupons.Count+" CUPONES</span></div><div class='coupon-grid'>");
            foreach(XElement c in coupons)
            {
                decimal cp=Money(S(c,"DiscountPercent")), ca=Money(S(c,"DiscountAmount")); string discount=cp>0?cp.ToString("0.##")+"% OFF":"$ "+ca.ToString("N0")+" OFF"; string uses=E(S(c,"Used"))+" / "+(S(c,"MaxUses")=="0"?"∞":E(S(c,"MaxUses")));
                b.Append("<article class='coupon-tile'><div class='coupon-ticket'><span class='coupon-label'>CUPÓN</span><strong>"+E(S(c,"Code"))+"</strong><em>"+E(discount)+"</em><small>"+E(string.IsNullOrWhiteSpace(S(c,"Description"))?"Usá este cupón en tu compra":S(c,"Description"))+"</small><div class='coupon-meta'><span>USOS "+uses+"</span><span>"+(S(c,"Active")=="0"?"PAUSADO":"ACTIVO")+"</span></div></div></article>");
            }
            if(coupons.Count==0)b.Append("<div class='empty coupon-empty'>No hay cupones creados todavía.</div>");
        ...
        private void CentralSellerStoreSave(NetworkStream stream,string cookie,Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                ...
                if(f.ContainsKey("phone")){string phone=Get(f,"phone").Trim();u.Phone=phone;XDocument ad=LoadFile(_accountsFile,"NexoMarketAccounts","Users");XElement ae=ad.Root.Element("Users").Elements("User").FirstOrDefault(x=>string.Equals(S(x,"Email"),u.Email,StringComparison.OrdinalIgnoreCase));if(ae!=null){ae.SetElementValue("Phone",phone);SaveDoc(_accountsFile,ad);}}
                store.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o")); Save();
            }
        ...
        private string SellerOnlineSettingsView(CentralUser u)
        {
            ...
            "<label>Ciudad<input name='city' value='"+E(S(store,"City"))+"'/></label><label>Provincia<input name='province' value='"+E(S(store,"Province"))+"'/></label><label>Teléfono público<input name='phone' value='"+E(u.Phone)+"' placeholder='Ej.: 11 5555 5555'/></label>"+
            ...
            "<div class='wide-label'><b>Logo del local</b><input id='storeLogoFile' type='file' accept='image/jpeg,image/png,image/webp,image/gif' style='display:none'/><button type='button' class='btn' onclick='chooseSellerImage(document.getElementById(&quot;storeLogoFile&quot;))'>📁 ELEGIR LOGO</button><small>Elegí desde Galería, Fotos o Archivos. No se abre la cámara automáticamente.</small><div id='storeLogoPreview' class='media-preview'>"+(string.IsNullOrWhiteSpace(logo)?"Sin logo cargado":"<img src='"+E(logo)+"' alt='Logo' />")+"</div></div>"+
            "<div class='wide-label'><b>Foto del local / portada</b><input id='storePhotoFile' type='file' accept='image/jpeg,image/png,image/webp,image/gif' style='display:none'/><button type='button' class='btn' onclick='chooseSellerImage(document.getElementById(&quot;storePhotoFile&quot;))'>📁 ELEGIR PORTADA</button><small>Elegí desde Galería, Fotos o Archivos. Esta foto se mostrará como portada pública.</small><div id='storePhotoPreview' class='media-preview'>"+(string.IsNullOrWhiteSpace(photo)?"Sin foto cargada":"<img src='"+E(photo)+"' alt='Local' />")+"</div></div>"+
        ...
        private void CentralBuyer(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append("</table></section><section class='card buyer-enhanced-panel'><div id='buyerNotice' class='buyer-notice' role='status'></div><div class='section-title'><div><span class='eyebrow'>SEGUIMIENTO INTELIGENTE</span><h2>Seguimiento de mis pedidos</h2><p>Actualización automática y confirmación de recepción.</p></div></div><div id='buyerTrackingCards'>");
            foreach(XElement o in orders){string bid=S(o,"CentralOrderId"),bst=S(o,"Status"),bstore=S(o,"StoreId"),bshort=bid.Length>10?bid.Substring(0,10):bid;bool bc=!string.IsNullOrWhiteSpace(S(o,"BuyerConfirmedAt"));b.Append("<article class='buyer-track' data-order='"+E(bid)+"' data-store='"+E(bstore)+"'><div class='buyer-track-head'><div><span class='eyebrow'>PEDIDO</span><h3>#"+E(bshort)+"</h3><small>"+E(GetStoreName(bstore))+" · "+E(S(o,"CreatedAt"))+"</small></div><div class='buyer-track-total'><strong>$ "+Money(S(o,"Total")).ToString("N2")+"</strong><span class='buyer-track-badge'>"+BadgeC(bst)+"</span></div></div><div class='buyer-progress' data-status='"+E(bst)+"'><span data-step='Pendiente'>1</span><span data-step='Preparando'>2</span><span data-step='Listo'>3</span><span data-step='Enviado'>4</span><span data-step='En reparto'>5</span><span data-step='Entregado'>6</span></div><div class='buyer-track-status'>ESTADO ACTUAL: <b>"+E(bst)+"</b></div><div class='buyer-track-actions'><button class='btn small ghost' type='button' onclick='buyerTrackDetail(this)'>VER PRODUCTOS</button><button class='btn small buyer-received' type='button' onclick='buyerTrackReceived(this)' style='display:"+(bst=="Entregado"&&!bc?"inline-block":"none")+"'>✓ CONFIRMAR RECEPCIÓN</button><span class='buyer-received-ok' style='display:"+(bc?"inline-block":"none")+"'>✓ RECEPCIÓN CONFIRMADA</span></div><div class='buyer-track-detail' style='display:none'></div></article>");}
            b.Append("</div></section></main>");
            b.Append("<style>.buyer-enhanced-panel{margin-top:16px}.buyer-track{background:linear-gradient(145deg,rgba(7,9,12,.92),rgba(18,11,29,.86));border:1px solid rgba(167,103,255,.18);border-radius:17px;padding:15px;margin-top:10px}.buyer-track-head{display:flex;justify-content:space-between;gap:12px}.buyer-track-head h3{margin:4px 0}.buyer-track-total{text-align:right}.buyer-track-total strong{display:block;font-size:20px}.buyer-progress{display:grid;grid-template-columns:repeat(6,1fr);gap:5px;margin:14px 0 6px}.buyer-progress span{display:flex;align-items:center;justify-content:center;height:24px;border-radius:999px;background:#111923;border:1px solid #273748;color:#687a8d;font-size:9px}.buyer-progress span.done{background:#153a22;border-color:#39ff66;color:#7cff9a}.buyer-progress span.current{background:#251638;border-color:#a767ff;color:#e1c7ff}.buyer-track-status{font-size:10px;color:#a9b6c3;margin:5px 0 8px}.buyer-track-status b{color:#fff}.buyer-track-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.buyer-received-ok{color:#39ff66;font-weight:900;font-size:11px}.buyer-track-detail{margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,.08)}.buyer-track-detail .bt-item{display:flex;gap:10px;align-items:center;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.06)}.buyer-track-detail img{width:56px;height:56px;object-fit:cover;border-radius:10px}.buyer-track-detail small{display:block;color:#8293a5;margin-top:3px}.buyer-orders-btn,.buyer-review-btn{background:linear-gradient(135deg,#b88a24,#f4d26a)!important;color:#0a0802!important}.buyer-notice{display:none;margin:12px 0;padding:14px;border:1px solid #39ff66;border-radius:14px;background:rgba(12,38,21,.92);color:#baffc9}.buyer-notice.show{display:block}@media(max-width:650px){.delivery-address-grid{grid-template-columns:1fr}.buyer-track-head{flex-direction:column}.buyer-track-total{text-align:left}.buyer-progress{grid-template-columns:repeat(3,1fr);row-gap:5px}.buyer-track-actions .btn{width:100%}}</style>");
            b.Append("<script>var buyerEmail="+JsonString(u.Email)+";function escBT(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;').replace(/'/g,'&#39;')}function buyerTrackDetail(btn){var c=btn.parentNode.parentNode,box=c.querySelector('.buyer-track-detail');if(box.style.display!=='none'){box.style.display='none';return}box.style.display='block';if(box.getAttribute('data-loaded')==='1')return;fetch('/api/orders/status?storeId='+encodeURIComponent(c.getAttribute('data-store'))+'&centralOrderId='+encodeURIComponent(c.getAttribute('data-order'))).then(function(r){return r.json()}).then(function(d){var h='';(d.items||[]).forEach(function(i){h+='<div class=\"bt-item\">'+(i.image?'<img src=\"'+escBT(i.image)+'\" alt=\"Producto\">':'<div class=\"od-product-img od-no-image\">SIN FOTO</div>')+'<div><b>'+escBT(i.name)+'</b><small>Cantidad: '+escBT(i.qty)+' · $ '+escBT(i.price)+'</small>'+(i.note?'<small>📝 '+escBT(i.note)+'</small>':'')+'</div></div>'});box.innerHTML=h||'<div class=\"muted\">No hay productos.</div>';box.setAttribute('data-loaded','1')}).catch(function(){box.innerHTML='<div class=\"muted\">No se pudo cargar el detalle.</div>'})}function buyerTrackReceived(btn){var c=btn.parentNode.parentNode,id=c.getAttribute('data-order'),store=c.getAttribute('data-store'),x=new XMLHttpRequest();x.open('POST','/api/orders/confirm',true);x.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=UTF-8');x.onreadystatechange=function(){if(x.readyState===4){if(x.status===200&&x.responseText.indexOf('OK|')===0){btn.style.display='none';c.querySelector('.buyer-received-ok').style.display='inline-block';}else alert('No se pudo confirmar la recepción.')}};x.send('storeId='+encodeURIComponent(store)+'&centralOrderId='+encodeURIComponent(id)+'&email='+encodeURIComponent(buyerEmail))}function refreshBuyerTrack(){document.querySelectorAll('.buyer-track').forEach(function(c){var id=c.getAttribute('data-order'),store=c.getAttribute('data-store');fetch('/api/orders/status?storeId='+encodeURIComponent(store)+'&centralOrderId='+encodeURIComponent(id)).then(function(r){return r.json()}).then(function(d){if(d.error)return;var b=c.querySelector('.buyer-received'),ok=c.querySelector('.buyer-received-ok'),rv=c.querySelector('.buyer-review-btn'),st=c.querySelector('.buyer-track-status b'),pg=c.querySelector('.buyer-progress'),badge=c.querySelector('.buyer-track-badge');if(st)st.textContent=d.status;buyerStatusNotice(id,d.status);if(pg){var steps=['Pendiente','Preparando','Listo','Enviado','En reparto','Entregado'],idx=steps.indexOf(d.status);pg.setAttribute('data-status',d.status);pg.querySelectorAll('span').forEach(function(el){var n=steps.indexOf(el.getAttribute('data-step'));el.className=n<idx||(d.status==='Entregado'&&n===idx)?'done':(n===idx?'current':'');});}if(badge){var bc=(d.status==='Cancelado'||d.status==='Rechazado')?'red':(d.status==='Pendiente')?'yellow':'green';badge.innerHTML='<span class=\"badge '+bc+'\">'+escBT(d.status)+'</span>';}if(b)b.style.display=(d.status==='Entregado'&&!d.buyerConfirmed)?'inline-block':'none';if(ok)ok.style.display=d.buyerConfirmed?'inline-block':'none';if(d.buyerConfirmed){if(!rv){rv=document.createElement('a');rv.className='btn small buyer-review-btn';rv.textContent='⭐ DEJAR RESEÑA';rv.href='/store/'+encodeURIComponent(store)+'?review='+encodeURIComponent(id);c.querySelector('.buyer-track-actions').appendChild(rv)}rv.style.display='inline-block';}else if(rv)rv.style.display='none';})})}var __buyerNoticeStatus={};function buyerStatusNotice(id,status){var k=id+':'+status;if(__buyerNoticeStatus[k])return;__buyerNoticeStatus[k]=1;var n=document.getElementById('buyerNotice');if(!n)return;n.innerHTML='🔔 <b>Pedido #'+escBT(id.substring(0,10))+'</b> · '+escBT(status);n.className='buyer-notice show';setTimeout(function(){n.className='buyer-notice'},7000)}refreshBuyerTrack();setInterval(refreshBuyerTrack,5000);</script>");b.Append("<style>.buyer-main{max-width:1200px;margin:auto}.store-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}.store-card{display:flex;gap:12px;align-items:center;background:#050507;border:1px solid #202025;border-radius:16px;padding:15px;color:#fff;text-decoration:none}.store-card:hover{border-color:#39ff66}.store-logo{width:64px;height:64px;border-radius:14px;background:#0b1118;border:1px solid #7b4bd1;color:#b98cff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:23px;overflow:hidden}.store-logo img{width:100%;height:100%;object-fit:cover}.store-card b,.store-card small,.store-card span{display:block}.store-card small{color:#8193a5;margin-top:4px}.store-card span{color:#39ff66;font-size:10px;margin-top:6px}@media(max-width:1000px){.store-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.store-grid{grid-template-columns:1fr}}</style>");b.Append(AuthShellEnd());Write(stream,200,"text/html; charset=utf-8",b.ToString());
        }
        ...
        {
            public string StoreId = ""; public string Name = ""; public string Category = ""; public string City = ""; public string Province = ""; public string PublicUrl = ""; public string Logo = ""; public string StorePhoto = ""; public string Address = ""; public string Description = ""; public string Phone = ""; public bool Delivery; public bool Pickup; public double Latitude; public double Longitude; public double Distance; public bool Active; public bool Featured; public bool StorePlus; public bool Listed; public string RatingSummary = "0.0|0";
            public CentralStore() { }
            public CentralStore(XElement e) { StoreId=S(e,"StoreId"); Name=S(e,"Name"); Category=S(e,"Category"); City=S(e,"City"); Province=S(e,"Province"); PublicUrl=S(e,"PublicUrl"); Logo=S(e,"Logo"); StorePhoto=S(e,"StorePhoto"); Address=S(e,"Address"); Description=S(e,"Description"); Delivery=S(e,"Delivery")=="1"; Pickup=S(e,"Pickup")=="1"; Active=S(e,"Active")=="1"; Featured=IsFeaturedCommon(e); StorePlus=IsStorePlus(e); Listed=S(e,"Listed")!="0"; RatingSummary="0.0|0"; double.TryParse(S(e,"Latitude"), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out Latitude); double.TryParse(S(e,"Longitude"), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out Longitude); }
        }
