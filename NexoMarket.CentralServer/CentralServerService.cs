...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private string Marketplace(string query, string cookie)
        {
            ...
            if (stores.Count > 0) { b.Append("<div class='grid'>"); foreach (CentralStore cs in stores) { string href = "/store/" + Uri.EscapeDataString(cs.StoreId); string d = cs.Distance > 0 ? cs.Distance.ToString("0.0") + " km · " : ""; string logo = string.IsNullOrWhiteSpace(cs.Logo) ? "<span>N</span>" : "<img src='" + E(cs.Logo) + "' alt='" + E(cs.Name) + "' loading='lazy'/>"; string promoHtml=StorePromoBadges(cs.StoreId); string photoHtml=string.IsNullOrWhiteSpace(cs.StorePhoto)?"":"<div class='store-photo-mini'><img src='"+E(cs.StorePhoto)+"' alt='"+E(cs.Name)+"' loading='lazy'/></div>"; b.Append("<a class='card' href='" + E(href) + "'><div class='logo'>" + logo + "</div>" + (cs.Featured ? "<div class='sale'>★ DESTACADA</div>" : "") + "<div class='name'>" + E(cs.Name) + "</div><div class='meta'>" + E(cs.Category.Length == 0 ? "Comercio" : cs.Category) + " · " + E(cs.City) + "</div><div class='open " + (cs.Active ? "" : "closed") + "'>● " + (cs.Active ? "Abierta" : "Cerrada") + " · " + (cs.Delivery ? "Delivery" : "Retiro") + "</div><div class='distance'>📍 " + E(d) + (cs.Delivery ? "🚚 Delivery" : "🏪 Retiro") + "</div><div class='meta'>"+E(cs.Address)+"</div><div class='meta'>"+E(cs.Description.Length>110?cs.Description.Substring(0,110)+"…":cs.Description)+"</div>"+promoHtml+photoHtml+"</a>"); } b.Append("</div>"); }
        ...
        private void CentralSeller(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append("<script>(function(){var last='',lastPending=-1,audioCtx=null;function beep(){try{audioCtx=audioCtx||new(window.AudioContext||window.webkitAudioContext)();var o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type='sine';o.frequency.value=880;g.gain.value=.08;o.connect(g);g.connect(audioCtx.destination);o.start();setTimeout(function(){o.frequency.value=1175;},120);setTimeout(function(){o.stop();},240);}catch(e){}}function showNewOrderAlert(){var el=document.getElementById('newOrderAlert');if(!el)return;el.className='new-order-alert show';clearTimeout(window.__nexoOrderAlertTimer);window.__nexoOrderAlertTimer=setTimeout(function(){el.className='new-order-alert';},10000);}function live(){var x=new XMLHttpRequest();x.open('GET','/api/seller/live',true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText),v=d.updatedAt||'',pending=parseInt(d.pendingOrders||0,10);if(lastPending>=0&&pending>lastPending){beep();showNewOrderAlert();}lastPending=pending;var editing=document.querySelector('form.product-form input:focus,form.product-form textarea:focus,form.edit-form input:focus,form.edit-form textarea:focus,form.settings-form input:focus,form.settings-form textarea:focus');var hasDraft=document.querySelector('form.product-form input:not([type=hidden])[value]:not([value=\"\"])')||document.querySelector('form.product-form textarea:not(:placeholder-shown)');if(last&&v!==last&&!editing&&!document.querySelector('details[open]')&&!hasDraft&&!document.querySelector('.order-cards'))location.reload();last=v;}catch(e){}}};x.send();}setTimeout(live,900);setInterval(live,2500);document.addEventListener('click',function(){if(audioCtx&&audioCtx.state==='suspended')audioCtx.resume();},{once:true});})();</script>");
