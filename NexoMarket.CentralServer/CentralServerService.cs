...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        {
            string baseUrl = (Environment.GetEnvironmentVariable("PUBLIC_BASE_URL") ?? "").TrimEnd('/');
            if (string.IsNullOrWhiteSpace(baseUrl)) return "";
            return baseUrl + "/media/" + Uri.EscapeDataString(key.TrimStart('/')).Replace("%2F", "/");
        }
        ...
        {
            if (_r2 == null || !_r2.Enabled || string.IsNullOrWhiteSpace(rawKey))
            {
            ...
            try
            {
                ...
                if (key.IndexOf("..", StringComparison.Ordinal) >= 0 || !key.StartsWith("stores/", StringComparison.OrdinalIgnoreCase))
                {
                    Write(stream, 404, "text/plain; charset=utf-8", "media_not_found");
                    return;
                }
                ...
                string contentType = ext == ".png" ? "image/png" : ext == ".webp" ? "image/webp" : ext == ".gif" ? "image/gif" : ext == ".svg" ? "image/svg+xml" : "image/jpeg";
        ...
        private string Storefront(string slug)
        {
            ...
            lock(_sync)
            {
                ...
                foreach (XElement x in ps)
                {
                    ...
                    string img = string.IsNullOrWhiteSpace(S(x, "WebImageUrl")) ? S(x, "ImagePath") : S(x, "WebImageUrl");
        ...
        private void CentralSellerStoreLogin(NetworkStream stream,string method,string body)
        {
            if(method=="GET")
            {
                string html="<div class='card seller-login-card'><div class='brand'><span>NEXO</span>MARKET <small>SELLER CENTER</small></div><div class='eyebrow'>CUENTA CENTRAL</div><h1>Ingresar como vendedor</h1><p class='muted'>La cuenta, el Store ID y los dispositivos autorizados pertenecen a una única identidad central.</p><form method='post' action='/seller-login'><label class='muted'>Correo</label><input name='email' type='email' autocomplete='username' required/><label class='muted'>Contraseña</label><input name='password' type='password' autocomplete='current-password' required/><label class='muted'>Store ID</label><input name='storeId' autocomplete='off' required/><button class='btn violet' type='submit'>INGRESAR AL SELLER CENTER</button></form><p class='muted small'>¿Todavía no vinculaste Windows? Entrá al Seller Center y usá <b>Herramientas → Dispositivos</b> para generar un QR/código temporal.</p></div>";
                Write(stream,200,"text/html; charset=utf-8",AuthPage("Vendedor · Cuenta central",html)); return;
            }
            Dictionary<string,string> f=Form(body); string email=Get(f,"email").Trim().ToLowerInvariant(), password=Get(f,"password"), storeId=NormalizeStoreId(Get(f,"storeId")); CentralUser u;
            if(!VerifyAccount(email,password,out u)||u==null||u.Role!="seller") { Write(stream,200,"text/html; charset=utf-8",AuthPage("Ingreso vendedor","<div class='error'>Correo, contraseña o cuenta de vendedor incorrectos.</div><a class='btn violet' href='/seller-login'>Volver a intentar</a>")); return; }
            if(!string.Equals(u.StoreId,storeId,StringComparison.OrdinalIgnoreCase)) { Write(stream,200,"text/html; charset=utf-8",AuthPage("Ingreso vendedor","<div class='error'>El Store ID no pertenece a esta cuenta.</div><p class='muted'>Usá el Store ID que aparece en tu Seller Center y en NexoMarket Windows.</p><a class='btn violet' href='/seller-login'>Volver</a>")); return; }
            string token=Guid.NewGuid().ToString("N"); lock(_sync)_sessions[token]=u;
        ...
        private void CentralSeller(NetworkStream stream,string cookie,string query)
        {
            ...
            else if(view=="devices") b.Append("<section class='card'><h2>Dispositivos y QR</h2><p>Administrá las PCs autorizadas desde la misma cuenta central.</p><a class='btn violet' href='/seller/devices'>ABRIR VINCULACIÓN DE WINDOWS</a></section>");
            else b.Append(SellerSummaryView(orders,products));
            b.Append("<script>(function(){var last='',dirty=false;document.addEventListener('input',function(e){if(e.target.closest&&e.target.closest('form.product-form,form.edit-form'))dirty=true;});document.addEventListener('change',function(e){if(e.target.closest&&e.target.closest('form.product-form,form.edit-form'))dirty=true;});function live(){var x=new XMLHttpRequest();x.open('GET','/api/seller/live',true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText);var v=d.updatedAt||'';var editing=document.querySelector('form.product-form input:focus,form.product-form textarea:focus,form.edit-form input:focus,form.edit-form textarea:focus');if(last&&v!==last&&!dirty&&!editing&&!document.querySelector('details[open]'))location.reload();last=v;}catch(e){}}};x.send();}setTimeout(live,700);setInterval(live,1800);})();</script>");
        ...
        {
            StringBuilder b=new StringBuilder();
            b.Append("<div class='section-title'><div><span class='eyebrow'>CATÁLOGO CENTRAL</span><h2>Productos e inventario</h2><p>El inventario se muestra como tarjetas con foto. Los cambios se guardan en Central y se sincronizan con NexoMarket Windows.</p></div><span class='sync-pill'>● CENTRAL EN TIEMPO REAL</span></div>");
            b.Append("<section class='card'><h3>Nuevo producto</h3><form method='post' action='/seller/products/save' class='product-form' autocomplete='off'><div class='form-grid'>");
            b.Append("<input name='name' placeholder='Nombre del producto' required/><input name='category' placeholder='Categoría'/><input name='brand' placeholder='Marca'/><input name='sku' placeholder='SKU'/><input name='barcode' placeholder='Código de barras'/><input name='price' placeholder='Precio' value='0'/><input name='salePrice' placeholder='Precio de oferta' value='0'/><input name='cost' placeholder='Costo' value='0'/><input name='stock' placeholder='Stock' value='0'/><input name='minimumStock' placeholder='Stock mínimo' value='0'/><input name='size' placeholder='Talle / tamaño'/><input name='color' placeholder='Color'/><input name='imageUrl' placeholder='URL pública de imagen'/><input name='videoUrl' placeholder='URL de video'/></div><textarea name='description' placeholder='Descripción interna'></textarea><textarea name='publicDescription' placeholder='Descripción pública'></textarea><label><input type='checkbox' name='onlineEnabled' value='1' checked/> Publicar online</label><label><input type='checkbox' name='active' value='1' checked/> Activo</label><button class='btn' type='submit'>GUARDAR PRODUCTO</button></form></section>");
            b.Append("<section class='card'><div class='section-title inventory-head'><div><span class='eyebrow'>INVENTARIO</span><h3>Productos con foto</h3></div><span class='muted'>"+products.Count+" productos</span></div><div class='inventory-grid'>");
            foreach(XElement x in products)
            {
                string pid=S(x,"ProductId"), img=S(x,"WebImageUrl");
                decimal stock=Money(S(x,"Stock")), min=Money(S(x,"MinimumStock"));
                string price=Money(S(x,"SalePrice")=="0"?S(x,"Price"):S(x,"SalePrice")).ToString("N2");
            ...
            b.Append("</div></section><section class='card'><h3>Sincronización</h3><p>Los cambios de catálogo se guardan en Central. La página ya no se recarga automáticamente mientras estás escribiendo un producto.</p><div class='sync-pill'>● Store ID único · ● cambios bidireccionales · ● formulario protegido</div></section>");
            return b.ToString();
        }
