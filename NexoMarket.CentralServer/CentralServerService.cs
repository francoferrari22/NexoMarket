...
namespace NexoMarket.CentralServer
{
    ...
    public sealed class CentralServerService : IDisposable
    {
            ...
            using (client)
            {
                try
                {
                    ...
                    using (NetworkStream stream = client.GetStream())
                    {
                        ...
                        if (path == "/api/version" && method == "GET") { Write(stream, 200, "application/json; charset=utf-8", "{\"version\":\"5.12.10\",\"ordersResolver\":\"product-evidence-recovery\",\"proofResolver\":\"same-product-evidence-recovery\"}"); return; }
        ...
        private void CentralSellerLive(NetworkStream stream, string cookie)
        {
            ...
            lock (_sync)
            {
                // LoadFile vuelve a consultar el almacenamiento externo, por lo que esta
                // consulta es válida aunque Render haya cambiado de instancia.
