    ...
    public sealed class CentralServerService : IDisposable
    {
            ...
            using (client)
            {
                try
                {
                    ...
                    using (NetworkStream stream = client.GetStream())
                    {
                        ...
                        if (path == "/api/admin/store/store-plus" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", AdminSetStorePlusCompat(HeaderValue(request,"X-Nexo-Admin-Key"), Form(body))); return; }
        ...
        private string Register(Dictionary<string,string> f)
        {
            ...
            lock (_sync)
            {
                ...
                // DESTACADA/TIENDA PLUS son controles exclusivos del Super Admin.
        ...
        private string AdminSetStoreFeatured(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync){XElement e=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),id,StringComparison.OrdinalIgnoreCase));if(e==null)return "ERROR|store_not_found";e.SetElementValue("Featured",featured?"1":"0");e.SetElementValue("FeaturedPlus","0");e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));Save();}
            return "OK|"+(featured?"featured":"unfeatured");
        }
        // TIENDA PLUS: API moderna del Super Administrador. Internamente reutiliza el estado FeaturedPlus legado
        // para conservar compatibilidad con los datos existentes sin tocar la web ni migrar registros a mano.
        private string AdminSetStorePlusCompat(string key,Dictionary<string,string> f)
        {
            Dictionary<string,string> mapped=new Dictionary<string,string>(StringComparer.OrdinalIgnoreCase);
            foreach(KeyValuePair<string,string> p in f) mapped[p.Key]=p.Value;
            mapped["featuredPlus"] = Get(f,"storePlus") == "0" ? "0" : "1";
            return AdminSetStoreFeaturedPlus(key,mapped);
        }
        private string AdminResetStoreStatus(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                ...
                e.SetElementValue("FeaturedPlus","0");
        ...
        private string AdminSetStoreFeaturedPlus(string key,Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                ...
                // TIENDA PLUS es un nivel exclusivo: nunca puede convivir con DESTACADA común.
                if(plus)e.SetElementValue("Featured","0");
                e.SetAttributeValue("UpdatedAt",DateTime.UtcNow.ToString("o"));
                Save();
            }
        ...
        private string Storefront(string slug, string cookie, string query)
        {
            ...
            string publicTier=featuredPlus?"<div class='public-tier plus'>✦ TIENDA PLUS</div>":(featured?"<div class='public-tier featured'>★ TIENDA DESTACADA</div>":"");
            StringBuilder b=new StringBuilder();
...
"@media(max-width:650px){.store-product-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px!important}.store-product-card{padding:7px!important}.store-product-card h3{font-size:14px}.store-product-card .product-description{font-size:10px}.store-product-card .price{font-size:17px}.store-product-card .btn{font-size:10px;padding:8px 5px}}@media(max-width:1000px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.grid{grid-template-columns:1fr;gap:12px}.grid .card{min-height:300px;height:300px}.grid .card .name{font-size:20px}.grid .card .logo-wrap,.grid .card .logo-wrap .logo{width:68px;height:68px}}</style></head><body><div class='wrap'><div class='top'><a class='brand brand-home' href='/' aria-label='Volver al inicio de NexoMarket'><span>NEXO</span>MARKET</a><div class='nav'>"+storeAccountLinks+"</div></div><button id='scrollHome' class='scroll-home' type='button' onclick=\"location.href='/'\">NEXO<span>MARKET</span></button>"+(string.IsNullOrWhiteSpace(S(store,"StorePhoto"))?"":"<div class='store-cover-shell'><img class='store-cover-photo' src='"+E(S(store,"StorePhoto"))+"' alt='Foto del local' loading='eager'/><div class='store-cover-caption'>"+E(S(store,"Name"))+"</div></div>")+"<section class='hero store-hero'>").Append(logo).Append("<div class='store-hero-main'>").Append(publicTier).Append("<div class='live'>● TIENDA CENTRAL EN TIEMPO REAL</div><h1>").Append(E(S(store,"Name"))).Append("</h1><div class='muted'>").Append(E(S(store,"Category"))).Append(" · ").Append(E(S(store,"City"))).Append("</div><p class='muted'>").Append(E(S(store,"Description"))).Append("</p></div><div class='hero-rating'><div class='hero-rating-label'>REPUTACIÓN</div><div class='hero-rating-score'><span id='heroReviewAverage'>0.0</span><span class='hero-rating-max'>/5</span></div><div class='hero-rating-stars' id='heroReviewStars'>☆☆☆☆☆</div><div class='hero-rating-count' id='heroReviewCount'>0 reseñas</div></div></section>"+(S(store,"Active")=="0"?"<div class='closed-store-banner'>🔴 TIENDA CERRADA · En este momento no se aceptan nuevos pedidos.</div>":"")+"<div class='section'><div style='display:flex;justify-content:space-between;align-items:end'><div><h2>Productos</h2><div class='muted'>El catálogo se actualiza automáticamente desde NexoMarket Central.</div></div><span id='liveState' class='live'>● LIVE</span></div><div id='products' class='grid store-product-grid'>");
        ...
        private string Marketplace(string query, string cookie)
        {
            ...
            List<CentralStore> stores = new List<CentralStore>();
            {
                ...
                using (StringReader reader = new StringReader(lines)) { string line; while ((line = reader.ReadLine()) != null) { string[] p = line.Split('|'); if (p.Length < 12) continue; CentralStore cs = new CentralStore(); cs.StoreId = Uri.UnescapeDataString(p[1]); cs.Name = Uri.UnescapeDataString(p[2]); cs.PublicUrl = Uri.UnescapeDataString(p[3]); cs.City = Uri.UnescapeDataString(p[4]); cs.Province = Uri.UnescapeDataString(p[5]); cs.Category = Uri.UnescapeDataString(p[6]); cs.Latitude = ParseDouble(Uri.UnescapeDataString(p[7])); cs.Longitude = ParseDouble(Uri.UnescapeDataString(p[8])); cs.Active = Uri.UnescapeDataString(p[9]) == "1"; cs.Delivery = Uri.UnescapeDataString(p[10]) == "1"; cs.Pickup = Uri.UnescapeDataString(p[11]) == "1"; cs.Distance = p.Length > 13 ? ParseDouble(Uri.UnescapeDataString(p[13])) : 0d; cs.Logo = p.Length > 14 ? Uri.UnescapeDataString(p[14]) : ""; cs.Featured = p.Length > 15 && Uri.UnescapeDataString(p[15]) == "1"; cs.StorePhoto = p.Length > 16 ? Uri.UnescapeDataString(p[16]) : ""; cs.Address = p.Length > 17 ? Uri.UnescapeDataString(p[17]) : ""; cs.Description = p.Length > 18 ? Uri.UnescapeDataString(p[18]) : ""; cs.RatingSummary = p.Length > 19 ? Uri.UnescapeDataString(p[19]) : "0.0|0"; cs.FeaturedPlus = p.Length > 20 && Uri.UnescapeDataString(p[20]) == "1"; cs.Phone = p.Length > 21 ? Uri.UnescapeDataString(p[21]) : ""; stores.Add(cs); } }
            }
            StringBuilder b = new StringBuilder();
