    ...
    public sealed class CentralServerService : IDisposable
    {
            ...
            using (client)
            {
                try
                {
                    ...
                    using (NetworkStream stream = client.GetStream())
                    {
                        ...
                        if (path == "/api/admin/account/commission" && method == "POST") { Write(stream, 200, "text/plain; charset=utf-8", AdminSetCommission(HeaderValue(request,"X-Nexo-Admin-Key"), Form(body))); return; }
        ...
        private string Register(Dictionary<string,string> f)
        {
            ...
            lock (_sync)
            {
                ...
                XElement e = new XElement("Store", new XAttribute("UpdatedAt", string.IsNullOrWhiteSpace(Get(f,"updatedAt")) ? DateTime.UtcNow.ToString("o") : Get(f, "updatedAt")),
                    ...
                    new XElement("Slug", Get(f, "slug")), new XElement("PublicUrl", Get(f, "publicUrl")), new XElement("Featured", Get(f,"featured") == "1" ? "1" : "0"), new XElement("Listed", Get(f,"listed") == "0" ? "0" : "1"), new XElement("Active", active),
        ...
        private string AdminAccounts(string key)
        {
            ...
            lock(_sync)
            {
                ...
                {
                    string rate=NormalizeCommission(S(e,"CommissionRate"));decimal rateValue=Money(rate);decimal sales=string.Equals(S(e,"Role"),"seller",StringComparison.OrdinalIgnoreCase)?StoreMonthSales(S(e,"StoreId"),CurrentMonth()):0m;decimal due=Math.Round(sales*rateValue/100m,2,MidpointRounding.AwayFromZero);
        ...
        {
            string month=CurrentMonth(); decimal rate=CommissionDecimal(u); decimal sales=StoreMonthSales(u.StoreId,month); decimal amount=Math.Round(sales*rate/100m,2,MidpointRounding.AwayFromZero); string status=rate>0?"Pendiente":"Sin porcentaje"; string requestedAt="",paidAt="";
            lock(_sync){XElement p=FindPlatformPayment(u.StoreId,month);if(p!=null){status=S(p,"Status");requestedAt=S(p,"RequestedAt");paidAt=S(p,"PaidAt");}}
            return "{\"storeId\":"+JsonString(u.StoreId)+",\"month\":"+JsonString(month)+",\"rate\":"+rate.ToString("0.####",CultureInfo.InvariantCulture)+",\"sales\":"+sales.ToString("0.00",CultureInfo.InvariantCulture)+",\"amount\":"+amount.ToString("0.00",CultureInfo.InvariantCulture)+",\"status\":"+JsonString(status)+",\"requestedAt\":"+JsonString(requestedAt)+",\"paidAt\":"+JsonString(paidAt)+"}";
        }
        private string PlatformFeeForStore(string storeId,string syncKey)
        {
            storeId=NormalizeStoreId(storeId);if(storeId.Length==0)return "{\"error\":\"store\"}";if(!ValidateStoreSyncKey(storeId,syncKey))return "{\"error\":\"sync_key\"}";CentralUser u=FindSellerByStore(storeId);return u==null?"{\"error\":\"seller\"}":BuildPlatformFeeJson(u);
        }
        ...
        private string StoreLines(string query)
        {
            ...
            else list = list.OrderByDescending(x => S(x.Element, "Featured") == "1").ThenByDescending(x => S(x.Element, "Active") != "0").ThenBy(x => S(x.Element, "City")).ThenBy(x => S(x.Element, "Name")).ToList();
            StringBuilder b = new StringBuilder();
        ...
        private string Storefront(string slug)
        {
            ...
            string realId=S(store,"StoreId"); string reviewSummary=ReviewSummaryForStore(realId); string[] reviewParts=reviewSummary.Split('|'); string reviewAvg=reviewParts.Length>0?reviewParts[0]:"0.0"; string reviewCount=reviewParts.Length>1?reviewParts[1]:"0"; string openTime=S(store,"OpenTime"); string closeTime=S(store,"CloseTime");
            StringBuilder b=new StringBuilder();
        ...
        private CentralUser FindSellerByStore(string storeId)
        {
            ...
            if(_database!=null && _database.Enabled)
            {
                ...
                if(a!=null)
                {
                    ...
                    try{lock(_sync){XDocument xd=LoadFile(_accountsFile,"NexoMarketAccounts","Users");XElement xe=xd.Root.Element("Users").Elements("User").FirstOrDefault(x=>string.Equals(S(x,"StoreId"),storeId,StringComparison.OrdinalIgnoreCase)&&string.Equals(S(x,"Role"),"seller",StringComparison.OrdinalIgnoreCase));if(xe!=null)dbUser.CommissionRate=NormalizeCommission(S(xe,"CommissionRate"));}}catch{}
                    return dbUser;
                }
            }
            lock(_sync)
            ...
            if (_database != null && _database.Enabled)
            {
                ...
                if (a != null)
                {
                    ...
                    try
                    {
                        lock(_sync)
                        {
                            XDocument xd=LoadFile(_accountsFile,"NexoMarketAccounts","Users");
                            XElement xe=xd.Root.Element("Users").Elements("User").FirstOrDefault(x=>string.Equals(S(x,"Email"),email.Trim(),StringComparison.OrdinalIgnoreCase));
                            if(xe!=null)dbUser.CommissionRate=NormalizeCommission(S(xe,"CommissionRate"));
                        }
                    }catch{}
                    return dbUser;
                }
