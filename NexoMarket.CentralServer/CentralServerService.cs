    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private string CreateOrder(Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                ...
                if(store==null)
                {
                    ...
                    store=_doc.Root.Element("Stores").Elements("Store").FirstOrDefault(x =>
        ...
        private string Heartbeat(Dictionary<string,string> f)
        {
            ...
            lock(_sync)
            {
                ...
                // Windows is the operational source for the local store status. Central
                // only changes Active here when the caller explicitly sends the value.
        ...
        private string Storefront(string slug)
        {
            ...
            lock(_sync)
            {
                ...
                foreach (XElement x in ps)
                {
                    ...
                    string onclick = "add(" + JsonString(id) + "," + JsonString(S(x, "Name")) + "," + JsonNumber(shown) + "," + JsonNumber(S(x, "Stock")) + ")";
