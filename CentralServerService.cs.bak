    ...
    public sealed class CentralServerService : IDisposable
    {
        ...
        private string StoreLines(string query)
        {
            ...
            lock (_sync)
            {
                ...
                {
                    // Closed stores remain visible in the directory; the card shows their status.
        ...
        private void CentralRegister(NetworkStream stream,string method,string body)
        {
            ...
            CentralUser u=FindAccount(email); string token=CreateSessionToken(u); lock(_sync)_sessions[token]=u; WriteRedirectCookie(stream,u.Role=="seller"?"/seller":"/buyer","NexoCentralSession="+token+"; Path=/; Max-Age=2592000; HttpOnly; SameSite=Lax");
        }
        ...
        private void CentralSeller(NetworkStream stream,string cookie,string query)
        {
            ...
            b.Append("<script>(function(){var last='';function live(){var x=new XMLHttpRequest();x.open('GET','/api/seller/live',true);x.onreadystatechange=function(){if(x.readyState===4&&x.status===200){try{var d=JSON.parse(x.responseText),v=d.updatedAt||'';var editing=document.querySelector('form.product-form input:focus,form.product-form textarea:focus,form.edit-form input:focus,form.edit-form textarea:focus,form.settings-form input:focus,form.settings-form textarea:focus');var hasDraft=document.querySelector('form.product-form input:not([type=hidden])[value]:not([value=\"\"])')||document.querySelector('form.product-form textarea:not(:placeholder-shown)');if(last&&v!==last&&!editing&&!document.querySelector('details[open]')&&!hasDraft&&!document.querySelector('.order-cards'))location.reload();last=v;}catch(e){}}};x.send();}setTimeout(live,900);setInterval(live,2500);})();</script>");
        ...
        private string SellerProductsView(List<XElement> products)
        {
            ...
            b.Append("<script>function filterInventory(){var q=(document.getElementById('inventorySearch').value||'').toLowerCase(),f=document.getElementById('inventoryStockFilter').value;document.querySelectorAll('#inventoryGrid .inventory-card').forEach(function(c){var ok=c.getAttribute('data-name').indexOf(q)>=0&&(f==='all'||c.getAttribute('data-stock')===f);c.style.display=ok?'block':'none';});}function imageForUpload(file){return new Promise(function(resolve,reject){if(!file){resolve(null);return}if(file.type.indexOf('image/')!==0){resolve(file);return}var max=1600,r=new FileReader();r.onload=function(){var im=new Image();im.onload=function(){var w=im.naturalWidth,h=im.naturalHeight;if(w>max||h>max){var s=Math.min(max/w,max/h);w=Math.max(1,Math.round(w*s));h=Math.max(1,Math.round(h*s));}var c=document.createElement('canvas');c.width=w;c.height=h;c.getContext('2d').drawImage(im,0,0,w,h);var quality=.88,data=c.toDataURL('image/jpeg',quality);while(data.length>2500000&&quality>.55){quality-=.06;data=c.toDataURL('image/jpeg',quality);}var raw=atob(data.split(',')[1]);var arr=new Uint8Array(raw.length);for(var i=0;i<raw.length;i++)arr[i]=raw.charCodeAt(i);resolve(new Blob([arr],{type:'image/jpeg'}));};im.onerror=function(){resolve(file)};im.src=r.result;};r.onerror=function(){reject(new Error('No se pudo leer la imagen.'))};r.readAsDataURL(file);});}function toBase64Url(buf){var bytes=new Uint8Array(buf),bin='';for(var i=0;i<bytes.length;i++)bin+=String.fromCharCode(bytes[i]);var out=btoa(bin).split('+').join('-').split('/').join('_');while(out.length&&out.charAt(out.length-1)==='=')out=out.slice(0,-1);return out;}function uploadMedia(file,kind){return new Promise(async function(resolve,reject){if(!file){resolve('');return}try{if(kind==='image')file=await imageForUpload(file);var limit=kind==='image'?2500000:8*1024*1024;if(file.size>limit){reject(new Error(kind==='image'?'La foto supera 2,5 MB incluso después de optimizarla.':'El video supera 8 MB.'));return}var buf=await file.arrayBuffer(),base=toBase64Url(buf),name=(file.name||('nexomarket-'+kind)).replace(/[^a-zA-Z0-9._-]/g,'_');if(kind==='image')var dot=name.lastIndexOf('.');if(dot>=0)name=name.substring(0,dot)+'.jpg';var data='fileName='+encodeURIComponent(name)+'&contentType='+encodeURIComponent(kind==='image'?'image/jpeg':(file.type||'application/octet-stream'))+'&base64='+base;var x=new XMLHttpRequest();x.open('POST','/seller/media/upload',true);x.timeout=60000;x.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=UTF-8');x.onreadystatechange=function(){if(x.readyState===4){if(x.status===200&&x.responseText.indexOf('OK|')===0)resolve(x.responseText.split('|')[2]);else reject(new Error(x.responseText||('HTTP '+x.status)));}};x.ontimeout=function(){reject(new Error('El servidor tardó demasiado en guardar el archivo.'));};x.onerror=function(){reject(new Error('No se pudo conectar con NexoMarket Central para subir el archivo.'));};x.send(data);}catch(err){reject(err);}});}function previewFile(input){var p=document.getElementById('newMediaPreview'),f=input.files&&input.files[0];if(!f)return;var u=URL.createObjectURL(f);p.innerHTML=f.type.indexOf('video/')===0?'<video src=\"'+u+'\" controls></video>':'<img src=\"'+u+'\"/>';}</script>");
            b.Append("<script>document.getElementById('newImageFile').addEventListener('change',function(){previewFile(this)});document.getElementById('newVideoFile').addEventListener('change',function(){previewFile(this)});document.getElementById('newProductForm').addEventListener('submit',async function(e){e.preventDefault();var btn=document.getElementById('saveProductBtn'),state=document.getElementById('uploadState');btn.disabled=true;state.textContent='Preparando archivos...';try{var im=await uploadMedia(document.getElementById('newImageFile').files[0],'image');document.getElementById('newImageUrl').value=im||'';state.textContent='Foto guardada. Subiendo video...';var vid=await uploadMedia(document.getElementById('newVideoFile').files[0],'video');document.getElementById('newVideoUrl').value=vid||'';state.textContent='Guardando producto...';this.submit();}catch(err){state.textContent='Error: '+err.message;btn.disabled=false;}});document.querySelectorAll('.edit-form').forEach(function(form){form.addEventListener('submit',async function(e){var input=form.querySelector('.editImageFile');if(!input||!input.files||!input.files[0])return;e.preventDefault();var btn=form.querySelector('button[type=submit]'),file=input.files[0];if(btn)btn.disabled=true;try{var url=await uploadMedia(file,'image');form.querySelector('input[name=imageUrl]').value=url||form.querySelector('input[name=imageUrl]').value;form.submit();}catch(err){alert('No se pudo subir la nueva foto: '+err.message);if(btn)btn.disabled=false;}});});</script>");
            return b.ToString();
        }
